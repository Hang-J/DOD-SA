...
__all__ = ['DETRLoss', 'DINOLoss', 'DINOLoss_Paired', 'DETRLoss_Paired','DINOLoss_Decoupled_Position']
...
@register
class DETRLoss_Decoupled_Position(nn.Layer):
    __shared__ = ['num_classes', 'use_focal_loss']
    __inject__ = ['matcher']
    ...
    def __init__(self,
        ...
        Args:
            ...
            use_focal_loss (bool): Use focal loss or not.
        """
        super(DETRLoss_Decoupled_Position, self).__init__()
    ...
    def _get_loss_bbox(self, boxes_vis, boxes_ir, gt_bbox_vis, gt_bbox_ir, match_indices, num_gts,
        ...
        if self.paired_gt_bbox_match_shape != None:
            ...
            for i, (idx_tensor, value_tensor) in enumerate(sorted_match_indices):
                n = self.paired_gt_bbox_match_shape[i]  # 取前 n 个值
            ...
            if not is_all_empty(sorted_match_indices_match):
                ...
                bbox_vis_match_loss= self.loss_coeff['bbox'] * F.l1_loss(
                    src_bbox_vis[:, :4], target_bbox_vis, reduction='sum') / num_gts
                giou_vis_match_loss = self.giou_loss(
                    bbox_cxcywh_to_xyxy(src_bbox_vis), bbox_cxcywh_to_xyxy(target_bbox_vis))
            ...
            if not is_all_empty(sorted_match_indices_unmatch):
                ...
                bbox_vis_unmatch_loss= self.loss_coeff['bbox'] * F.l1_loss(
                    src_bbox_vis[:, :4], target_bbox_vis, reduction='sum') / num_gts
                giou_vis_unmatch_loss = self.giou_loss(
                    bbox_cxcywh_to_xyxy(src_bbox_vis), bbox_cxcywh_to_xyxy(target_bbox_vis))
    ...
    def forward(self,
        ...
        return total_loss
@register
class DINOLoss_Decoupled_Position(DETRLoss_Decoupled_Position):
    def forward(self,
                boxes_vis,
                boxes_ir,
                logits,
                ...
                generate_vis_pic=None,
                generate_ir_pic=None,
                paired_gt_bbox_match_shape=None,
                paired_gt_bbox_unmatch_shape=None,
                **kwargs):
        self.paired_gt_bbox_match_shape=paired_gt_bbox_match_shape
        self.paired_gt_bbox_unmatch_shape=paired_gt_bbox_unmatch_shape
        num_gts = self._get_num_gts(gt_class)
        num_quires = 300
        total_loss = super(DINOLoss_Decoupled_Position, self).forward(
        ...
        if dn_meta is not None:
            ...
            # compute denoising training loss
            num_gts *= dn_num_group
            dn_loss = super(DINOLoss_Decoupled_Position, self).forward(
                dn_out_bboxes_vis,
                dn_out_bboxes_ir,
                dn_out_logits,
                gt_bbox_vis,

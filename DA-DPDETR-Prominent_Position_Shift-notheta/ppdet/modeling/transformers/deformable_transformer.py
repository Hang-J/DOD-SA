...
class MSDeformableAttention_Position_Decoupled(nn.Layer):
    def __init__(self,
                 embed_dim=256,
                 num_heads=8,
                 num_levels=4,
        ...
        """
        super(MSDeformableAttention_Position_Decoupled, self).__init__()
        ...
        self.sampling_offsets_vis = nn.Linear(
            embed_dim,
            self.total_points * 2 // 2,
            weight_attr=ParamAttr(learning_rate=lr_mult),
            bias_attr=ParamAttr(learning_rate=lr_mult))

        self.sampling_offsets_ir = nn.Linear(
            embed_dim,
            self.total_points * 2 // 2,
            weight_attr=ParamAttr(learning_rate=lr_mult),
            bias_attr=ParamAttr(learning_rate=lr_mult))
    ...
    def _reset_parameters(self):
        # sampling_offsets
        constant_(self.sampling_offsets_vis.weight)
        constant_(self.sampling_offsets_ir.weight)
        ...
        grid_init = grid_init.reshape([self.num_heads, 1, 1, 2]).tile(
            [1, self.num_levels//2, self.num_points, 1])
        scaling = paddle.arange(
            1, self.num_points + 1,
            dtype=paddle.float32).reshape([1, 1, -1, 1])
        grid_init *= scaling
        self.sampling_offsets_vis.bias.set_value(grid_init.flatten())
        self.sampling_offsets_ir.bias.set_value(grid_init.flatten())
        # attention_weights
...
class MSDeformableAttention_Position_split(nn.Layer):
    def __init__(self,
                 embed_dim=256,
                 num_heads=8,
                 num_levels=4,
        ...
        Multi-Scale Deformable Attention Module
        """
        super(MSDeformableAttention_Position_split, self).__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.num_levels = num_levels
        self.num_points = num_points
...
class MSDeformableAttention_split_attention(nn.Layer):
    def __init__(self,
                 embed_dim=256,
                 num_heads=8,
                 num_levels=4,
        ...
        Multi-Scale Deformable Attention Module
        """
        super(MSDeformableAttention_split_attention, self).__init__()
        ...
        self.attention_weights_vis = nn.Linear(embed_dim, self.total_points)
        self.attention_weights_ir = nn.Linear(embed_dim, self.total_points)
        self.vis_ir_weight = nn.Linear(embed_dim, 2)
        self.value_proj = nn.Linear(embed_dim, embed_dim)
        self.output_proj = nn.Linear(embed_dim, embed_dim)
        try:
            # use cuda op
    ...
    def _reset_parameters(self):
        ...
        self.sampling_offsets_ir.bias.set_value(grid_init.flatten())
        # attention_weights
        constant_(self.attention_weights_vis.weight)
        constant_(self.attention_weights_vis.bias)
        constant_(self.attention_weights_ir.weight)
        constant_(self.attention_weights_ir.bias)
        constant_(self.vis_ir_weight.weight)
        constant_(self.vis_ir_weight.bias)
        # proj
        xavier_uniform_(self.value_proj.weight)
        constant_(self.value_proj.bias)
        xavier_uniform_(self.output_proj.weight)
    ...
    def forward(self,
        ...
        sampling_offsets_vis = self.sampling_offsets_vis(query).reshape(
            [bs, Len_q, self.num_heads, self.num_levels, self.num_points, 2])
        attention_weights_vis = self.attention_weights_vis(query).reshape(
            [bs, Len_q, self.num_heads, self.num_levels * self.num_points])
        attention_weights_vis = F.softmax(attention_weights_vis).reshape(
            [bs, Len_q, self.num_heads, self.num_levels, self.num_points])

        sampling_offsets_ir = self.sampling_offsets_ir(query).reshape(
            [bs, Len_q, self.num_heads, self.num_levels, self.num_points, 2])
        attention_weights_ir = self.attention_weights_ir(query).reshape(
            [bs, Len_q, self.num_heads, self.num_levels * self.num_points])
        attention_weights_ir = F.softmax(attention_weights_ir).reshape(
            [bs, Len_q, self.num_heads, self.num_levels, self.num_points])

        attention_weights_vis_ir = F.softmax(self.vis_ir_weight(query))
        ...
        if reference_points.shape[-1] == 2:
            ...
            sampling_locations_vis = (
                reference_points[:, :, None, :, None, :2] + sampling_offsets_vis /
                self.num_points * reference_points[:, :, None, :, None, 2:] *
                0.5)
            sampling_locations_ir = (
                    reference_points[:, :, None, :, None, :2] + sampling_offsets_ir /
                    self.num_points * reference_points[:, :, None, :, None, 2:] *
                    0.5)
        else:
            raise ValueError(
...
class MSDeformableAttention_Missing(nn.Layer):
    def __init__(self,
                 embed_dim=256,
                 num_heads=8,
                 num_levels=4,
        ...
        Multi-Scale Deformable Attention Module
        """
        super(MSDeformableAttention_Missing, self).__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.num_levels = num_levels
        self.num_points = num_points
        self.total_points = num_heads * num_levels * num_points
        self.total_points_vis = num_heads * (num_levels//2) * num_points
        self.total_points_ir = num_heads * (num_levels//2) * num_points
        ...
        self.sampling_offsets_vis = nn.Linear(
            embed_dim,
            self.total_points_vis * 2,
            weight_attr=ParamAttr(learning_rate=lr_mult),
            bias_attr=ParamAttr(learning_rate=lr_mult))

        self.sampling_offsets_ir = nn.Linear(
            embed_dim,
            self.total_points_ir * 2,
            weight_attr=ParamAttr(learning_rate=lr_mult),
            bias_attr=ParamAttr(learning_rate=lr_mult))

        self.attention_weights = nn.Linear(embed_dim, self.total_points)
        self.attention_weights_vis = nn.Linear(embed_dim, self.total_points_vis)
        self.attention_weights_ir = nn.Linear(embed_dim, self.total_points_ir)
        self.value_proj = nn.Linear(embed_dim, embed_dim)
        self.output_proj = nn.Linear(embed_dim, embed_dim)
        try:
            # use cuda op
    ...
    def _reset_parameters(self):
        ...
        grid_init_single = paddle.stack([thetas.cos(), thetas.sin()], -1)
        grid_init_single = grid_init_single / grid_init_single.abs().max(-1, keepdim=True)
        grid_init_single = grid_init_single.reshape([self.num_heads, 1, 1, 2]).tile(
            [1, self.num_levels // 2, self.num_points, 1])
        grid_init_single *= scaling

        self.sampling_offsets.bias.set_value(grid_init.flatten())
        self.sampling_offsets_ir.bias.set_value(grid_init_single.flatten())
        self.sampling_offsets_vis.bias.set_value(grid_init_single.flatten())
        # attention_weights
        constant_(self.attention_weights.weight)
        constant_(self.attention_weights.bias)
        constant_(self.attention_weights_vis.weight)
...
class MSDeformableAttention_Missing_3samplinglayer(nn.Layer):
    def __init__(self,
                 embed_dim=256,
                 num_heads=8,
                 num_levels=4,
        ...
        Multi-Scale Deformable Attention Module
        """
        super(MSDeformableAttention_Missing_3samplinglayer, self).__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.num_levels = num_levels
        self.num_points = num_points
        self.total_points = num_heads * num_levels * num_points
        self.total_points_vis = num_heads * (num_levels) * num_points
        self.total_points_ir = num_heads * (num_levels) * num_points

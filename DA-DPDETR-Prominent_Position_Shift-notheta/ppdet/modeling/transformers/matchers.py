...
__all__ = ['HungarianMatcher','HungarianMatcher_Rotate','HungarianMatcher_Rotate_Paired','HungarianMatcher_Decoupled_Position']
...
@register
@serializable
class HungarianMatcher_Rotate(nn.Layer):
    __shared__ = ['use_focal_loss', 'with_mask', 'num_sample_points']
    ...
    def __init__(self,
        ...
        Args:
            matcher_coeff (dict): The coefficient of hungarian matcher cost.
        """
        super(HungarianMatcher_Rotate, self).__init__()
        self.matcher_coeff = matcher_coeff
        self.use_focal_loss = use_focal_loss
        self.with_mask = with_mask
        self.num_sample_points = num_sample_points
    ...
    def forward(self,
        ...
        out_bbox = boxes.detach().flatten(0, 1)
        out_angles_cls = angles_cls.detach().flatten(0, 1)
        ...
        tgt_bbox_nomalized = tgt_bbox[:,:4] / im_shape
        tgt_bbox_nomalized = paddle.concat([tgt_bbox_nomalized, tgt_bbox[:,-1].unsqueeze(axis=1)], axis=-1)

        # Compute the L1 cost between boxes
        cost_bbox = (
            out_bbox[:,:4].unsqueeze(1) - tgt_bbox_nomalized[:,:4].unsqueeze(0)).abs().sum(-1)

        out_bbox_real = out_bbox[:, :4] * im_shape
        out_bbox_real = paddle.concat([out_bbox_real, out_bbox[:,-1].unsqueeze(axis=1)], axis=-1)
        # Compute the giou cost betwen boxes
        cost_piou = self.piou_loss(
            out_bbox_real.unsqueeze(1),
            tgt_bbox.unsqueeze(0),cost=True)

        tgt_angle_pos = (
                tgt_bbox[:, 4] /
                self.half_pi_bin).clip(0, self.angle_max - 0.01)
        cost_dfl = self._df_cost(out_angles_cls, tgt_angle_pos)
        ...
        C = self.matcher_coeff['class'] * cost_class + \
            self.matcher_coeff['bbox'] * cost_bbox + \
            self.matcher_coeff['giou'] * cost_piou + \
            0.05 * cost_dfl
        # Compute the mask cost and dice cost
        if self.with_mask:
            assert (masks is not None and gt_mask is not None,
...
@register
@serializable
class HungarianMatcher_Rotate_Paired(nn.Layer):
    __shared__ = ['use_focal_loss', 'with_mask', 'num_sample_points']
    ...
    def __init__(self,
        ...
        Args:
            matcher_coeff (dict): The coefficient of hungarian matcher cost.
        """
        super(HungarianMatcher_Rotate_Paired, self).__init__()
        self.matcher_coeff = matcher_coeff
        self.use_focal_loss = use_focal_loss
        self.with_mask = with_mask
        self.num_sample_points = num_sample_points
    ...
    def forward(self,
        ...
        #     C[i] = paddle.where(paddle.isnan(C[i]), paddle.full_like(C[i],1e6), C[i])
...
@register
@serializable
class HungarianMatcher_Paired(nn.Layer):
    __shared__ = ['use_focal_loss', 'with_mask', 'num_sample_points']
    ...
    def __init__(self,
        ...
        Args:
            matcher_coeff (dict): The coefficient of hungarian matcher cost.
        """
        super(HungarianMatcher_Paired, self).__init__()
        self.matcher_coeff = matcher_coeff
        self.use_focal_loss = use_focal_loss
        self.with_mask = with_mask
        self.num_sample_points = num_sample_points

...
from .deformable_transformer import MSDeformableAttention,MSDeformableAttention_Position_Decoupled,MSDeformableAttention_key_aware,MSDeformableAttention_Missing,MSDeformableAttention_Missing_3samplinglayer,MSDeformableAttention_split_attention, MSDeformableAttention_Position_split
...
import numpy as np
import cv2
__all__ = ['RTDETRTransformer', 'DPDETR_Transformer']
class PPMSDeformableAttention_Position_Decoupled(MSDeformableAttention_Position_Decoupled):
    def forward(self,
                gt_meta,
                query,
                query_vis_sampling,
        ...
        sampling_locations_vis = (
            reference_points_vis[:, :, None, :, None, :2] + sampling_offsets_vis /
            self.num_points * reference_points_vis[:, :, None, :, None, 2:] *
            0.5)
        sampling_locations_ir = (
                reference_points_ir[:, :, None, :, None, :2] + sampling_offsets_ir /
                self.num_points * reference_points_ir[:, :, None, :, None, 2:] *
                0.5)
        ...
        return output
class TransformerDecoderLayer_Decoupled(nn.Layer):
    def __init__(self,
                 d_model=256,
                 n_head=8,
        ...
        super(TransformerDecoderLayer_Decoupled, self).__init__()
        ...
        self.cross_attn_cls = PPMSDeformableAttention_Position_Decoupled(d_model, n_head, n_levels,
                                              n_points, 1.0)
        self.cross_attn_visp = PPMSDeformableAttention_Position_Decoupled(d_model, n_head, n_levels,
                                                      n_points, 1.0)
        self.cross_attn_irp = PPMSDeformableAttention_Position_Decoupled(d_model, n_head, n_levels,
                                                      n_points, 1.0)
    ...
    def forward(self,
        ...
        tgt2_cls = self.cross_attn_cls(gt_meta,
            ...
            self.with_pos_embed(tgt_cls, query_pos_embed_ir), reference_points_vis, reference_points_ir, memory,
            memory_spatial_shapes, memory_level_start_index, memory_mask,topk_ind_mask,topk_score,flag='cls')
        tgt2_visp = self.cross_attn_visp(gt_meta,
                                   self.with_pos_embed(tgt_visp, query_pos_embed_vis),
                                   self.with_pos_embed(tgt_visp, query_pos_embed_vis),
                                   self.with_pos_embed(tgt_visp, query_pos_embed_ir), reference_points_vis,
                                   reference_points_ir, memory,
                                   memory_spatial_shapes, memory_level_start_index, memory_mask, topk_ind_mask,
                                   topk_score,flag='visp')
        tgt2_irp = self.cross_attn_irp(gt_meta,
                                    ...
                                    self.with_pos_embed(tgt_irp, query_pos_embed_ir), reference_points_vis,
                                    reference_points_ir, memory,
                                    memory_spatial_shapes, memory_level_start_index, memory_mask, topk_ind_mask,
                                    topk_score,flag='irp')
...
class TransformerDecoder_Decoupled(nn.Layer):
    def __init__(self, hidden_dim, decoder_layer, num_layers, eval_idx=-1):
        super(TransformerDecoder_Decoupled, self).__init__()
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            output_cls, output_visp, output_irp = layer(gt_meta, output_cls, output_visp, output_irp, ref_points_input_vis, ref_points_input_ir, memory,
                           memory_spatial_shapes, memory_level_start_index,
                           attn_mask, memory_mask, query_pos_embed_vis, query_pos_embed_ir,topk_ind_mask,topk_score, dn_meta)
            ...
            if self.training:
                ...
                if i == 0:
                    ...
                    dec_out_bboxes_vis.append(F.sigmoid(bbox_head_vis[i](output_visp) + inverse_sigmoid(
                            ref_points_vis)))
                    dec_out_bboxes_ir.append(F.sigmoid(bbox_head_ir[i](output_irp) + inverse_sigmoid(
                        ref_points_ir)))
        ...
        return paddle.stack(dec_out_bboxes_vis), paddle.stack(dec_out_bboxes_ir), paddle.stack(dec_out_logits)
...
@register
class DPDETR_Transformer(nn.Layer):
    __shared__ = ['num_classes', 'hidden_dim', 'eval_size']
    ...
    def __init__(self,
                 ...
                 proj_all = False,
                 split_attention = False):
        super(DPDETR_Transformer, self).__init__()
        assert position_embed_type in ['sine', 'learned'], \
            f'ValueError: position_embed_type not supported {position_embed_type}!'
        assert len(backbone_feat_channels) <= num_levels
        assert len(feat_strides) == len(backbone_feat_channels)
        ...
        # Transformer module
        decoder_layer = TransformerDecoderLayer_Decoupled(
            hidden_dim, nhead, dim_feedforward, dropout, activation, num_visir_levels,
            num_decoder_points,key_aware=key_aware,split_attention=split_attention)
        self.decoder = TransformerDecoder_Decoupled(hidden_dim, decoder_layer,
                                          num_decoder_layers, eval_idx)
    ...
    def _reset_parameters(self):
        ...
        for cls_, reg_, reg_ir in zip(self.dec_score_head, self.dec_bbox_head_vis, self.dec_bbox_head_ir):
            linear_init_(cls_)
            constant_(cls_.bias, bias_cls)
            constant_(reg_.layers[-1].weight)
            constant_(reg_.layers[-1].bias)
    ...
    def forward(self, feats, vis_feats, ir_feats, pad_mask=None, gt_meta=None,topk_ind_mask=None,topk_score = None):
        ...
        # prepare denoising training
        if self.training:
            denoising_class, denoising_bbox_unact_vis, denoising_bbox_unact_ir, attn_mask, dn_meta = \
                get_contrastive_denoising_training_group_paired(gt_meta,
                                            self.num_classes,
                                            self.num_queries,
                                            self.denoising_class_embed.weight,
                                            self.num_denoising,
            ...
            denoising_class, denoising_bbox_unact_vis,denoising_bbox_unact_ir, attn_mask, dn_meta = None, None, None, None, None
        ...
        if self.proj_all:
            ...
            target, init_ref_points_unact_vis, init_ref_points_unact_ir, enc_topk_bboxes_vis, enc_topk_bboxes_ir, enc_topk_logits, topk_ind_mask, topk_score, mask_vis = \
                self._get_decoder_input(gt_meta,
                                        visir_memory, visir_spatial_shapes, visir_level_start_index, denoising_class,
                                        denoising_bbox_unact_vis, denoising_bbox_unact_ir)

        # decoder
        out_bboxes_vis, out_bboxes_ir, out_logits = self.decoder(
            gt_meta,
            target,
            ...
            mask_vis = mask_vis,
            dn_meta = dn_meta)
        return (out_bboxes_vis, out_bboxes_ir, out_logits, enc_topk_bboxes_vis, enc_topk_bboxes_ir, enc_topk_logits,
                dn_meta, mask_vis)
    ...
    def _get_decoder_input(self,
        ...
        topk_score = None
        # prepare input for decoder
        if self.training or self.eval_size is None or gt_meta['flag'] == 5 or gt_meta['flag'] == 9 or gt_meta['flag'] == 2:
            anchors, valid_mask = self._generate_anchors(spatial_shapes)
        else:
            anchors, valid_mask = self.anchors, self.valid_mask
        memory = paddle.where(valid_mask, memory, paddle.to_tensor(0.))
        ...
        return target, reference_points_unact_vis, reference_points_unact_ir, enc_topk_bboxes_vis, enc_topk_bboxes_ir, enc_topk_logits, topk_ind_mask, topk_score, mask_vis

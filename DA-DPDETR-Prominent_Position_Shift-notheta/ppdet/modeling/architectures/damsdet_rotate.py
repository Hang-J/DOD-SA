...
class DAMSDET_Rotate_Paired_DAOD(BaseArch):
    ...
    def _forward(self,):
        ...
        if (self.training and (self.inputs['flag'] == 3 or self.inputs['flag'] == 7)) or self.inputs['flag'] == 9:
            ...
            self.new_gt_bbox_vis=self.inputs['gt_bbox_vis']
            self.new_gt_bbox_ir=self.inputs['gt_bbox_ir']
            self.new_gt_class=self.inputs['gt_class']
            self.new_is_crowd=self.inputs['is_crowd']
        ...
        # stage 1
        if self.inputs['flag'] == 1 or self.inputs['flag'] == 2:  # do sup_cat
            # backbone

            body_feats = self.backbone_stream2(self.inputs)

            # Neck
            if self.neck_stream2 is not None:
                body_feats = self.neck_stream2(body_feats,self.inputs['flag'])


            if self.inputs['flag'] == 1:
                self.inputs['gt_bbox_dn'] = self.inputs['teacher_gt_bbox_vis']
            ...
            # Transformer
            pad_mask = self.inputs.get('pad_mask', None)
            out_transformer = self.transformer_single(body_feats, pad_mask, self.inputs, is_teacher=True if self.inputs['flag'] == 5 or self.inputs['flag'] == 9  or self.inputs['flag'] == 2 else False)

            # DETR Head
            if self.training and self.inputs['flag'] != 1:
                ...
                return detr_losses
            elif self.training and self.inputs['flag'] == 1:
        ...
        if self.inputs['flag'] == 3 or self.inputs['flag'] == 4 or self.inputs['flag'] == 5 or self.inputs['flag'] == 'eval_stage1' or self.inputs['flag'] == 'eval_stage1_2':  # do sup_cat
            ...
            if self.neck_stream1 is not None:
                body_feats = self.neck_stream1(body_feats,self.inputs['flag'])

            if self.inputs['flag'] == 3:
                self.inputs['gt_bbox_dn'] = self.new_gt_bbox_ir + self.new_gt_bbox_ir
        ...
        if self.inputs['flag'] == 7:  # do sup
            ...
            self.inputs['gt_bbox_dn'] = self.new_gt_bbox_ir
            self.inputs['gt_class_dn'] = self.new_gt_class
            self.inputs['im_shape']= paddle.tile(self.inputs['im_shape'][0], repeat_times=[b, 1])

            # Transformer
            pad_mask = self.inputs.get('pad_mask', None)
            out_transformer = self.transformer_single(body_feats, pad_mask, self.inputs, is_teacher=True if self.inputs['flag'] == 5 or self.inputs['flag'] == 9 else False)
        ...
        if self.inputs['flag'] == 8 or self.inputs['flag'] == 10 or self.inputs['flag'] == 'eval_stage2':  # do multi branch
            ...
            # Neck
            if self.neck_stream1 is not None:
                ir_body_feats = self.neck_stream1(ir_body_feats,self.inputs['flag'])
                vis_body_feats = self.neck_stream2(vis_body_feats,self.inputs['flag'])

            # if self.inputs['flag'] == 8:
            #     self.inputs['gt_bbox_ir_dn'] = self.inputs['paired_gt_bbox_ir']
            #     self.inputs['gt_bbox_vis_dn'] = self.inputs['paired_teacher_bbox_vis']
            ...
            if self.inputs['flag'] == 8 or self.inputs['flag'] == 10:    
                ...
                for t1, t2 in zip(self.inputs['paired_gt_bbox_ir_match'], self.inputs['paired_gt_bbox_ir_unmatch']):
                    # Check if tensors are not empty before concatenating
                ...
                self.inputs['gt_bbox_ir_dn'] = paired_gt_bbox_ir#self.inputs['paired_gt_bbox_ir_match']
                self.inputs['gt_bbox_vis_dn'] = paired_teacher_bbox_vis#self.inputs['paired_teacher_bbox_match']
                self.inputs['gt_class_dn'] = self.inputs['paired_gt_class']
                self.inputs['im_shape']= paddle.tile(self.inputs['im_shape'][0], repeat_times=[b, 1])                    
            # Transformer
            pad_mask = self.inputs.get('pad_mask', None)
            ...
            if self.training:
                if self.inputs['flag'] == 10 or self.inputs['flag'] == 8:
                    ...
                    #                             self.inputs)
                    # for k, v in detr_losses.items():
                    #     if 'log' not in k:  # 排除包含 'log' 的项
                    #         if 'ir' in k:  # 如果键中包含 'ir'
                    #             total_loss += 0.5 * v  # 赋予 0.6 的权重
                    #         elif 'vis' in k:  # 如果键中包含 'vis'
                    #             total_loss += 0.5 * v  # 赋予 0.4 的权重
                    #         else:  # 其他情况（不包含 'ir' 或 'vis'）
                    #             total_loss += v  # 默认权重为 1.0
                    # detr_losses.update({'loss': total_loss})
                    detr_losses = self.detr_head_DPDETR(out_transformer, None,
                                                self.inputs)
                    detr_losses.update({
                        'loss': paddle.add_n(

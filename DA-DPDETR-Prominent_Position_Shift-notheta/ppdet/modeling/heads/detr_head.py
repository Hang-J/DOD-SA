...
__all__ = ['DETRHead', 'DeformableDETRHead', 'DINOHead', 'DINOHead_DAOD', 'MaskDINOHead','DINOHead_Decoupled_Position_DAOD','DINOHead_Decoupled_Position']
...
@register
class DINOHead_DAOD(nn.Layer):
    __inject__ = ['loss']

    def __init__(self, loss='DINOLoss',group=False,groupx4=False,groupx5=False,groupx3_v3=False,groupx3_v4=False,groupx3_v5=False,
                 groupx3_v6=False):
        super(DINOHead_DAOD, self).__init__()
        self.loss = loss
        self.group = group
        self.groupx4 = groupx4
        self.groupx5 = groupx5
    ...
    def forward(self, out_transformer, body_feats, inputs=None,flag=None):
        if flag == 'DAOD':
            (out_bboxes,out_logits,gt_bbox,gt_class,im_shape,dn_out_bboxes,
             dn_out_logits,dn_meta) = out_transformer
        ...
        if self.training:
            assert inputs is not None
            assert 'gt_bbox_ir' in inputs and 'gt_class' in inputs
...
@register
class DINOHead_Decoupled_Position(nn.Layer):
    __inject__ = ['loss']

    def __init__(self, loss='DINOLoss_Decoupled_Position'):
        super(DINOHead_Decoupled_Position, self).__init__()
        self.loss = loss

    def forward(self, out_transformer, body_feats, inputs=None, infer = False):
...
@register
class DINOHead_Decoupled_Position_DAOD(nn.Layer):
    __inject__ = ['loss']

    def __init__(self, loss='DINOLoss_Decoupled_Position'):
        super(DINOHead_Decoupled_Position_DAOD, self).__init__()
        self.loss = loss

    def forward(self, out_transformer, body_feats, inputs=None, infer = False, flag=None):
        ...
        if self.training:
            ...
            if inputs['flag'] == 10 or inputs['flag'] == 8:
                ...
                paired_gt_bbox_ir=[]
                paired_teacher_bbox_vis=[]
                for t1, t2 in zip(inputs['paired_gt_bbox_ir_match'], inputs['paired_gt_bbox_ir_unmatch']):
                    #combined_tensor = paddle.concat((t1, t2), axis=0)
                    if t1 is not None and t2 is not None and t1.numel() > 0 and t2.numel() > 0:
                        combined_tensor = paddle.concat((t1, t2), axis=0)
                    elif t1 is not None and t1.numel() > 0:
                        combined_tensor = t1
                    elif t2 is not None and t2.numel() > 0:
                        combined_tensor = t2
                    else:
                        # Create an empty tensor with appropriate shape if both are empty
                        combined_tensor = paddle.empty([0, 4], dtype=paddle.float32)
                    paired_gt_bbox_ir.append(combined_tensor)
                for t1, t2 in zip(inputs['paired_teacher_bbox_match'], inputs['paired_teacher_bbox_unmatch']):
                    ...
                    if t1 is not None and t2 is not None and t1.numel() > 0 and t2.numel() > 0:
                        ...
                        combined_tensor = paddle.empty([0, 4], dtype=paddle.float32)
                    paired_teacher_bbox_vis.append(combined_tensor)
                return self.loss(
                    out_bboxes_vis,
                    out_bboxes_ir,
                    out_logits,
                    ...
                    dn_out_logits=dn_out_logits,
                    dn_meta=dn_meta,
                    paired_gt_bbox_match_shape=[tensors.shape[0] for tensors in inputs['paired_gt_bbox_ir_match']],
                    paired_gt_bbox_unmatch_shape=[tensors.shape[0] for tensors in inputs['paired_gt_bbox_ir_unmatch']])  
        else:
            if inputs['flag'] == 9 or inputs['flag'] == 'infer_paired':
                return (dec_out_bboxes_vis[-1], dec_out_bboxes_ir[-1], dec_out_logits[-1], None)
            else:
                return (dec_out_bboxes_vis[-1], dec_out_logits[-1], None)       

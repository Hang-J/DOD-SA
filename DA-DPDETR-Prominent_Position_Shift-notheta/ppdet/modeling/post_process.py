...
__all__ = [
    ...
    'CenterNetPostProcess', 'DETRPostProcess','DETRPostProcess_DAOD','SparsePostProcess',
    'DETRPostProcess_Decoupled_Position','DETRPostProcess_Decoupled_Position_DAOD'
]
...
@register
class DETRPostProcess_DAOD(object):
    __shared__ = ['num_classes', 'use_focal_loss', 'with_mask']
    __inject__ = []
    ...
    def __init__(self,
                 ...
                 use_avg_mask_score=False,
                 bbox_decode_type='origin'):
        super(DETRPostProcess_DAOD, self).__init__()
        assert bbox_decode_type in ['origin', 'pad']
    ...
    def __call__(self, head_out, im_shape, scale_factor, pad_shape,flag=0):
        """
        Decode the bbox and mask.
        ...
        if flag == 0:
            ...
            bbox_pred=bboxes
            img_h, img_w = paddle.split(im_shape, 2, axis=-1)
        if self.bbox_decode_type == 'pad':
            # calculate the shape of the image with padding
            out_shape = pad_shape / im_shape * origin_shape
            out_shape = out_shape.flip(1).tile([1, 2]).unsqueeze(1)
            ...
            if flag == 0:
                ...
                out_shape = im_shape.flip(1).tile([1, 2]).unsqueeze(1)
        else:
            raise Exception(
                f'Wrong `bbox_decode_type`: {self.bbox_decode_type}.')
        bbox_pred *= out_shape
        ...
        # #paired infer
        # bboxes_vis, bboxes_ir, logits, masks = head_out
        #
        #
        # bbox_pred_vis = bbox_cxcywh_to_xyxy(bboxes_vis)
        # bbox_pred_ir = bbox_cxcywh_to_xyxy(bboxes_ir)
        # # calculate the original shape of the image
        # origin_shape = paddle.floor(im_shape / scale_factor + 0.5)
        # img_h, img_w = paddle.split(origin_shape, 2, axis=-1)
        # if self.bbox_decode_type == 'pad':
        ...
        #     raise Exception(
        #         f'Wrong `bbox_decode_type`: {self.bbox_decode_type}.')
        # bbox_pred_vis *= out_shape
        # bbox_pred_ir *= out_shape
        #
        # scores = F.sigmoid(logits) if self.use_focal_loss else F.softmax(
        #     logits)[:, :, :-1]
        #
        ...
        #         index = paddle.stack([batch_ind, index], axis=-1)
        #         labels = paddle.gather_nd(labels, index)
        #         bbox_pred_vis = paddle.gather_nd(bbox_pred_vis, index)
        #         bbox_pred_ir = paddle.gather_nd(bbox_pred_ir, index)
        # else:
        #     scores, index = paddle.topk(
        #         scores.flatten(1), self.num_top_queries, axis=-1)
        #     labels = index % self.num_classes
        ...
        #     mask_pred, scores = self._mask_postprocess(masks, scores, index)
        #
        # bbox_pred_vis = paddle.concat(
        #     [
        #         labels.unsqueeze(-1).astype('float32'), scores.unsqueeze(-1),
        #         bbox_pred_vis
        #     ],
        #     axis=-1)
        # bbox_pred_ir = paddle.concat(
        #     [
        #         labels.unsqueeze(-1).astype('float32'), scores.unsqueeze(-1),
        #         bbox_pred_ir
        #     ],
        #     axis=-1)
        # bbox_num = paddle.to_tensor(
        #     self.num_top_queries, dtype='int32').tile([bbox_pred_vis.shape[0]])
        # bbox_pred_vis = bbox_pred_vis.reshape([-1, 6])
        # bbox_pred_ir = bbox_pred_ir.reshape([-1, 6])
        # return bbox_pred_vis, bbox_pred_ir, bbox_num, mask_pred
        #

@register
class DETRPostProcess_Rotate(object):
    __shared__ = ['num_classes', 'use_focal_loss', 'with_mask']
    __inject__ = []
    ...
    def __init__(self,
                 ...
                 use_avg_mask_score=False,
                 bbox_decode_type='origin'):
        super(DETRPostProcess_Rotate, self).__init__()
        assert bbox_decode_type in ['origin', 'pad']
    ...
    def _mask_postprocess(self, mask_pred, score_pred, index):
        ...
        return mask_pred[0].astype('int32'), score_pred
    def _box2corners_xminyminxmaxymax(self, pred_bboxes):
        """Convert (x, y, w, h, angle) to (xmin, ymin, xmax, ymax)
        ignoring angle.
        ...
        Returns:
            boxes (Tensor): [B, N, 4]
        """
        x, y, w, h, _ = paddle.split(pred_bboxes, 5, axis=-1)
        half_w = w * 0.5
        half_h = h * 0.5
        xmin = x - half_w
        ymin = y - half_h
        xmax = x + half_w
        ymax = y + half_h
        return paddle.concat([xmin, ymin, xmax, ymax], axis=-1)
    ...
    def __call__(self, head_out, im_shape, scale_factor, pad_shape, flag=0):
        ...
        bbox_pred =paddle.concat( [bboxes[:,:,:4] * out_shape,bboxes[:,:,4].unsqueeze(2)], axis=-1 )

        if flag == 0:
            bbox_pred = self._box2corners_xminyminxmaxymax(bbox_pred)
        ...
        # if self.dual_queries:
        #     num_queries = logits.shape[1]
        #     logits, bboxes_vis = logits[:, :int(num_queries // (self.dual_groups + 1)), :], \
        #                      bboxes_vis[:, :int(num_queries // (self.dual_groups + 1)), :], \
        #                      bboxes_ir[:, :int(num_queries // (self.dual_groups + 1)), :]
        #
        # #bbox_pred = bbox_cxcywh_to_xyxy(bboxes)
        # # calculate the original shape of the image
        # origin_shape = paddle.floor(im_shape / scale_factor + 0.5)
        ...
        #         f'Wrong `bbox_decode_type`: {self.bbox_decode_type}.')
        #
        # bbox_pred_vis =paddle.concat( [bboxes_vis[:,:,:4] * out_shape,bboxes_vis[:,:,4].unsqueeze(2)], axis=-1 )
        # bbox_pred_vis = self._box2corners(bbox_pred_vis)
        # bbox_pred_ir = paddle.concat([bboxes_ir[:, :, :4] * out_shape, bboxes_ir[:, :, 4].unsqueeze(2)], axis=-1)
        # bbox_pred_ir = self._box2corners(bbox_pred_ir)
        #
        # scores = F.sigmoid(logits) if self.use_focal_loss else F.softmax(
        #     logits)[:, :, :-1]
        #
        ...
        # bbox_num = paddle.to_tensor(
        #     self.num_top_queries, dtype='int32').tile([bbox_pred_vis.shape[0]])
        # bbox_pred_vis = bbox_pred_vis.reshape([-1, 10])
        # bbox_pred_ir = bbox_pred_ir.reshape([-1, 10])
        # return bbox_pred_vis,bbox_pred_ir,  bbox_num, mask_pred

@register
class DETRPostProcess_Decoupled_Position(object):
    __shared__ = ['num_classes', 'use_focal_loss', 'with_mask']
    __inject__ = []
    ...
    def __init__(self,
                 ...
                 use_avg_mask_score=False,
                 bbox_decode_type='origin'):
        super(DETRPostProcess_Decoupled_Position, self).__init__()
        assert bbox_decode_type in ['origin', 'pad']
    ...
    def __call__(self, head_out, im_shape, scale_factor, pad_shape, infer=False):
        """
        Decode the bbox and mask.
...
@register
class DETRPostProcess_Decoupled_Position_DAOD(object):
    __shared__ = ['num_classes', 'use_focal_loss', 'with_mask']
    __inject__ = []
    ...
    def __init__(self,
                 ...
                 use_avg_mask_score=False,
                 bbox_decode_type='origin'):
        super(DETRPostProcess_Decoupled_Position_DAOD, self).__init__()
        assert bbox_decode_type in ['origin', 'pad']
    ...
    def __call__(self, head_out, im_shape, scale_factor, pad_shape,flag=0):
        ...
        if flag == 0:
            ...
            bbox_pred_vis=bboxes_vis
            bbox_pred_ir=bboxes_ir
            img_h, img_w = paddle.split(im_shape, 2, axis=-1)
        # calculate the original shape of the image
        # origin_shape = paddle.floor(im_shape / scale_factor + 0.5)
        # img_h, img_w = paddle.split(origin_shape, 2, axis=-1)

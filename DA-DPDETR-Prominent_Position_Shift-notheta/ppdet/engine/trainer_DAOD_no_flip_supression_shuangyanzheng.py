...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def _eval_with_loader(self, loader, eval_flag=0, my_eval_flag = 'eval_stage3'):
        ...
        eval_model.eval()
        paired_eval = my_eval_flag == 'infer_paired'
        paired_metrics = copy.deepcopy(self._metrics) if paired_eval else None
        if self.cfg.get('print_flops', False):
            flops_loader = create('{}Reader'.format(self.mode.capitalize()))(
        ...
        for step_id, data in enumerate(loader):
            ...
            if paired_eval:
                ...
                if 'mask' in outs:
                    outs_ir['mask'] = outs['mask']
                for metric in self._metrics:
                    metric.update(data, outs_ir)
                ...
                if 'mask' in outs:
                    outs_vis['mask'] = outs['mask']
                for metric in paired_metrics:
                    metric.update(data, outs_vis)
            else:
                data['gt_bbox']=data['gt_bbox_ir']
        ...
        if paired_eval:
            logger.info("***** infrared metrics *****")
            for metric in self._metrics:
                metric.accumulate()
                metric.log()
            self._compose_callback.on_epoch_end(self.status)

            logger.info("***** visible metrics *****")
            for metric in paired_metrics:
                metric.accumulate()
                metric.log()
            self._compose_callback.on_epoch_end(self.status)

...
__all__ = ['Trainer_DAOD']

MOT_ARCH = ['JDE', 'FairMOT', 'DeepSORT', 'ByteTrack', 'CenterTrack']


class Trainer_DAOD(object):
    ...
    def load_weights(self, weights, ARSL_eval=False):
        if self.is_loaded_weights:
            return
        self.start_epoch = 0
        load_pretrain_weight(self.model, weights, ARSL_eval, mode = 'default')
                    ...
                    if isinstance(
                        ...
                        if curr_iter > st_iter:
                            ...
                            with paddle.no_grad():
                                ...
                                #flip persudo labels
                                data['flag'] = 6 #6 means do flip teacher preds unsup_w_flip
                                teacher_preds_flip = self.ema.model(data)
                                # fliter high socre preds as persudo labels
                                teacher_preds_flip['bbox'] = paddle.split(teacher_preds_flip['bbox'], bs, 0)
                                teacher_preds_flip['class'] = []
                                for xx in range(len(teacher_preds_flip['bbox'])):
                                    teacher_preds_flip['bbox'][xx] = teacher_preds_flip['bbox'][xx][
                                        teacher_preds_flip['bbox'][xx][:, 1] > self.psudo_labels_threhold]
                                    teacher_preds_flip['bbox_num'][xx] = len(teacher_preds_flip['bbox'][xx])

                                    teacher_preds_flip['class'].append(
                                        teacher_preds_flip['bbox'][xx][:, 0].unsqueeze(1).astype('int32'))

                                    teacher_preds_flip['bbox'][xx] = teacher_preds_flip['bbox'][xx][:, 2:]

                                    # flip labels IOU-Guided Suppression
                                       # flip labels axis exchange
                                    if teacher_preds_flip['bbox'][xx].size != 0 and teacher_preds['bbox'][xx].size != 0:
                                        flipped_teacher_preds_flip =  teacher_preds_flip['bbox'][xx].clone()
                                        flipped_teacher_preds_flip[:,0] = W -1 - teacher_preds_flip['bbox'][xx][:,0]
                                        # flipped_teacher_preds_flip[1] = teacher_preds_flip['bbox'][xx][1]
                                        # flipped_teacher_preds_flip[2] = teacher_preds_flip['bbox'][xx][2]
                                        # flipped_teacher_preds_flip[3] = teacher_preds_flip['bbox'][xx][3]
                                        flipped_teacher_preds_flip[:,4] = -teacher_preds_flip['bbox'][xx][:,4]
                                        teacher_preds_flip['bbox'][xx] = flipped_teacher_preds_flip

                                        #do IOU-guided bbox Suppression
                                        piou_matrix_cost = self.iou_matrix(
                                            teacher_preds['bbox'][xx].unsqueeze(1),
                                            teacher_preds_flip['bbox'][xx].unsqueeze(0), cost=True)
                                        index_t, index_t_f = paddle.nonzero(piou_matrix_cost < 0.1, as_tuple=True)
                                        if index_t.size != 0:
                                            filterd_teacher_preds['bbox'].append(teacher_preds['bbox'][xx][index_t.squeeze()].reshape((-1,5)))
                                            filterd_teacher_preds['class'].append(teacher_preds['class'][xx][index_t.squeeze()].reshape((-1,1)))
                                        else:
                                            filterd_teacher_preds['bbox'].append(paddle.empty([0,5]))
                                            filterd_teacher_preds['class'].append(paddle.empty([0,1]))
                                    else:
                                        filterd_teacher_preds['bbox'].append(teacher_preds['bbox'][xx])
                                        filterd_teacher_preds['class'].append(teacher_preds['class'][xx])
                            ...
                            loss_dict_unsup = self.model.get_ssod_loss((student_preds[0], student_preds[1], student_preds[2],
                                                                        filterd_teacher_preds['bbox'], filterd_teacher_preds['class'], data['im_shape'],
                                                                        None,None,None,None))
            ...
            if validate and is_snapshot:
                ...
                with paddle.no_grad():
                    ...
                    if epoch_id > self.semi_start_epochs:
                        ...
                        eval_flag = 0
                    self._eval_with_loader(self._eval_loader, eval_flag)
    ...
    def _eval_with_loader(self, loader, eval_flag=0):
        sample_num = 0
        tic = time.time()
        self._compose_callback.on_epoch_begin(self.status)

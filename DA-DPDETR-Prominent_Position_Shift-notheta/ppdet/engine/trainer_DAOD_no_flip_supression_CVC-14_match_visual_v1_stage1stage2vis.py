...
from ppdet.optimizer import ModelEMA_v1, SimpleModelEMA
                        ...
                        if curr_iter < stage2_iter:
                            ...
                            if curr_iter >= st_iter:
                                ...
                                if self.epoch>=self.semi_start_epochs+4:
                                    ...
                                    loss_dict_unsup_vis = self.model.get_ssod_loss_single((student_preds[0], student_preds[1],
                                                                        paired_teacher_bbox_match, class_ir_match, data['im_shape'],
                                                                        student_preds[2],student_preds[3],student_preds[4]))                                

                                    losses_unsup_vis = loss_dict_unsup_vis['loss'] * 1 #
                                    losses_unsup_vis.backward()

                                    # loss_dict.update(loss_dict_unsup)
                                    loss_dict.update({'loss_unsup_sum': losses_unsup_vis})
                                    losses += losses_unsup_vis.detach()
                                    # loss_dict.update({"fg_sum": fg_num})
                                    loss_dict['loss'] = losses
###############################################################################################################STAGE 2
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
                        if curr_iter >= stage2_iter and curr_iter < stage3_iter: #STAGE 2
                            ...
                            with paddle.no_grad():
                                # data_unsup_w['is_teacher'] = True
                                data['flag'] = 2  # 5 means do teacher preds  unsup_w
                                teacher_preds = self.ema.model(data)
                                bbox_y = teacher_preds['bbox'][:, 1]
                                valid_y = bbox_y[bbox_y > 0.3]
                                if valid_y.shape[0] > 0:
                            ...
                            data['teacher_gt_class']=class_ir_match
                            data['teacher_gt_bbox_vis']=paired_teacher_bbox_match           
                            data['flag'] = 1  # 4 means do student preds unsup_s
                            student_preds = self.model(data)
                            train_cfg['curr_iter'] = curr_iter
                            train_cfg['st_iter'] = st_iter
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
            ...
            if validate and is_snapshot:
                if not hasattr(self, '_eval_loader'):
                    ...
                    self._eval_dataset = self.cfg.EvalDataset2
                    self._eval_batch_sampler = \
                    ...
                    if self.cfg.metric == "Pose3DEval":
                        self._eval_loader = create('EvalReader2')(
                ...
                with paddle.no_grad():
                    ...
                    if curr_iter <= stage2_iter:
                        ...
                        self._eval_with_loader(self._eval_loader, eval_flag, my_eval_flag='infer_paired')
    ...
    def paired_label_visual(self, im_id, ir_image, vis_image, bboxes_ir, bboxes_vis, H, W):
        ...
        for i,id in enumerate(im_id.numpy()):
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_alllabeled/vis/images/{filename}'):
                id_str_ir = filename.split('.')[0]+'_vis.jpg'
                id_str_vis = filename.split('.')[0]+'_ir.jpg'
...
##############################################################################################################
    def _normalize_batch_bboxes(self, bboxes_list, H, W):
        """模块1: 批量归一化处理"""
    ...
    def _save_debug_visualization(self, ir_boxes, affine_mm, ori_hw, curr_hw, im_id_mm, index):
        ...
        try:
            ...
            vis_img_path = os.path.join(dataset_root, 'train_alllabeled', 'ir', 'images', filename)
            ...
            # [绘制红外图像]：画上 IR GT 框(红)
            ir_img_path = os.path.join(dataset_root,  'train_alllabeled', 'vis', 'images',filename)
    ...
    def points_in_rotated_box(self,points, box):
        ...
        return mask
    # ---------------------------------------------------------------------- #
    # Matching with debug info
    # ---------------------------------------------------------------------- #
    def _solve_global_matching_hbox(self,
                                    ir_boxes,
                                    vis_boxes,
                                    vis_scores,
                                    return_debug=False):
        """
        Same logic as the base implementation, but optionally returns detailed
        cost components for visualization.
        """
        num_ir = len(ir_boxes)
        ...
        if num_ir == 0 or num_vis == 0:
            return (matched_dict, debug) if return_debug else matched_dict
        ...
        dist_sq = (cx_ir - cx_vis) ** 2 + (cy_ir - cy_vis) ** 2
        diag_len = np.sqrt(w_ir ** 2 + h_ir ** 2) + eps
        dist_cost = np.sqrt(dist_sq) / diag_len
        ...
        total_cost = (
            lambda_shape * shape_cost
            + lambda_dist * dist_cost
            + lambda_cls * score_cost
        )
        total_cost_raw = total_cost.copy()
        total_cost_masked = total_cost.copy()

        for i in range(num_ir):

            is_in_box = self.points_in_rotated_box(vis_centers, ir_boxes[i])
            total_cost_masked[i, ~is_in_box] = 10000.0

        row_inds, col_inds = linear_sum_assignment(total_cost_masked)
        selected_pairs = []
        for r, c in zip(row_inds, col_inds):
            if total_cost_masked[r, c] < 5000.0 and shape_cost[r, c] < 0.35:
                matched_dict[int(r)] = int(c)
                selected_pairs.append((int(r), int(c)))

        if return_debug:
            debug.update({
                'total_raw': total_cost_raw,
                'total_masked': total_cost_masked,
                'shape_cost': shape_cost,
                'dist_cost': dist_cost,
                'score_cost': score_cost,
                'selected_pairs': selected_pairs,
            })
            return matched_dict, debug
        return matched_dict
    ...
    def _text_size(self, text, font):
        """兼容性文字尺寸获取，避免 draw.textsize 在部分版本不可用。"""
    ...
    def _to_absolute_boxes(self, boxes, affine_mm, ori_hw, curr_hw):
        if boxes is None:
            return np.zeros((0, 4), dtype=np.float32)
        boxes_copy = copy.deepcopy(boxes)
        tensor_input = paddle.to_tensor(boxes_copy, dtype='float32')
        if affine_mm is not None:
            abs_boxes = self._bbox_curr_to_orig(
                tensor_input, affine_mm, ori_hw, curr_hw, plot=True).numpy()
        else:
            curr_h, curr_w = float(curr_hw[0]), float(curr_hw[1])
            abs_boxes = tensor_input.numpy()
            abs_boxes[:, 0] *= curr_w
            abs_boxes[:, 2] *= curr_w
            abs_boxes[:, 1] *= curr_h
            abs_boxes[:, 3] *= curr_h
    ...
    def _draw_pair_visualization(self, ir_boxes, vis_boxes, matched_pairs,
                                 im_id_mm, affine_mm, ori_hw, curr_hw):
        try:
            filename = self.coco_ir.loadImgs([int(im_id_mm)])[0]['file_name']
            dataset_root = self.cfg.TrainDataset.dataset_dir
            ir_path = os.path.join(dataset_root, 'train_alllabeled', 'vis', 'images',
                                   filename)
            vis_path = os.path.join(dataset_root, 'train_alllabeled', 'ir', 'images',
                                    filename)
            ...
            ir_abs = self._to_absolute_boxes(ir_boxes, affine_mm, ori_hw,
                                             curr_hw)
            vis_abs = self._to_absolute_boxes(vis_boxes, affine_mm, ori_hw,
                                              curr_hw)

            # 为匹配对分配统一的编号，方便观察
            pair_id_map = {}
            for k, (ir_idx, vis_idx) in enumerate(sorted(matched_pairs.items())):
                pair_id_map[ir_idx] = k + 1
                pair_id_map[f'ir_{vis_idx}'] = k + 1

            ir_xyxy = self._cxcywh_to_xyxy(ir_abs)
            vis_xyxy = self._cxcywh_to_xyxy(vis_abs)
            font = ImageFont.load_default()

            ir_draw = ImageDraw.Draw(ir_img)
            for idx, box in enumerate(ir_xyxy):
                if not np.isfinite(box).all():
                    continue
                x1, y1, x2, y2 = box.tolist()
                ir_draw.rectangle([x1, y1, x2, y2],
                                  outline=(255, 0, 0),
                                  width=2)
                label_txt = str(pair_id_map.get(idx, idx))
                ir_draw.text((x1, max(0, y1 - 12)),
                             label_txt,
                             fill=(255, 0, 0),
                             font=font)

            vis_draw = ImageDraw.Draw(vis_img)
            for idx, box in enumerate(vis_xyxy):
                if not np.isfinite(box).all():
                    continue
                x1, y1, x2, y2 = box.tolist()
                vis_draw.rectangle([x1, y1, x2, y2],
                                   outline=(0, 128, 255),
                                   width=2)
                label_txt = str(pair_id_map.get(f'ir_{idx}', idx))
                vis_draw.text((x1, max(0, y1 - 12)),
                              label_txt,
                              fill=(0, 128, 255),
                              font=font)

            gap = 20
            width = max(ir_img.width, vis_img.width)
            height = ir_img.height + vis_img.height + gap
            combined = Image.new('RGB', (width, height), color=(255, 255, 255))
            combined.paste(ir_img, (0, 0))
            combined.paste(vis_img, (0, ir_img.height + gap))

            draw = ImageDraw.Draw(combined)
            for ir_idx, vis_idx in matched_pairs.items():
                if ir_idx >= len(ir_abs) or vis_idx >= len(vis_abs):
                    continue
                ir_cx, ir_cy = ir_abs[ir_idx, 0], ir_abs[ir_idx, 1]
                vis_cx, vis_cy = vis_abs[vis_idx, 0], vis_abs[vis_idx, 1]
                pair_txt = str(pair_id_map.get(ir_idx, ''))
                draw.line(
                    [
                        (ir_cx, ir_cy),
                        (vis_cx, vis_cy + ir_img.height + gap)
                    ],
                    fill=(255, 215, 0),
                    width=2)
                if pair_txt:
                    draw.text((ir_cx, ir_cy - 10),
                              pair_txt,
                              fill=(255, 0, 0),
                              font=font)
                    draw.text((vis_cx, vis_cy + ir_img.height + gap - 10),
                              pair_txt,
                              fill=(0, 128, 255),
                              font=font)
            return combined
        except Exception as exc:  # noqa: BLE001
            logger.warning(f'[match_visual] pair visualization failed: {exc}')
            return None
    ...
    def _draw_single_cost_matrix(self, matrix, shape_cost, dist_cost,
                                 score_cost, selected_pairs, title, font):
        num_ir, num_vis = matrix.shape
        cell_w, cell_h = 130, 90
        header_w, header_h = 70, 40
        title_space = 28
        width = header_w + num_vis * cell_w
        height = title_space + header_h + num_ir * cell_h
        img = Image.new('RGB', (width, height), color=(255, 255, 255))
        draw = ImageDraw.Draw(img)

        # 标题
        title_w, title_h = self._text_size(title, font)
        draw.text(((width - title_w) / 2, (title_space - title_h) / 2),
        ...
        for j in range(num_vis):
            ...
            draw.rectangle([x0, title_space, x0 + cell_w, title_space + header_h],
                           outline=(0, 0, 0), width=2, fill=(245, 245, 245))
            tw, th = self._text_size(col_label, font)
            draw.text((x0 + (cell_w - tw) / 2, title_space + (header_h - th) / 2),
                      col_label, fill=(0, 0, 0), font=font)

        # 左侧行标题
        for i in range(num_ir):
            row_label = str(i + 1)
            y0 = title_space + header_h + i * cell_h
            draw.rectangle([0, y0, header_w, y0 + cell_h],
                           outline=(0, 0, 0), width=2, fill=(245, 245, 245))
            tw, th = self._text_size(row_label, font)
            draw.text(((header_w - tw) / 2, y0 + (cell_h - th) / 2),
                      row_label, fill=(0, 0, 0), font=font)
        ...
        for i in range(num_ir):
            for j in range(num_vis):
                ...
                draw.rectangle([x0, y0, x1, y1],
                               ...
                               fill=fill)
                lines = [
                    f'T: {self._format_val(matrix[i, j])}',
                    f'S: {self._format_val(shape_cost[i, j])}',
                    f'D: {self._format_val(dist_cost[i, j])}',
                    f'C: {self._format_val(score_cost[i, j])}',
                ]
                line_h = self._text_size('T: 0.000', font)[1]
                total_text_h = line_h * len(lines) + 4 * 2
                start_y = y0 + (cell_h - total_text_h) / 2
                for k, line in enumerate(lines):
                    tw, _ = self._text_size(line, font)
                    draw.text((x0 + (cell_w - tw) / 2, start_y + k * (line_h + 2)),
                              line, fill=(0, 0, 0), font=font)
        return img

    def _draw_cost_matrices(self, debug_info, save_path):
        try:
            total_raw = debug_info.get('total_raw')
            total_masked = debug_info.get('total_masked')
            shape_cost = debug_info.get('shape_cost')
            dist_cost = debug_info.get('dist_cost')
            score_cost = debug_info.get('score_cost')
            selected_pairs = debug_info.get('selected_pairs', [])

            if total_raw is None or total_masked is None:
                return
            font = ImageFont.load_default()
            before_img = self._draw_single_cost_matrix(
                total_raw, shape_cost, dist_cost, score_cost, selected_pairs,
                'Before Hungarian (raw cost)', font)
            after_img = self._draw_single_cost_matrix(
                total_masked, shape_cost, dist_cost, score_cost,
                selected_pairs, 'After mask + Hungarian', font)

            gap = 20
            width = before_img.width + after_img.width + gap
            ...
            logger.warning(f'[match_visual] cost matrix visualization failed: {exc}')

    # ---------------------------------------------------------------------- #
    # Paired label processing with visualization
    # ---------------------------------------------------------------------- #
    def paired_label_processing(self,
                                data,
                                teacher_bbox,
                                teacher_class,
                                teacher_score,
        ...
        affine_info, ori_shape_info, curr_shape_info = self._extract_transform_info(
            data)
        ...
        for mm in range(len(gt_bbox_ir)):
            ...
            ir_boxes_np = gt_bbox_ir[mm].numpy()
            ir_cls_np = gt_class[mm].numpy()
            vis_boxes_np = teacher_bbox[mm].numpy(
            ) if teacher_bbox[mm] is not None else np.zeros((0, 4),
                                                            dtype=np.float32)
            vis_scores_np = teacher_score[mm].numpy(
            ) if teacher_score[mm] is not None else np.zeros(
                (len(vis_boxes_np), ), dtype=np.float32)

            im_id_mm = float(im_id[mm])
            affine_mm = affine_info[mm] if use_affine else None
            ori_hw = ori_shape_info[mm] if use_affine else None
            curr_hw = curr_shape_info[mm] if use_affine else np.array(
                [H, W], dtype=np.float32)

            global_idx = np.where(
                self.vis_gt_data['im_id'].clone().numpy() == im_id_mm)[0][0]

            if use_affine:
                self.vis_gt_data['gt_bbox_vis'][global_idx] = self._bbox_orig_to_curr(
                    self.vis_gt_data['gt_bbox_vis'][global_idx], affine_mm,
                    ori_hw, curr_hw)

            matched_pairs, cost_debug = self._solve_global_matching_hbox(
                ir_boxes_np, vis_boxes_np, vis_scores_np, return_debug=True)
            ...
            for i in range(len(ir_boxes_np)):
                ...
                vis_box_candidate = vis_boxes_np[
                    matched_pairs[i]] if is_matched else None
                final_vis_box, final_vis_cls = self._update_label_state(
                    global_idx, mask_rank, ir_boxes_np[i], ir_cls_np[i],
            ...
            if use_affine:
                self._save_debug_visualization(ir_boxes_np, affine_mm, ori_hw,
                                               curr_hw, im_id_mm, global_idx)
            ...
            results['ir_match'].append(
                paddle.to_tensor(batch_res['ir'], dtype='float32')
                if batch_res['ir'] else paddle.empty([0, 4]))
            results['vis_match'].append(
                paddle.to_tensor(batch_res['vis'], dtype='float32')
                if batch_res['vis'] else paddle.empty([0, 4]))
            results['cls_match'].append(
                paddle.to_tensor(batch_res['cls'], dtype='int32')
                if batch_res['cls'] else paddle.empty([0, 1], dtype='int32'))
            results['ir_unmatch'].append(paddle.empty([0, 4]))
            results['vis_unmatch'].append(paddle.empty([0, 4]))
            results['cls_vis_match'].append(
                paddle.to_tensor(batch_res['cls'], dtype='int32')
                if batch_res['cls'] else paddle.empty([0, 1], dtype='int32'))
            ...
            try:
                ...
                viz_dir = os.path.join(REPO_ROOT, 'label_vision',
                                       'match_visual_pairs', 'vision4',str(self.epoch))
                os.makedirs(viz_dir, exist_ok=True)
                vis_img = self._draw_pair_visualization(
                    ir_boxes_np, vis_boxes_np, matched_pairs, im_id_mm,
                    affine_mm, ori_hw, curr_hw)
                if vis_img is not None:
                    vis_img.save(
                        os.path.join(
                            viz_dir,
                            f"{base_name}_match.png"),
                        format='PNG')

                cost_dir = os.path.join(REPO_ROOT, 'label_vision',
                                        'match_visual_cost','vision4', str(self.epoch))
                os.makedirs(cost_dir, exist_ok=True)
                # 直接复用匹配时计算的成本调试信息，避免额外开销
                cost_debug_full = cost_debug
                cost_path = os.path.join(cost_dir, f"{base_name}_cost.png")
               # self._draw_cost_matrices(cost_debug_full, cost_path)
            except Exception as exc:  # noqa: BLE001
                logger.warning(
                    f'[match_visual] save visualization failed: {exc}')

        return results['ir_match'], results['ir_unmatch'], results[
            'cls_match'], results['vis_match'], results['vis_unmatch']

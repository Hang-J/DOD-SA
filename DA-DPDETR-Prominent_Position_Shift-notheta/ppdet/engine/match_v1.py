...
##############################################################################################################
    ...
    def _solve_global_matching_hbox(self, ir_boxes, vis_boxes, vis_scores):
        """模块2: 核心匹配引擎 (Hungarian Algorithm)"""
        num_ir = len(ir_boxes)
        ...
        # --- 关键修复 1: 确保 vis_scores 是 1D 数组 [N,] ---
        # 如果是 [N, 1] 会导致后续广播生成 3D 矩阵，引发 IndexError
        vis_scores = vis_scores.flatten() 

        # 数据准备
        # ir: [M, 4], vis: [N, 4] -> (cx, cy, w, h)
        ...
        # Cost 计算
        # [A] Shape Cost: log长宽比
        # shape_cost = np.abs(np.log(w_vis / (w_ir + eps))) + \
        #              np.abs(np.log(h_vis / (h_ir + eps)))
        #         
        # [A] Shape Cost: Re-centered IoU (中心对齐 IoU)
        # 结果形状: [M, N]
        ...
        # [B] Distance Cost (Soft Constraint)
        # 结果形状: [M, N]
        ...
        # [C] Score Cost
        # 结果形状: [M, N] (由于 vis_scores 已 flatten，这里广播正常)
        ...
        for r, c in zip(row_inds, col_inds):
            if total_cost[r, c] < 5000.0 and shape_cost[r, c] < 0.35: # Shape IoU > 0.7
                matched_dict[r] = c

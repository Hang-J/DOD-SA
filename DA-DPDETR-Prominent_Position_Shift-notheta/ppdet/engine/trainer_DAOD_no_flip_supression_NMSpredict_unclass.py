...
#######################640*544 --> 840*712#####################
        ...
        for mm in range(len(gt_rbox_ir)):
            ...
            for i in range(len(coordinates_ir)):
                ...
                # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/rbox_Drone_Vehicle/train/label_trainimg/{(im_id_mm+1):05d}_epoch{self.epoch}_{i}.jpg', format='PNG')   
                if np.any(mask):
            ...
            # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/rbox_Drone_Vehicle/train/label_trainimg/{(im_id_mm+1):05d}_before.jpg', format='PNG')   
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_1000/trainimg/{(im_id_mm+1):05d}.jpg'):
                ...
                image.save(f'{folder}/{(im_id_mm+1):05d}_vis_shiftcorrect.jpg', format='PNG')
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_1000/trainimgr/{(im_id_mm+1):05d}.jpg'):
                ...
                image.save(f'{folder}/{(im_id_mm+1):05d}_ir_gt.jpg', format='PNG')
            ...
            # image=tensor_to_pil_image(data['vis_image'][mm])
            # image=draw_bbox_vis(image,bboxes)
            # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/rbox_Drone_Vehicle/train/label_trainimg/{(im_id_mm+1):05d}_latter.jpg', format='PNG')      
            if self.epoch<self.semi_start_epochs+4:
    ...
    def paired_spatial_search_nms(self, boxes_vis, boxes_ir, score_thr, iou_thr=0.5):
        ...
        for i in range(num_boxes):
            ...
            # --- A. 空间搜索 ---
            # 搜索范围扩大 1.5 倍 (保留你的逻辑)
            search_box = (cx_i, cy_i, 0.5 * w_i, 0.5 * h_i, angle_i)
        ...
        if visualize:
            for outs in results:
                ...
                for i, im_id in enumerate(outs['im_id']):
                    ...
                    vis_image.save(save_vis_name, format='PNG')
                    ir_image.save(save_ir_name, format='PNG')

...
class Checkpointer(Callback):
    ...
    def on_epoch_end(self, status):
        ...
        if dist.get_world_size() < 2 or dist.get_rank() == 0:
            ...
            if status.get('vis_gt_data') is not None:
                data_to_save = status['vis_gt_data']
                # 保存
                paddle.save(data_to_save, os.path.join(self.save_dir, f'vis_gt_data_{epoch_id}.pdparams'))
            if weight:

...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
                        if curr_iter >= stage2_iter and curr_iter < stage3_iter: #STAGE 2
                            ...
                            if epoch_id==self.stage2_start_epochs:###记得保存模型的时候也得把self.vis_gt_data保存下来                                  
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def paired_label_visual_shiftcorrectshow(self,im_id, ir_image, vis_image, bboxes_ir, bboxes_vis):
        ...
        folder = os.path.join(
            REPO_ROOT,
            'label_vision',
            'pseudo_labels_shiftcorrect_vision_shiftcorrectshow6',
            str(self.epoch))
        # 创建文件夹（如果不存在，且支持递归创建多级目录）
        os.makedirs(folder, exist_ok=True)
        ir_images=tensor_to_pil_images(ir_image)
    ...
    def paired_label_processing(self, data, teacher_bbox, teacher_class, teacher_score,iou_threshold=0.7):
        ...
        for mm in range(len(gt_bbox_ir)):
            mask = gt_bbox_keep_mask[mm]  # paddle.Tensor
            ...
            if use_affine:
                self.vis_gt_data['gt_bbox_vis'][index] = self._bbox_orig_to_curr(
                    self.vis_gt_data['gt_bbox_vis'][index], affine_mm, ori_hw,
                    curr_hw)
            for i in range(len(coordinates_ir)):
                ...
                if np.any(mask):
                    ...
                    if best_k!=-1:  # 匹配上了
                        ...
                        if self.epoch == self.stage2_start_epochs:
                            self.vis_gt_data['gt_bbox_vis'][index][mask_rank,:]=paddle.to_tensor(coordinates_vis_whcopy)
                            self.vis_gt_data['gt_class'][index][mask_rank,:]=paddle.to_tensor(categories_ir[i])
                        elif self.epoch>self.stage2_start_epochs:
                            ...
                            if paddle.isnan(self.vis_gt_data['gt_bbox_vis'][index][mask_rank]).any():
                                self.vis_gt_data['gt_bbox_vis'][index][mask_rank,:] = paddle.to_tensor(coordinates_ir[i])
                                self.vis_gt_data['gt_class'][index][mask_rank,:] = paddle.to_tensor(categories_ir[i])     
                            deta_x=coordinates_vis_whcopy[0]-self.vis_gt_data['gt_bbox_vis'][index][mask_rank][0]
                            deta_y=coordinates_vis_whcopy[1]-self.vis_gt_data['gt_bbox_vis'][index][mask_rank][1]
                            ...
                            self.vis_gt_data['gt_bbox_vis'][index][mask_rank,:]=paddle.to_tensor(np.array([self.vis_gt_data['gt_bbox_vis'][index][mask_rank][0]+lam*deta_x,self.vis_gt_data['gt_bbox_vis'][index][mask_rank][1]+lam*deta_y,coordinates_ir[i][2],coordinates_ir[i][3]], dtype='float32'))
                            self.vis_gt_data['gt_class'][index][mask_rank,:]=paddle.to_tensor(categories_ir[i])
                        coordinates_ir_match.append(coordinates_ir[i])
                        coordinates_vis_match.append(self.vis_gt_data['gt_bbox_vis'][index][mask_rank,:])
                        categories_ir_new.append(categories_ir[i])
                        categories_vis_match.append(self.vis_gt_data['gt_class'][index][mask_rank,:])
                        record_pop_idx_ir.append(i)  # 记录匹配上的idx
                        record_pop_idx_vis.append(best_k)
                    else:
                        if self.epoch==self.stage2_start_epochs: #从这一刻才开始做 Bag-update
                            self.vis_gt_data['gt_bbox_vis'][index][mask_rank,:]=paddle.to_tensor(coordinates_ir[i])
                            self.vis_gt_data['gt_class'][index][mask_rank,:]=paddle.to_tensor(categories_ir[i])
                            coordinates_ir_match.append(coordinates_ir[i])
                            coordinates_vis_match.append(self.vis_gt_data['gt_bbox_vis'][index][mask_rank])
            ...
            if use_affine:
                origin_vis_bboxes_mm = self._bbox_curr_to_orig(
                    copy.deepcopy(self.vis_gt_data['gt_bbox_vis'][index]),
                    affine_mm, ori_hw, curr_hw, plot=True).numpy()
                origin_ir_bboxes_mm = self._bbox_curr_to_orig(
                    paddle.to_tensor(copy.deepcopy(coordinates_ir), dtype='float32'),
                    affine_mm, ori_hw, curr_hw, plot=True).numpy()
            folder = os.path.join(
                REPO_ROOT,
                'label_vision',
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs_1000/vis/{filename}'):
                image_vis=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs_1000/vis/{filename}').convert('RGB')
                ...
                image.save(f"{folder}/{filename.split('.')[0]}_vis_shiftcorrect.jpg", format='PNG')
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs_1000/ir/{filename}'):
                image_ir=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs_1000/ir/{filename}').convert('RGB')
                bboxes=copy.deepcopy(origin_ir_bboxes_mm)
                image=draw_bbox_ir(image_ir,bboxes)
                image.save(f"{folder}/{filename.split('.')[0]}_ir_gt.jpg", format='PNG')
            if use_affine:
                self.vis_gt_data['gt_bbox_vis'][index] = self._bbox_curr_to_orig(
                    self.vis_gt_data['gt_bbox_vis'][index], affine_mm, ori_hw,
                    curr_hw)
            # bboxes=copy.deepcopy(self.vis_gt_data['gt_bbox_vis'][index]) 
            # image=tensor_to_pil_image(data['vis_image'][mm])

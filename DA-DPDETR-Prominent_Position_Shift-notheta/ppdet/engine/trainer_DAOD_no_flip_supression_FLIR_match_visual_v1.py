...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def paired_label_visual(self, im_id, ir_image, vis_image, bboxes_ir, bboxes_vis, H, W):
        ...
        for i,id in enumerate(im_id.numpy()):
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/FLIR-image/vi-train_1000/{filename}'):
...
##############################################################################################################
    ...
    def _save_debug_visualization(self, ir_boxes, affine_mm, ori_hw, curr_hw, im_id_mm, index):
        ...
        try:
            ...
            vis_img_path = os.path.join(dataset_root, 'FLIR-image', 'vi-train_1000', filename)
            if os.path.exists(vis_img_path):
                ...
                save_path_vis = os.path.join(folder, f"{file_prefix}_vi_shiftcorrect.jpg")
                image_vis.save(save_path_vis, format='PNG')

            # [绘制红外图像]：画上 IR GT 框(红)
            ir_img_path = os.path.join(dataset_root,'FLIR-image', 'ir-train_1000', filename)
    ...
    def _draw_pair_visualization(self, ir_boxes, vis_boxes, matched_pairs,
        ...
        try:
            ...
            dataset_root = self.cfg.TrainDataset.dataset_dir
            ir_path = os.path.join(dataset_root, 'FLIR-image', 'ir-train_1000', 
                                   filename)
            vis_path = os.path.join(dataset_root, 'FLIR-image', 'vi-train_1000',
                                    filename)
            ...
            for k, (ir_idx, vis_idx) in enumerate(sorted(matched_pairs.items())):
                ...
                pair_id_map[f'vis_{vis_idx}'] = k + 1
            ...
            for idx, box in enumerate(vis_xyxy):
                ...
                vis_draw.rectangle([x1, y1, x2, y2],
                                   outline=(0, 128, 255),
                                   width=2)
                label_txt = str(pair_id_map.get(f'vis_{idx}', idx))
        ...
        for mm in range(len(gt_bbox_ir)):
            ...
            try:
                ...
                viz_dir = os.path.join(REPO_ROOT, 'label_vision',
                                       'match_visual_pairs', 'vision3',str(self.epoch))
                ...
                cost_dir = os.path.join(REPO_ROOT, 'label_vision',
                                        'match_visual_cost','vision3', str(self.epoch))

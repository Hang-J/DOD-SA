    ...
    def train(self, validate=False):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id, data in enumerate(self.loader):
                H = data['im_shape'][0,0]
                W = data['im_shape'][0,1]                
                gt_bbox_ir=copy.deepcopy(data['gt_bbox_ir'])
                for i,_ in enumerate(gt_bbox_ir):
                    gt_bbox_ir[i][:,0] = gt_bbox_ir[i][:,0] * W
                    gt_bbox_ir[i][:,1] = gt_bbox_ir[i][:,1] * H
                    gt_bbox_ir[i][:,2] = gt_bbox_ir[i][:,2] * W
                    gt_bbox_ir[i][:,3] = gt_bbox_ir[i][:,3] * H
                self.step = step_id
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder###################
                        if curr_iter < stage2_iter:
                            ...
                            if curr_iter >= st_iter:
                                ...
                                if epoch_id==self.semi_start_epochs:###记得保存模型的时候也得把self.vis_gt_data保存下来                                  
                                    if 'im_id' not in self.vis_gt_data and 'gt_bbox_vis' not in self.vis_gt_data:
                                        self.vis_gt_data = {
                                            ...
                                            'gt_bbox_vis': [t.clone() for t in gt_bbox_ir],
                                        ...
                                        self.vis_gt_data['gt_bbox_vis']+= copy.deepcopy(gt_bbox_ir)
                                ...
                                self.paired_label_visual(data['im_id'],data['ir_image'],data['vis_image'],{'bbox': gt_bbox_ir,'class':data['gt_class']},teacher_preds)
                                ...
                                data['teacher_gt_bbox_ir']=paired_teacher_bbox_match
                                data['flag'] = 4
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
                        if curr_iter >= stage2_iter and curr_iter < stage3_iter: #STAGE 2
                            ...
                            # paired_gt_bbox_vis, paired_gt_class, paired_teacher_bbox_ir = self.paired_label_processing(data['gt_bbox_vis'],
                            #                                                                                  data['gt_class'],
                            #                                                                                  teacher_preds['bbox'],
                            #                                                                                  teacher_preds['class'])
                            # data['paired_gt_bbox_vis'] = paired_gt_bbox_vis
                            # data['paired_gt_class'] = paired_gt_class
                            # data['paired_teacher_bbox_ir'] = paired_teacher_bbox_ir

                            #self.paired_label_visual_shiftcorrectshow(data['im_id'],data['ir_image'],data['vis_image'],{'bbox': paired_gt_bbox_ir_match,'class':paired_gt_class},paired_teacher_bbox_match) 
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def paired_label_visual(self, im_id, ir_image, vis_image, bboxes_ir, bboxes_vis):
        ...
        def tensor_to_pil_images(tensor):
            ...
            return images
        folder = f'paired_label_visual/{self.epoch}'
        ...
        for i,id in enumerate(im_id.numpy()):
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs/vis/{filename}'):
    ...
    def paired_label_visual_shiftcorrectshow(self,im_id, ir_image, vis_image, bboxes_ir, bboxes_vis):
        def box2corners(pred_bboxes):
            ...
            h_y = cos_a_half * h
            return np.array([x + w_x + h_x, y + w_y + h_y, x - w_x + h_x, y - w_y + h_y,x - w_x - h_x, y - w_y - h_y, x + w_x - h_x, y + w_y - h_y])
        def draw_bbox_paired_ir(image, bboxes,i):
            """
            Draw bbox on image
            """
            bboxes=copy.deepcopy(bboxes)
            ...
            for j,_ in enumerate(bboxes['bbox'][i]):
                ...
                draw.line(
                    ...
                    fill=(255,0,0))
            return image
        def draw_bbox_paired_vis(image, bboxes_vis,bboxes_ir,i):
            """
            Draw bbox on image
            """
        ...
        def tensor_to_pil_images(tensor):
            ...
            return images
        folder = f'paired_label_visual_shiftcorrectshow/{self.epoch}'
        # 创建文件夹（如果不存在，且支持递归创建多级目录）
        os.makedirs(folder, exist_ok=True)
        ir_images=tensor_to_pil_images(ir_image)
        vis_images=tensor_to_pil_images(vis_image)
        ...
        for i,id in enumerate(im_id.numpy()):
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs/vis/{filename}'):
                ...
                vis_image=vis_images[i]
                ir_image=ir_images[i]
                vis_image=draw_bbox_paired_vis(vis_image,bboxes_vis,bboxes_ir,i)
    ...
    def paired_label_processing(self, data, teacher_bbox, teacher_class, teacher_score,iou_threshold=0.7):
        gt_bbox_ir_Normalized=copy.deepcopy(data['gt_bbox_ir'])
        gt_class=copy.deepcopy(data['gt_class'])   
        im_id=copy.deepcopy(data['im_id'])
        H=data['vis_image'].shape[2]
        W=data['vis_image'].shape[3]
        gt_bbox_ir=[]
        for i,_ in enumerate(gt_bbox_ir_Normalized):
            gt_bbox_ir.append(paddle.zeros_like(gt_bbox_ir_Normalized[i]))
            gt_bbox_ir[i][:,0] = gt_bbox_ir_Normalized[i][:,0] * W
            gt_bbox_ir[i][:,1] = gt_bbox_ir_Normalized[i][:,1] * H
            gt_bbox_ir[i][:,2] = gt_bbox_ir_Normalized[i][:,2] * W
            gt_bbox_ir[i][:,3] = gt_bbox_ir_Normalized[i][:,3] * H
        flipped=copy.deepcopy(data['flipped'])
        def transform_bbox_filp(bboxes, img_w, img_h,flipped_mm,ratio3=1.0):
...
#######################640*544 --> 840*712#####################
            ...
            w = w/ratio3
            h = h/ratio3
            return paddle.stack([x2, y1, w, h], axis=1)
        ...
        def compute_iou(rect1_np, rect2_np):
            ...
            intersection = cv2.rotatedRectangleIntersection(
                ((rect1_np[0], rect1_np[1]), (rect1_np[2], rect1_np[3]),0),
                ((rect2_np[0], rect2_np[1]), (rect2_np[2], rect2_np[3]),0)
            )
            # 如果相交区域存在
            if intersection[0] != 0:
        ...
        new_gt_bbox_ir_unmatch = []
        new_gt_bbox_ir_match = []
        new_gt_class_ir = []
        new_teacher_bbox_unmatch = []
        new_teacher_bbox_match = []

        for mm in range(len(gt_bbox_ir)):
            coordinates_ir = gt_bbox_ir[mm].numpy()
            ...
            self.vis_gt_data['gt_bbox_vis'][index] = transform_bbox_filp(self.vis_gt_data['gt_bbox_vis'][index],W,H,flipped_mm)
            for i in range(len(coordinates_ir)):
                center_points_vis=coordinates_vis[:,0:2]
                ir_search_scale=(coordinates_ir[i][0],coordinates_ir[i][1],coordinates_ir[i][2]*1.60,coordinates_ir[i][3]*1.60)
                ...
                # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/bbox_Drone_Vehicle/train_imgs/label_vis/{(im_id_mm+1):05d}_epoch{self.epoch}_{i}.jpg', quality=100)   
                if np.any(mask):
                    for k in index_in_search:
                        if k in record_pop_idx_vis:
                            continue
                        coordinates_vis_k_transform2sameorigin=np.array([coordinates_ir[i][0],coordinates_ir[i][1],coordinates_vis[k][2],coordinates_vis[k][3]])
                    ...
                    if best_k!=-1:  # 匹配上了
                        ...
                        if self.epoch <= self.semi_start_epochs+4:
                            self.vis_gt_data['gt_bbox_vis'][index][i,:]=paddle.to_tensor(coordinates_vis_whcopy)
                            ...
                            deta_x=coordinates_vis_whcopy[0]-self.vis_gt_data['gt_bbox_vis'][index][i][0]
                            deta_y=coordinates_vis_whcopy[1]-self.vis_gt_data['gt_bbox_vis'][index][i][1]
                            ...
                            self.vis_gt_data['gt_bbox_vis'][index][i,:]=paddle.to_tensor(np.array([self.vis_gt_data['gt_bbox_vis'][index][i][0]+lam*deta_x,self.vis_gt_data['gt_bbox_vis'][index][i][1]+lam*deta_y,coordinates_ir[i][2],coordinates_ir[i][3]], dtype='float32'))
                        coordinates_ir_match.append(coordinates_ir[i])
                        coordinates_vis_match.append(self.vis_gt_data['gt_bbox_vis'][index][i,:])
                        ...
                        if self.epoch<self.semi_start_epochs+4:
                            self.vis_gt_data['gt_bbox_vis'][index][i,:]=paddle.to_tensor(np.array([np.nan, np.nan, np.nan, np.nan]), dtype='float32')
                            self.vis_gt_data['gt_class'][index][i,:]=paddle.to_tensor([-1], dtype='int32')

                        elif self.epoch==self.semi_start_epochs+4:
                            self.vis_gt_data['gt_bbox_vis'][index][i,:]=paddle.to_tensor(coordinates_ir[i])
                            self.vis_gt_data['gt_class'][index][i,:]=paddle.to_tensor(categories_ir[i])
                            coordinates_ir_match.append(coordinates_ir[i])
                            coordinates_vis_match.append(self.vis_gt_data['gt_bbox_vis'][index][i])
                            categories_ir_new.append(categories_ir[i])
                    ...
                    if self.epoch<self.semi_start_epochs+4:
                        ...
                        categories_ir_new.append(categories_ir[i])
                        categories_vis_match.append(self.vis_gt_data['gt_class'][index][i])              
            # bboxes=copy.deepcopy(self.vis_gt_data['gt_bbox_vis'][index]) 
            # image=tensor_to_pil_image(data['vis_image'][mm])
            # image=draw_bbox_vis(image,bboxes)
            # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/bbox_Drone_Vehicle/train_imgs/label_vis/{(im_id_mm+1):05d}_before.jpg', quality=99)   
            
            origin_vis_bboxes_mm = transform_bbox_filp(copy.deepcopy(self.vis_gt_data['gt_bbox_vis'][index]),W,H,flipped_mm,ratio3=1.0)          
            origin_ir_bboxes_mm = transform_bbox_filp(paddle.to_tensor(copy.deepcopy(coordinates_ir),dtype='float32'),W,H,flipped_mm,ratio3=1.0)        
            folder = f'/data1/jinhang/Experiment/shiftcorrect_vision/{self.epoch}/train'
            # 创建文件夹（如果不存在，且支持递归创建多级目录）
            os.makedirs(folder, exist_ok=True)
            filename=self.coco_ir.loadImgs([im_id_mm])[0]['file_name']
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs/vis/{filename}'):
                image_vis=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs/vis/{filename}').convert('RGB')
                bboxes_vis=copy.deepcopy(origin_vis_bboxes_mm)
                bboxes_ir=copy.deepcopy(origin_ir_bboxes_mm)
                image=draw_bbox_vis(image_vis,bboxes_vis,bboxes_ir)
                image.save(f"{folder}/{filename.split('.')[0]}_vis_shiftcorrect.jpg", quality=100)
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs/ir/{filename}'):
                image_ir=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs/ir/{filename}').convert('RGB')
                bboxes=copy.deepcopy(origin_ir_bboxes_mm)
                image=draw_bbox_ir(image_ir,bboxes)
                image.save(f"{folder}/{filename.split('.')[0]}_ir_gt.jpg", quality=100)
            self.vis_gt_data['gt_bbox_vis'][index] = transform_bbox_filp(self.vis_gt_data['gt_bbox_vis'][index],W,H,flipped_mm)
            # bboxes=copy.deepcopy(self.vis_gt_data['gt_bbox_vis'][index]) 
            # image=tensor_to_pil_image(data['vis_image'][mm])
            # image=draw_bbox_vis(image,bboxes)
            # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/bbox_Drone_Vehicle/train_imgs/label_vis/{(im_id_mm+1):05d}_latter.jpg', quality=99)              
            if self.epoch<self.semi_start_epochs+4:
            ...
            new_gt_bbox_ir_match.append(paddle.to_tensor(coordinates_ir_match))
            ...
            if self.epoch<self.semi_start_epochs+4:
                new_gt_bbox_ir_unmatch.append(paddle.to_tensor([]))
            else:
                new_gt_bbox_ir_unmatch.append(paddle.to_tensor(coordinates_ir_unmatch))            
        ...
        for i,_ in enumerate(new_gt_bbox_ir_match):
            if new_gt_bbox_ir_match[i].shape[0] > 0:
                new_gt_bbox_ir_match[i][:,0] = new_gt_bbox_ir_match[i][:,0] / W
                new_gt_bbox_ir_match[i][:,1] = new_gt_bbox_ir_match[i][:,1] / H
                new_gt_bbox_ir_match[i][:,2] = new_gt_bbox_ir_match[i][:,2] / W
                new_gt_bbox_ir_match[i][:,3] = new_gt_bbox_ir_match[i][:,3] / H
            else:
                pass
        for i, _ in enumerate(new_gt_bbox_ir_unmatch):
            # 检查每个张量是否为空
            if new_gt_bbox_ir_unmatch[i].shape[0] > 0:
                # 仅对非空张量进行操作
                new_gt_bbox_ir_unmatch[i][:, 0] = new_gt_bbox_ir_unmatch[i][:, 0] / W
                new_gt_bbox_ir_unmatch[i][:, 1] = new_gt_bbox_ir_unmatch[i][:, 1] / H
                new_gt_bbox_ir_unmatch[i][:, 2] = new_gt_bbox_ir_unmatch[i][:, 2] / W
                new_gt_bbox_ir_unmatch[i][:, 3] = new_gt_bbox_ir_unmatch[i][:, 3] / H
            else:
                pass
        for i,_ in enumerate(new_teacher_bbox_match):
            if new_teacher_bbox_match[i].shape[0] > 0:
                new_teacher_bbox_match[i][:,0] = new_teacher_bbox_match[i][:,0] / W
                new_teacher_bbox_match[i][:,1] = new_teacher_bbox_match[i][:,1] / H
                new_teacher_bbox_match[i][:,2] = new_teacher_bbox_match[i][:,2] / W
                new_teacher_bbox_match[i][:,3] = new_teacher_bbox_match[i][:,3] / H
            else:
                pass
        for i, _ in enumerate(new_teacher_bbox_unmatch):
            # 检查每个张量是否为空
            if new_teacher_bbox_unmatch[i].shape[0] > 0:
                # 仅对非空张量进行操作
                new_teacher_bbox_unmatch[i][:, 0] = new_teacher_bbox_unmatch[i][:, 0] / W
                new_teacher_bbox_unmatch[i][:, 1] = new_teacher_bbox_unmatch[i][:, 1] / H
                new_teacher_bbox_unmatch[i][:, 2] = new_teacher_bbox_unmatch[i][:, 2] / W
                new_teacher_bbox_unmatch[i][:, 3] = new_teacher_bbox_unmatch[i][:, 3] / H
            else:
                pass
        return new_gt_bbox_ir_match, new_gt_bbox_ir_unmatch, new_gt_class_ir, new_teacher_bbox_match, new_teacher_bbox_unmatch

...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder###################
                        if curr_iter < stage2_iter:
                            ...
                            if curr_iter >= st_iter:
                                ...
                                #self.paired_label_visual(data['im_id'],data['ir_image'],data['vis_image'],{'bbox': data['gt_bbox_ir'],'class':data['gt_class']},{'bbox': data['gt_bbox_vis'],'class':data['gt_class']},H,W)
                                data['teacher_gt_class']=data['gt_class']
                                data['teacher_gt_bbox_vis']=data['gt_bbox_vis']
                                data['flag'] = 4
                                ...
                                loss_dict_unsup = self.model.get_ssod_loss_single((student_preds[0], student_preds[1],
                                                                            data['gt_bbox_vis'], data['gt_class'], data['im_shape'],
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
                        if curr_iter >= stage2_iter and curr_iter < stage3_iter: #STAGE 2
                            ...
                            losses_unsup = loss_dict_unsup['loss'] * 1.0 #
                            losses_unsup.backward()
                            ...
                            data['paired_gt_bbox_ir_match'] = data['gt_bbox_ir']
                            data['paired_gt_bbox_ir_unmatch'] = [paddle.to_tensor([])] * len(data['gt_bbox_ir'])
                            data['paired_gt_class'] = data['gt_class']
                            data['paired_teacher_bbox_match'] = data['gt_bbox_vis'] 
                            data['paired_teacher_bbox_unmatch'] = [paddle.to_tensor([])] * len(data['gt_bbox_vis'])
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def paired_label_visual(self, im_id, ir_image, vis_image, bboxes_ir, bboxes_vis, H, W):
        ...
        folder = os.path.join(
            REPO_ROOT,
            'label_vision',
            'irgt_vispred_pairs_iou_vision',
            'paired_label_visual3',
            str(self.epoch))
    ...
    def paired_label_visual_shiftcorrectshow(self,im_id, ir_image, vis_image, bboxes_ir, bboxes_vis):
        ...
        folder = os.path.join(
            REPO_ROOT,
            'label_vision',
            'pseudo_labels_shiftcorrect_vision_shiftcorrectshow3',
            str(self.epoch))
        # 创建文件夹（如果不存在，且支持递归创建多级目录）
        os.makedirs(folder, exist_ok=True)
        ir_images=tensor_to_pil_images(ir_image)

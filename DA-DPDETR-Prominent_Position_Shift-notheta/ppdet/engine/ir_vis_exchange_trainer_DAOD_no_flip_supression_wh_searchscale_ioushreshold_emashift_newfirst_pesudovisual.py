    ...
    def train(self, validate=False):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id, data in enumerate(self.loader):
                ...
                # # --- 使用更稳妥的方法进行修改 ---
                # modified_slices = []  # 1. 创建一个空列表
                # for i in range(gt_bbox_ir.shape[0]):
                #     # 从原始tensor中取出一个切片
                #     original_slice = gt_bbox_ir[i]
                #     # 复制这个切片，确保我们修改的是一个全新的对象
                #     new_slice = original_slice.clone()
                #     # 2. 对这个全新的切片进行计算
                #     new_slice[:, 0] = new_slice[:, 0] * W
                #     new_slice[:, 1] = new_slice[:, 1] * H
                #     new_slice[:, 2] = new_slice[:, 2] * W
                #     new_slice[:, 3] = new_slice[:, 3] * H
                #     # 3. 将修改后的新切片添加到列表中
                #     modified_slices.append(new_slice)
                # # 4. 使用 paddle.stack 将列表中的所有小 Tensor 重新组合成一个大 Tensor
                # # (如果你用的是 PyTorch, 则用 torch.stack)
                # gt_bbox_ir = modified_slices 
                self.step = step_id
                                ...
                                if epoch_id==self.semi_start_epochs:###记得保存模型的时候也得把self.vis_gt_data保存下来                                  
                                    if 'im_id' not in self.vis_gt_data and 'gt_bbox_vis' not in self.vis_gt_data:
                                        self.vis_gt_data = {
                                            ...
                                            'gt_bbox_vis': [t.clone() for t in data['gt_bbox_ir']],
                                        ...
                                        self.vis_gt_data['gt_bbox_vis']+= copy.deepcopy(data['gt_bbox_ir'])
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def paired_label_visual(self, im_id, ir_image, vis_image, bboxes_ir, bboxes_vis, H, W):
        ...
        #     if len(bbox_tensor.shape) < 2 or bbox_tensor.shape[0] == 0:
        #         bbox_tensor = paddle.empty([0, 4], dtype=bbox_tensor.dtype)
        for bbox_tensor in bboxes_vis['bbox']:
            if bbox_tensor is not None and bbox_tensor.shape[0] > 0:
                # 只有在张量包含至少一个边界框时，才执行缩放操作
                bbox_tensor[:, 0] *= W
                bbox_tensor[:, 2] *= W
        ...
        def tensor_to_pil_images(tensor):
            ...
            return images
        folder = f'/data1/jinhang/Projects/DOD-SA/DA-DPDETR-Prominent_Position_Shift-kaist-notheta/DA-DPDETR/label_vision/irgt_vispred_pairs_iou_vision/paired_label_visual6/{self.epoch}'
    ...
    def paired_label_visual_shiftcorrectshow(self,im_id, ir_image, vis_image, bboxes_ir, bboxes_vis):
        ...
        def tensor_to_pil_images(tensor):
            ...
            return images
        folder = f'/data1/jinhang/Projects/DOD-SA/DA-DPDETR-Prominent_Position_Shift-kaist-notheta/DA-DPDETR/label_vision/pseudo_labels_shiftcorrect_vision_shiftcorrectshow6/{self.epoch}'
        # 创建文件夹（如果不存在，且支持递归创建多级目录）
        os.makedirs(folder, exist_ok=True)
        ir_images=tensor_to_pil_images(ir_image)
        vis_images=tensor_to_pil_images(vis_image)
    ...
    def paired_label_processing(self, data, teacher_bbox, teacher_class, teacher_score,iou_threshold=0.7):
        ...
        gt_bbox_ir=[]
        #teacher_bbox=copy.deepcopy(teacher_bbox)
        for bbox_tensor in teacher_bbox:
            if bbox_tensor is not None and bbox_tensor.shape[0] > 0:
                # 只有在张量包含至少一个边界框时，才执行缩放操作
                bbox_tensor[:, 0] /= W
                bbox_tensor[:, 2] /= W
        ...
        for i,_ in enumerate(gt_bbox_ir_Normalized):
            ...
            gt_bbox_ir[i][:,0] = gt_bbox_ir_Normalized[i][:,0]
            gt_bbox_ir[i][:,1] = gt_bbox_ir_Normalized[i][:,1]
            gt_bbox_ir[i][:,2] = gt_bbox_ir_Normalized[i][:,2]
            gt_bbox_ir[i][:,3] = gt_bbox_ir_Normalized[i][:,3]
        flipped=copy.deepcopy(data['flipped'])
        def transform_bbox_filp(bboxes,flipped_mm,ratio3=None,H=None,W=None):
            x = bboxes[:, 0]
            y = bboxes[:, 1]
            w = bboxes[:, 2]
            h = bboxes[:, 3]
        ...
        for mm in range(len(gt_bbox_ir)):
            ...
            self.vis_gt_data['gt_bbox_vis'][index] = transform_bbox_filp(self.vis_gt_data['gt_bbox_vis'][index],flipped_mm,ratio3=1.0)
            for i in range(len(coordinates_ir)):
                #检查coordinates_vis的维度，如果是一维数组或为空，则跳过
                # if len(coordinates_vis.shape) < 2 or coordinates_vis.shape[0] == 0:
                #     if self.epoch<self.semi_start_epochs+4:
                ...
                ir_search_scale=(coordinates_ir[i][0],coordinates_ir[i][1],coordinates_ir[i][2]*1.0,coordinates_ir[i][3]*1.0)
                ...
                # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/bbox_Drone_Vehicle/train_imgs_1000/label_vis/{(im_id_mm+1):05d}_epoch{self.epoch}_{i}.jpg', format='PNG')   
                if np.any(mask):
            ...
            # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/bbox_Drone_Vehicle/train_imgs_1000/label_vis/{(im_id_mm+1):05d}_before.jpg', quality=99)   
            
            origin_vis_bboxes_mm = transform_bbox_filp(copy.deepcopy(self.vis_gt_data['gt_bbox_vis'][index]),flipped_mm,ratio3=None,H=512,W=640)          
            origin_ir_bboxes_mm = transform_bbox_filp(paddle.to_tensor(copy.deepcopy(coordinates_ir),dtype='float32'),flipped_mm,ratio3=None,H=512,W=640)        
            folder = f'/data1/jinhang/Projects/DOD-SA/DA-DPDETR-Prominent_Position_Shift-kaist-notheta/DA-DPDETR/label_vision/pseudo_labels_shiftcorrect_vision/shiftcorrect_vision6/{self.epoch}'
            # 创建文件夹（如果不存在，且支持递归创建多级目录）
            os.makedirs(folder, exist_ok=True)
            filename=self.coco_ir.loadImgs([im_id_mm])[0]['file_name']
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs_1000/ir/{filename}'):
                image_vis=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs_1000/ir/{filename}').convert('RGB')
                ...
                image.save(f"{folder}/{filename.split('.')[0]}_ir_shiftcorrect.jpg", format='PNG')
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs_1000/vis/{filename}'):
                image_ir=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/train_imgs_1000/vis/{filename}').convert('RGB')
                bboxes=copy.deepcopy(origin_ir_bboxes_mm)
                image=draw_bbox_ir(image_ir,bboxes)
                image.save(f"{folder}/{filename.split('.')[0]}_vis_gt.jpg", format='PNG')
            self.vis_gt_data['gt_bbox_vis'][index] = transform_bbox_filp(self.vis_gt_data['gt_bbox_vis'][index],flipped_mm,ratio3=1.0)
            # bboxes=copy.deepcopy(self.vis_gt_data['gt_bbox_vis'][index]) 
            # image=tensor_to_pil_image(data['vis_image'][mm])
            # image=draw_bbox_vis(image,bboxes)
            # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/bbox_Drone_Vehicle/train_imgs_1000/label_vis/{(im_id_mm+1):05d}_latter.jpg', quality=99)              
            if self.epoch<self.semi_start_epochs+4:

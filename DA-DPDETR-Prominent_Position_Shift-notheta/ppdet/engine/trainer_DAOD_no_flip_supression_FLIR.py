    ...
    def paired_label_processing(self, data, teacher_bbox, teacher_class, teacher_score,iou_threshold=0.7):
        ...
        for mm in range(len(gt_bbox_ir)):
            ...
            for i in range(len(coordinates_ir)):
                ...
                # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/bbox_Drone_Vehicle/FLIR-image/label_vis/{(im_id_mm+1):05d}_epoch{self.epoch}_{i}.jpg', format='PNG')   
                if np.any(mask):
            ...
            # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/bbox_Drone_Vehicle/FLIR-image/label_vis/{(im_id_mm+1):05d}_before.jpg', quality=99)   
            if use_affine:
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/FLIR-image/vi-train_1000/{filename}'):
                image_vis=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/FLIR-image/vi-train_1000/{filename}').convert('RGB')
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/FLIR-image/ir-train_1000/{filename}'):
                image_ir=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/FLIR-image/ir-train_1000/{filename}').convert('RGB')
            ...
            # image=tensor_to_pil_image(data['vis_image'][mm])
            # image=draw_bbox_vis(image,bboxes)
            # image.save(f'/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/bbox_Drone_Vehicle/FLIR-image/label_vis/{(im_id_mm+1):05d}_latter.jpg', quality=99)              
            # if self.epoch<self.semi_start_epochs+4:

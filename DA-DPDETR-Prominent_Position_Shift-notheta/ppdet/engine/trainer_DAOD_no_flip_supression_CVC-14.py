...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def paired_label_processing(self, data, teacher_bbox, teacher_class, teacher_score,iou_threshold=0.65):
        ...
        for mm in range(len(gt_bbox_ir)):
            ...
            for i in range(len(coordinates_ir)):
                ...
                ir_search_scale=(coordinates_ir[i][0],coordinates_ir[i][1],coordinates_ir[i][2]*2.0,coordinates_ir[i][3]*2.0)
                ...
                if np.any(mask):
                    ...
                    if best_k!=-1:  # 匹配上了
                        ...
                        self.vis_gt_data['gt_bbox_vis'][index][mask_rank,:]=paddle.to_tensor(np.array([coordinates_vis_whcopy[0], coordinates_vis_whcopy[1],coordinates_ir[i][2],coordinates_ir[i][3]], dtype='float32'))
                        ...
                        if self.epoch==self.semi_start_epochs+4: #从这一刻才开始做 Bag-update
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_alllabeled/ir/images/{filename}'):
                image_vis=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/train_alllabeled/ir/images/{filename}').convert('RGB')
            ...
            if os.path.exists(f'{self.cfg.TrainDataset.dataset_dir}/train_alllabeled/vis/images/{filename}'):
                image_ir=Image.open(f'{self.cfg.TrainDataset.dataset_dir}/train_alllabeled/vis/images/{filename}').convert('RGB')

                        ...
                        if curr_iter < stage2_iter:
                            ...
                            if curr_iter >= st_iter:
                                ...
                                paired_gt_bbox_ir_match, paired_gt_bbox_ir_unmatch, paired_gt_class, \
                                    paired_teacher_bbox_match, paired_teacher_bbox_unmatch,match_teacher_categories,match_teacher_score,res_teacher_coordinates,res_teacher_categories,res_teacher_score,match_iou,unmatch_iou\
                                    = self.paired_label_processing(
                                        data,
                                        teacher_preds['bbox'],
                                        teacher_preds['class'],
                                        teacher_preds['score']
                                ...
                                gt_bbox_ir=[]
                                for i,_ in enumerate(data['gt_bbox_ir']):
                                    gt_bbox_ir.append(copy.deepcopy(data['gt_bbox_ir'])[i])
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
                        if curr_iter >= stage2_iter and curr_iter < stage3_iter: #STAGE 2
                            ...
                            data['teacher_gt_bbox_vis']=teacher_preds['bbox']                
                            ...
                            loss_dict_unsup = self.model.get_ssod_loss_single(
                                (student_preds[0], student_preds[1],
                                 teacher_preds['bbox'], teacher_preds['class'], data['im_shape'],
                                 student_preds[2],student_preds[3],student_preds[4]))
    ...
    def paired_label_processing(self, data, teacher_bbox, teacher_class, teacher_score,iou_threshold=0.7):
        ...
        def points_in_rotated_box(points, box):
            ...
            参数:
                ...
                box: tuple (cx, cy, w, h, theta)，矩形中心、宽、高、角度（度）
        ...
        new_teacher_categories_match=[]
        new_teacher_score_match=[]
        res_teacher_coordinates=[]
        res_teacher_categories=[]
        res_teacher_score=[]
        new_record_ir_match_iou=[]
        new_record_ir_unmatch_iou=[]
        for mm in range(len(gt_bbox_ir)):
            ...
            score_vis = teacher_score[mm].numpy()
            ...
            #             max_iou=iou
            #             max_iou_k=k
            #     ir_iou.append(max_iou)
            for i in range(len(coordinates_ir)):
                center_points_vis=coordinates_vis[:,0:2]
                ir_search_scale=(coordinates_ir[i][0],coordinates_ir[i][1],coordinates_ir[i][2]*1.7,coordinates_ir[i][3]*1.7)
                ...
                if np.any(mask):
                    ...
                    if best_k!=-1:  # 匹配上了
                        ...
                        coordinates_vis_match.append(coordinates_vis_whcopy)
                        categories_ir_new.append(categories_ir[i])
                        categories_vis_match.append(categories_vis[best_k])
                        score_vis_match.append(score_vis[best_k])
                        record_pop_idx_ir.append(i)  # 记录匹配上的idx
                        record_pop_idx_vis.append(best_k)
                        record_ir_match_iou.append(ir_iou[i])
            ...
            score_vis = [score_vis[i] for i in range(len(score_vis)) if i not in record_pop_idx_vis]
            ir_iou=[ir_iou[i] for i in range(len(ir_iou)) if i not in record_pop_idx_ir]
            ...
            for box, cls in zip(coordinates_ir, categories_ir):
                ...
                #score_vis_new.append(score_vis[k])
                # 再复制红外的
            ...
            new_teacher_categories_match.append(paddle.to_tensor(categories_vis_match))
            new_teacher_score_match.append(paddle.to_tensor(score_vis_match))
            res_teacher_coordinates.append(paddle.to_tensor(coordinates_vis))
            res_teacher_categories.append(paddle.to_tensor(categories_vis))
            res_teacher_score.append(paddle.to_tensor(score_vis))
            new_record_ir_match_iou.append(paddle.to_tensor(record_ir_match_iou))
            new_record_ir_unmatch_iou.append(paddle.to_tensor(ir_iou))
        ...
        for i, _ in enumerate(new_teacher_bbox_unmatch):
            ...
            if new_teacher_bbox_unmatch[i].shape[0] > 0:
                ...
                pass
        return new_gt_bbox_ir_match, new_gt_bbox_ir_unmatch, new_gt_class_ir, new_teacher_bbox_match, new_teacher_bbox_unmatch,new_teacher_categories_match,new_teacher_score_match,res_teacher_coordinates,res_teacher_categories,res_teacher_score,new_record_ir_match_iou,new_record_ir_unmatch_iou
        #match红外框、unmatch红外框、[match+unmatch]的红外类别、match可见光框、unmatch可见光框、match的可见光类别、match的可见光分数、没匹配上的可见光坐标、类别、分数

...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def get_rect_params(self,box_10d):
        """
        从 10维向量中提取水平框参数 (cx, cy, w, h)，角度固定为 0
        box_10d: [cls, score, x1, y1, x2, y2, x3, y3, x4, y4]
        """
        coords = np.array(box_10d[2:], dtype=np.float32)
        if coords.size == 4:
            # 格式: [x1, y1, x2, y2]
            x1, y1, x2, y2 = coords
    ...
    def points_in_rotated_box(self,points, box):
        ...
        dx = points[:, 0] - cx
        dy = points[:, 1] - cy
        inside_x = np.logical_and(dx >= -half_w, dx <= half_w)
        inside_y = np.logical_and(dy >= -half_h, dy <= half_h)

...
from .op_helper import (satisfy_sample_constraint, filter_and_process,
                        ...
                        is_poly, get_border)
from .paired_transforms import PairedTransformHelper
...
class Multi_Decode(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        PairedTransformHelper.reset(sample, shape=vis_im.shape[:2])
        if 'h' not in sample:
            sample['h'] = ir_im.shape[0]
        elif sample['h'] != ir_im.shape[0]:
            logger.warning(
            ...
            sample['h'] = ir_im.shape[0]
        if 'w' not in sample:
            sample['w'] = ir_im.shape[1]
        elif sample['w'] != ir_im.shape[1]:
            logger.warning(
        ...
        if 'gt_bbox_keep_mask' not in sample:
            mask_src = None
            for key in ['gt_bbox_vis', 'gt_bbox_ir', 'gt_bbox']:
                if key in sample:
                    mask_src = sample[key]
                    break
            if mask_src is None:
                num_gts = 0
            elif hasattr(mask_src, 'shape'):
                num_gts = mask_src.shape[0]
            else:
                num_gts = len(mask_src)
            sample['gt_bbox_keep_mask'] = np.ones(
                (num_gts, ), dtype=np.int32)
        return sample

@register_op
class Multi_Decode_sizefree(BaseOperator):
    def __init__(self):
        """ Transform the muiti_image data to numpy format following the rgb format
        """
        super(Multi_Decode_sizefree, self).__init__()
...
@register_op
class Multi_Decode_MASK(BaseOperator):
    def __init__(self):
        """ Transform the muiti_image data to numpy format following the rgb format
        """
        super(Multi_Decode_MASK, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        if random.random() < 0.1:
            ...
            if random.random() < 0.5:
                sample['vis_image'] = np.zeros((int(sample['h']),int(sample['w']),3),dtype=np.uint8)
            else:
                sample['ir_image'] = np.zeros((int(sample['h']), int(sample['w']), 3), dtype=np.uint8)

        return sample

@register_op
class Multi_Decode_PATCH_MASK(BaseOperator):
    def __init__(self):
        """ Transform the muiti_image data to numpy format following the rgb format
        """
        super(Multi_Decode_PATCH_MASK, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        if random.random() < 0.3:
            ...
            width = sample['w']
            # 定义patch的大小
            patch_size = (width // 10, height // 10)

            # 创建一个与图像大小相同的全白掩膜
            mask_vis = np.ones_like(sample['vis_image']) * 255
            mask_ir = np.ones_like(sample['ir_image']) * 255
            ...
            for row in range(10):
                ...
                cols = list(range(10))
                # 随机选择两个不同的列索引
                selected_cols = np.random.choice(cols, size=2, replace=False)
                # 计算选中的patch的起始和结束坐标
                start_x1 = int(selected_cols[0] * patch_size[0])
                end_x1 = int((selected_cols[0] + 1) * patch_size[0])
                start_x2 = int(selected_cols[1] * patch_size[0])
                end_x2 = int((selected_cols[1] + 1) * patch_size[0])
                start_y = int(row * patch_size[1])
                end_y = int((row + 1) * patch_size[1])

                #
                # start_x3 = int(selected_cols[2] * patch_size[0])
                # end_x3 = int((selected_cols[2] + 1) * patch_size[0])
                # start_x4 = int(selected_cols[3] * patch_size[0])
                # end_x4 = int((selected_cols[3] + 1) * patch_size[0])



                # 在掩膜上将选中的patch区域设置为黑色（0）
                mask_vis[start_y:end_y, start_x1:end_x1] = 0
                mask_ir[start_y:end_y, start_x2:end_x2] = 0
                #
                # mask_vis[start_y:end_y, start_x3:end_x3] = 0
                # mask_ir[start_y:end_y, start_x4:end_x4] = 0

            # 应用掩膜到原始图像
            sample['vis_image'] = cv2.bitwise_and(sample['vis_image'], mask_vis)
            sample['ir_image'] = cv2.bitwise_and(sample['ir_image'], mask_ir)
...
@register_op
class Multi_Permute_DAOD(BaseOperator):
    def __init__(self):
        """
        Change the channel to be (C, H, W)
        """
        super(Multi_Permute_DAOD, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        ir_im = ir_im.transpose((2, 0, 1))
        sample['ir_image'] = ir_im
        if 'vis_image_aug' in sample:
            vis_im_aug = sample['vis_image_aug']
            vis_im_aug = vis_im_aug.transpose((2, 0, 1))
            sample['vis_image_aug'] = vis_im_aug
        if 'ir_image_aug' in sample:
            ir_im_aug = sample['ir_image_aug']
            ir_im_aug = ir_im_aug.transpose((2, 0, 1))
            sample['ir_image_aug'] = ir_im_aug
        if 'ir_image_flip' in sample:
            ir_im_flip = sample['ir_image_flip']
            ir_im_flip = ir_im_flip.transpose((2, 0, 1))
            sample['ir_image_flip'] = ir_im_flip
        if 'ir_image_aug_flip' in sample:
            ir_im_aug_flip = sample['ir_image_aug_flip']
            ir_im_aug_flip = ir_im_aug_flip.transpose((2, 0, 1))
            sample['ir_image_aug_flip'] = ir_im_aug_flip
...
@register_op
class Multi_NormalizeImage_DAOD(BaseOperator):
    def __init__(self,
                 mean=[0.485, 0.456, 0.406],
                 std=[0.229, 0.224, 0.225],
                 is_scale=True,
        ...
        Args:
            ...
            norm_type (str): type in ['mean_std', 'none']
        """
        super(Multi_NormalizeImage_DAOD, self).__init__()
        self.mean = mean
        self.std = std
        self.is_scale = is_scale
        self.norm_type = norm_type
    ...
    def apply(self, sample, context=None):
        ...
        # vis_im_aug = sample['vis_image_aug'].setflags(write=True)
        # ir_im_aug = sample['ir_image_aug'].setflags(write=True)
        vis_im_aug = sample['vis_image_aug']
        ir_im_aug = sample['ir_image_aug']
        ...
        vis_im_aug = vis_im_aug.copy().astype(np.float32, copy=False)
        ir_im_aug = ir_im_aug.copy().astype(np.float32, copy=False)

        # ir_im_flip = ir_im_flip.copy().astype(np.float32, copy=False)
        # ir_im_aug_flip = ir_im_aug_flip.copy().astype(np.float32, copy=False)
        ...
        if self.is_scale:
            ...
            try:
                ...
                # ir_im_flip *= scale
                # ir_im_aug_flip *= scale

            except:
                print(vis_im_aug.flags)
    ...
    def apply(self, sample, context=None):
        ...
        if self.random_channel:
            if np.random.randint(0, 2):
                temp = np.random.permutation(3)
                vis_img = vis_img[..., temp]
                ir_img = ir_img[..., temp]
        sample['vis_image'] = vis_img
        sample['ir_image'] = ir_img
        return sample
...
@register_op
class Flip_DAOD(BaseOperator):
    def __init__(self, prob=1.0):
        """
        Args:
            prob (float): the probability of flipping image
        """
        super(Flip_DAOD, self).__init__()
        self.prob = prob
        if not (isinstance(self.prob, float)):
            raise TypeError("{}: input type is invalid.".format(self))
    ...
    def apply_image(self, image):
        # cv2.imwrite('/data/hdd/guojunjie/pp-output-rtdetr-r-drone-vehicle-DAOD/original.jpg', image)
        # cv2.imwrite('/data/hdd/guojunjie/pp-output-rtdetr-r-drone-vehicle-DAOD/test.jpg', image[:, ::-1, :])
        return image[:, ::-1, :]
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ir_im = sample['ir_image']
            ir_im_aug = sample['ir_image_aug']
            height, width = ir_im.shape[:2]
            ir_im = self.apply_image(ir_im)
            ir_im_aug = self.apply_image(ir_im_aug)

            sample['ir_image_flip'] = ir_im
            sample['ir_image_aug_flip'] = ir_im_aug
        return sample
...
class Resize(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        PairedTransformHelper.scale(
            sample, im_scale_x, im_scale_y, new_size=(resize_w, resize_h))
        return sample
...
class Multi_Resize(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if len(vis_im.shape) == 3:
            if self.keep_ratio:
                vis_im = self.apply_image(sample['vis_image'],
                                          [im_scale_x, im_scale_y])
                ir_im = self.apply_image(sample['ir_image'],
                                         [im_scale_x, im_scale_y])
            else:
                vis_im = cv2.resize(
                    sample['vis_image'],
            ...
            for one_vis_im, one_ir_im in zip(vis_im, ir_im):
                if self.keep_ratio:
                    applied_vis_im = self.apply_image(one_vis_im,
                                                      [im_scale_x, im_scale_y])
                    applied_ir_im = self.apply_image(one_ir_im,
                                                     [im_scale_x, im_scale_y])
                else:
                    applied_vis_im = cv2.resize(
                        one_vis_im, (resize_w, resize_h),
                        interpolation=self.interp)
                    applied_ir_im = cv2.resize(
                        one_ir_im, (resize_w, resize_h),
                        interpolation=self.interp)
...
@register_op
class Multi_Resize_Paired(BaseOperator):
    def __init__(self, target_size, keep_ratio, interp=cv2.INTER_LINEAR):
        """
        Resize image to target size. if keep_ratio is True,
        resize the image's long side to the maximum of target_size
        ...
        Args:
            ...
            interp (int): the interpolation method
        """
        super(Multi_Resize_Paired, self).__init__()
        self.keep_ratio = keep_ratio
        self.interp = interp
        if not isinstance(target_size, (Integral, Sequence)):
            raise TypeError(
    ...
    def apply(self, sample, context=None):
        ...
        if len(vis_im.shape) == 3:
            vis_im_aug = self.apply_image(sample['vis_image_aug'], [im_scale_x, im_scale_y])
            ir_im_aug = self.apply_image(sample['ir_image_aug'], [im_scale_x, im_scale_y])            
            ...
            sample['vis_image_aug'] = vis_im_aug.astype(np.float32)
            sample['ir_image_aug'] = ir_im_aug.astype(np.float32)
            sample['vis_image'] = vis_im.astype(np.float32)
            sample['ir_image'] = ir_im.astype(np.float32)
        else:
            print('vis_im的形状不是3维的')
            resized_vis_images = []
            resized_ir_images = []
            for one_vis_im,one_ir_im in zip(vis_im,ir_im):
        ...
        if 'gt_bbox_vis' in sample and len(sample['gt_bbox_vis']) > 0:
            sample['gt_bbox_vis'] = self.apply_bbox(sample['gt_bbox_vis'],
                                                [im_scale_x, im_scale_y],
                                                [resize_w, resize_h])
            sample['gt_bbox_ir'] = self.apply_bbox(sample['gt_bbox_ir'],
                                                    [im_scale_x, im_scale_y],
                                                    [resize_w, resize_h])
...
@register_op
class Multi_RandomExpand_IRVIS_Paired(BaseOperator):
    """Random expand the canvas.
    Args:
        ratio (float): maximum expansion ratio.
        prob (float): probability to expand.
    ...
    def __init__(self, ratio=4., prob=0.5, fill_value=(127.5, 127.5, 127.5), fill_value_ir=(114.495, 114.495, 114.495)):
        super(Multi_RandomExpand_IRVIS_Paired, self).__init__()
        assert ratio > 1.01, "expand ratio must be larger than 1.01"
        self.ratio = ratio
        self.prob = prob
        assert isinstance(fill_value, (Number, Sequence)), \
    ...
    def apply(self, sample, context=None):
        ...
        pad = Multi_Pad_IRVIS_Paired(size,
                  pad_mode=-1,
                  offsets=offsets,
                  fill_value=self.fill_value,
                  fill_value_ir=self.fill_value_ir)
...
@register_op
class Multi_RandomExpand_IRVIS_Paired_sizefree(BaseOperator):
    """Random expand the canvas.
    Args:
        ratio (float): maximum expansion ratio.
        prob (float): probability to expand.
    ...
    def __init__(self, ratio=4., prob=0.5, fill_value=(127.5, 127.5, 127.5), fill_value_ir=(114.495, 114.495, 114.495)):
        super(Multi_RandomExpand_IRVIS_Paired_sizefree, self).__init__()
        assert ratio > 1.01, "expand ratio must be larger than 1.01"
        self.ratio = ratio
        self.prob = prob
        assert isinstance(fill_value, (Number, Sequence)), \
...
@register_op
class Multi_RandomCrop_Paired(BaseOperator):
    """Random crop image and bboxes.
    Args:
        aspect_ratio (list): aspect ratio of cropped region.
            in [min, max] format.
    ...
    def __init__(self,
                 ...
                 ioumode="iou",
                 prob=1.0):
        super(Multi_RandomCrop_Paired, self).__init__()
        self.aspect_ratio = aspect_ratio
        self.thresholds = thresholds
        self.scaling = scaling
        self.num_attempts = num_attempts
    ...
    def _set_bbox_keep_mask(self, sample, num_gts=None, valid_ids=None):
        if num_gts is None:
            gt_bbox = sample.get('gt_bbox_vis')
            num_gts = 0 if gt_bbox is None else gt_bbox.shape[0]
        keep_mask = np.zeros((num_gts, ), dtype=np.int32)
        if num_gts > 0:
            if valid_ids is None:
                keep_mask[:] = 1
            else:
                keep_mask[np.asarray(valid_ids, dtype=np.int64)] = 1
        sample['gt_bbox_keep_mask'] = keep_mask

    def apply(self, sample, context=None):
        if random.random() > self.prob:
            self._set_bbox_keep_mask(sample)
            return sample

        if 'gt_bbox_vis' not in sample:
            # only used in semi-det as unsup data
            sample = self.set_fake_bboxes(sample)
            sample = self.random_crop(sample, fake_bboxes=True)
            self._set_bbox_keep_mask(sample, num_gts=0)
            return sample
    ...
    def random_crop(self, sample, fake_bboxes=False):
        h, w = sample['vis_image'].shape[:2]
        gt_bbox_vis = sample['gt_bbox_vis']
        gt_bbox_ir = sample['gt_bbox_ir']
        num_gts = gt_bbox_vis.shape[0]
        ...
        for thresh in thresholds:
            if thresh == 'no_crop':
                if not fake_bboxes:
                    self._set_bbox_keep_mask(sample, num_gts)
                return sample
            ...
            for i in range(self.num_attempts):
                ...
                crop_box = [crop_x, crop_y, crop_x + crop_w, crop_y + crop_h]
                if self.ioumode == "iof":
                    iou_vis = self._gtcropiou_matrix(
                        gt_bbox_vis, np.array(
                            [crop_box], dtype=np.float32))
                    iou_ir = self._gtcropiou_matrix(
                        gt_bbox_ir, np.array(
                            [crop_box], dtype=np.float32))
                elif self.ioumode == "iou":
                    iou_vis = self._iou_matrix(
                        gt_bbox_vis, np.array(
                            [crop_box], dtype=np.float32))
                    iou_ir = self._iou_matrix(
                        gt_bbox_ir, np.array(
                            [crop_box], dtype=np.float32))
                if iou_vis.max() < thresh and iou_ir.max() < thresh :
                    continue

                if self.cover_all_box and iou_vis.min() < thresh and iou_ir.min() < thresh:
                    continue

                cropped_box_vis, valid_ids_vis = self._crop_box_with_center_constraint(
                    gt_bbox_vis, np.array(
                        crop_box, dtype=np.float32))
                cropped_box_ir, valid_ids_ir = self._crop_box_with_center_constraint(
                    gt_bbox_ir, np.array(
                        crop_box, dtype=np.float32))
                if valid_ids_vis.size > 0 and valid_ids_ir.size > 0:
                    if valid_ids_vis.size <= valid_ids_ir.size:
                        valid_ids = valid_ids_vis
                    else:
                        valid_ids = valid_ids_ir
                    found = True
                    break
            ...
            if found:
                ...
                sample['vis_image'] = self._crop_image(sample['vis_image'], crop_box)
                sample['ir_image'] = self._crop_image(sample['ir_image'], crop_box)
                PairedTransformHelper.translate(
                    sample, -crop_x, -crop_y, new_size=(crop_w, crop_h))
                if fake_bboxes == True:
                    return sample

                self._set_bbox_keep_mask(sample, num_gts, valid_ids)
                sample['gt_bbox_vis'] = np.take(cropped_box_vis, valid_ids, axis=0)
                sample['gt_bbox_ir'] = np.take(cropped_box_ir, valid_ids, axis=0)
                sample['gt_class'] = np.take(
                    sample['gt_class'], valid_ids, axis=0)
                if 'gt_score' in sample:
                    sample['gt_score'] = np.take(
...
@register_op
class Multi_NormalizeBox_Paired(BaseOperator):
    """Transform the bounding box's coornidates to [0,1]."""

    def __init__(self):
        super(Multi_NormalizeBox_Paired, self).__init__()
    ...
    def apply(self, sample, context):
        ...
        gt_bbox_ir = sample['gt_bbox_ir']
        height, width, _ = vis_im.shape
        for i in range(gt_bbox_vis.shape[0]):
            gt_bbox_vis[i][0] = gt_bbox_vis[i][0] / width
            gt_bbox_vis[i][1] = gt_bbox_vis[i][1] / height
            gt_bbox_vis[i][2] = gt_bbox_vis[i][2] / width
            gt_bbox_vis[i][3] = gt_bbox_vis[i][3] / height

            gt_bbox_ir[i][0] = gt_bbox_ir[i][0] / width
            gt_bbox_ir[i][1] = gt_bbox_ir[i][1] / height
            gt_bbox_ir[i][2] = gt_bbox_ir[i][2] / width
            gt_bbox_ir[i][3] = gt_bbox_ir[i][3] / height
        sample['gt_bbox_vis'] = gt_bbox_vis
        sample['gt_bbox_ir'] = gt_bbox_ir
...
@register_op
class BboxXYXY2XYWH_Paired(BaseOperator):
    """
    Convert bbox XYXY format to XYWH format.
    """

    def __init__(self):
        super(BboxXYXY2XYWH_Paired, self).__init__()

    def apply(self, sample, context=None):
        assert 'gt_bbox_vis' in sample
        bbox_vis = sample['gt_bbox_vis']
        bbox_vis[:, 2:4] = bbox_vis[:, 2:4] - bbox_vis[:, :2]
        bbox_vis[:, :2] = bbox_vis[:, :2] + bbox_vis[:, 2:4] / 2.
        sample['gt_bbox_vis'] = bbox_vis

        bbox_ir = sample['gt_bbox_ir']
        bbox_ir[:, 2:4] = bbox_ir[:, 2:4] - bbox_ir[:, :2]
        bbox_ir[:, :2] = bbox_ir[:, :2] + bbox_ir[:, 2:4] / 2.
        sample['gt_bbox_ir'] = bbox_ir
        return sample
...
class Multi_Pad_IRVIS(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        PairedTransformHelper.translate(sample, offset_x, offset_y, new_size=(w, h))
        if self.pad_mode == 0:
            return sample
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
            sample['gt_bbox'] = self.apply_bbox(sample['gt_bbox'], offsets)
...
@register_op
class Multi_Pad_IRVIS_Paired(BaseOperator):
    def __init__(self,
                 size=None,
                 size_divisor=32,
                 pad_mode=0,
        ...
        Args:
            ...
            fill_value (bool): rgb value of pad area, default (127.5, 127.5, 127.5)
        """
        super(Multi_Pad_IRVIS_Paired, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        if self.pad_mode == 0:
            return sample
        if 'gt_bbox_vis' in sample and len(sample['gt_bbox_vis']) > 0:
            sample['gt_bbox_vis'] = self.apply_bbox(sample['gt_bbox_vis'], offsets)
        if 'gt_bbox_ir' in sample and len(sample['gt_bbox_ir']) > 0:
            sample['gt_bbox_ir'] = self.apply_bbox(sample['gt_bbox_ir'], offsets)
...
@register_op
class RandomColorJitter_DAOD(BaseOperator):
    def __init__(self,
                 prob=0.8,
                 brightness=0.4,
                 contrast=0.4,
                 saturation=0.4,
                 hue=0.1):
        super(RandomColorJitter_DAOD, self).__init__()
        self.prob = prob
        self.brightness = brightness
        self.contrast = contrast
        self.saturation = saturation
    ...
    def apply(self, sample, context=None):
        sample['vis_image_aug'] = copy.deepcopy(sample['vis_image'])
        sample['ir_image_aug'] = copy.deepcopy(sample['ir_image'])
        if np.random.uniform(0, 1) < self.prob:
            from paddle.vision.transforms import ColorJitter
            transform = ColorJitter(self.brightness, self.contrast,
                                    self.saturation, self.hue)
            sample['vis_image_aug'] = transform(sample['vis_image_aug'].astype(np.uint8))
            sample['vis_image_aug'] = sample['vis_image_aug'].astype(np.float32)

            sample['ir_image_aug'] = transform(sample['ir_image_aug'].astype(np.uint8))
            sample['ir_image_aug'] = sample['ir_image_aug'].astype(np.float32)
        return sample
...
@register_op
class RandomGrayscale_DAOD(BaseOperator):
    def __init__(self, prob=0.2):
        super(RandomGrayscale_DAOD, self).__init__()
        self.prob = prob
    ...
    def apply(self, sample, context=None):
        if np.random.uniform(0, 1) < self.prob:
            from paddle.vision.transforms import Grayscale
            transform = Grayscale(num_output_channels=3)
            sample['vis_image_aug'] = transform(sample['vis_image_aug'].astype(np.float32))
            sample['ir_image_aug'] = transform(sample['ir_image_aug'].astype(np.float32))
        return sample
...
@register_op
class RandomGaussianBlur_DAOD(BaseOperator):
    def __init__(self, prob=0.5, sigma=[0.1, 2.0]):
        super(RandomGaussianBlur_DAOD, self).__init__()
        self.prob = prob
        self.sigma = sigma
    ...
    def apply(self, sample, context=None):
        if np.random.uniform(0, 1) < self.prob:
            sigma = np.random.uniform(self.sigma[0], self.sigma[1])
            im_vis_aug = cv2.GaussianBlur(sample['vis_image_aug'], (23, 23), sigma)
            sample['vis_image_aug'] = im_vis_aug

            im_ir_aug = cv2.GaussianBlur(sample['ir_image_aug'], (23, 23), sigma)
            sample['ir_image_aug'] = im_ir_aug
        return sample
...
@register_op
class RandomErasing_DAOD(BaseOperator):
    def __init__(self,
                 prob=0.5,
                 scale=(0.02, 0.33),
                 ratio=(0.3, 3.3),
                 value=0,
                 inplace=False):
        super(RandomErasing_DAOD, self).__init__()
        assert isinstance(scale,
                          (tuple, list)), "scale should be a tuple or list"
        assert (scale[0] >= 0 and scale[1] <= 1 and scale[0] <= scale[1]
                ), "scale should be of kind (min, max) and in range [0, 1]"
    ...
    def _get_param(self, img_vis, img_ir, scale, ratio, value):
        shape = np.asarray(img_vis).astype(np.uint8).shape
        h, w, c = shape[-3], shape[-2], shape[-1]
        img_area = h * w
        log_ratio = np.log(ratio)
        for _ in range(1):
            ...
            top = np.random.randint(0, h - erase_h + 1)
            left = np.random.randint(0, w - erase_w + 1)
            return top, left, erase_h, erase_w, v, v
        return 0, 0, h, w, img_vis,img_ir
    ...
    def apply(self, sample, context=None):
        if random.random() < self.prob:
            ...
            im_vis_aug = sample['vis_image_aug']
            im_ir_aug = sample['ir_image_aug']

            top, left, erase_h, erase_w, v_vis, v_ir = self._get_param(im_vis_aug,im_ir_aug, self.scale,
                                                             self.ratio, value)

            im_vis_aug = self._erase(im_vis_aug, top, left, erase_h, erase_w, v_vis, self.inplace)
            sample['vis_image_aug'] = im_vis_aug


            im_ir_aug = self._erase(im_ir_aug, top, left, erase_h, erase_w, v_ir, self.inplace)
            sample['ir_image_aug'] = im_ir_aug
        return sample
...
# @register_op
# class RandomErasingCrop_DAOD(BaseOperator):
#     def __init__(self):
#         super(RandomErasingCrop_DAOD, self).__init__()
#         self.transform1 = RandomErasing_DAOD(
#             prob=0.7, scale=(0.05, 0.2), ratio=(0.3, 3.3), value="random")
#         self.transform2 = RandomErasing_DAOD(
#             prob=0.5, scale=(0.05, 0.2), ratio=(0.1, 6), value="random")
#         self.transform3 = RandomErasing_DAOD(
#             prob=0.3, scale=(0.05, 0.2), ratio=(0.05, 8), value="random")
...
class RandomErasingCrop_DAOD(BaseOperator):
    def __init__(self):
        super(RandomErasingCrop_DAOD, self).__init__()
        # 只保留一个标准的 transform
        self.transform = RandomErasing_DAOD(
            prob=0.5, scale=(0.05, 0.2), ratio=(0.1, 6), value="random")
        
        # (移除了 transform1 和 transform3)
...
#         # 将所有 transform 放入一个列表中
#         self.all_transforms = [self.transform1, self.transform2, self.transform3]

#     def apply(self, sample, context=None):
#         # 每次调用时，从列表中随机选择一个 transform
#         transform_to_apply = random.choice(self.all_transforms)
        
#         # 应用被选中的那一个 transform
#         # 注意：这里的概率 (0.7, 0.5, 0.3) 是 "被选中后" 再应用的概率
#         sample = transform_to_apply(sample)

...
import math
import numpy as np
from .operators import register_op, BaseOperator, Resize, Multi_Resize, Multi_Resize_Paired
...
__all__ = [
    ...
    'Multi_BatchRandomResize_Paired',
    'Gt2YoloTarget',
    'Gt2FCOSTarget',
    'Gt2TTFTarget',
    'Gt2Solov2Target',
...
@register_op
class Multi_PadBatch(BaseOperator):
    """
    Pad a batch of samples so they can be divisible by a stride.
    The layout of each image should be 'CHW'.
    Args:
    ...
    def __init__(self, pad_to_stride=0):
        super(Multi_PadBatch, self).__init__()
        self.pad_to_stride = pad_to_stride
    ...
    def __call__(self, samples, context=None):
        ...
        shapes = []
        for data in inner_samples:
            shapes.append(data['vis_image'].shape)
            shapes.append(data['ir_image'].shape)
        ...
        for data in inner_samples:
            vis_im = data['vis_image']
            ir_im = data['ir_image']
            vis_c, vis_h, vis_w = vis_im.shape[:]
            ir_c, ir_h, ir_w = ir_im.shape[:]
            padding_im_vis = np.zeros(
                (vis_c, max_shape[1], max_shape[2]), dtype=np.float32)
            padding_im_ir = np.zeros(
                (ir_c, max_shape[1], max_shape[2]), dtype=np.float32)
            padding_im_vis[:, :vis_h, :vis_w] = vis_im
            padding_im_ir[:, :ir_h, :ir_w] = ir_im
            data['vis_image'] = padding_im_vis
            data['ir_image'] = padding_im_ir
            if 'semantic' in data and data['semantic'] is not None:
                semantic = data['semantic']
                sem_c, sem_h, sem_w = semantic.shape[:]
                padding_sem = np.zeros(
                    (sem_c, max_shape[1], max_shape[2]), dtype=np.float32)
                padding_sem[:, :sem_h, :sem_w] = semantic
                data['semantic'] = padding_sem
            if 'gt_segm' in data and data['gt_segm'] is not None:
                gt_segm = data['gt_segm']
                segm_c, segm_h, segm_w = gt_segm.shape[:]
                padding_segm = np.zeros(
                    (segm_c, max_shape[1], max_shape[2]),
                    dtype=np.uint8)
                padding_segm[:, :segm_h, :segm_w] = gt_segm
                data['gt_segm'] = padding_segm
...
@register_op
class Multi_PadBatch_DAOD(BaseOperator):
    """
    Pad a batch of samples so they can be divisible by a stride.
    The layout of each image should be 'CHW'.
    Args:
    ...
    def __init__(self, pad_to_stride=0):
        super(Multi_PadBatch_DAOD, self).__init__()
        self.pad_to_stride = pad_to_stride
    ...
    def __call__(self, samples, context=None):
        ...
        shapes = []
        for data in inner_samples:
            for k in ('vis_image', 'ir_image', 'vis_image_aug', 'ir_image_aug'):
                if k in data and data[k] is not None:
                    shapes.append(data[k].shape)
        max_shape = np.array(shapes).max(axis=0)
        if coarsest_stride > 0:
            max_shape[1] = int(
                np.ceil(max_shape[1] / coarsest_stride) * coarsest_stride)
        ...
        for data in inner_samples:
            vis_im = data['vis_image']
            ir_im = data['ir_image']
            # ir_im_flip = data['ir_image_flip']

            vis_im_aug = data['vis_image_aug']
            ir_im_aug = data['ir_image_aug']
            # ir_im_aug_flip = data['ir_image_aug_flip']
            vis_c, vis_h, vis_w = vis_im.shape[:]
            ir_c, ir_h, ir_w = ir_im.shape[:]
            vis_aug_c, vis_aug_h, vis_aug_w = vis_im_aug.shape[:]
            ir_aug_c, ir_aug_h, ir_aug_w = ir_im_aug.shape[:]
            padding_im_vis = np.zeros(
                (vis_c, max_shape[1], max_shape[2]), dtype=np.float32)
            padding_im_ir = np.zeros(
                (ir_c, max_shape[1], max_shape[2]), dtype=np.float32)
            ...
            padding_im_vis_aug = np.zeros(
                (vis_aug_c, max_shape[1], max_shape[2]), dtype=np.float32)
            padding_im_ir_aug = np.zeros(
                (ir_aug_c, max_shape[1], max_shape[2]), dtype=np.float32)
            padding_im_vis_aug[:, :vis_aug_h, :vis_aug_w] = vis_im_aug
            padding_im_ir_aug[:, :ir_aug_h, :ir_aug_w] = ir_im_aug

            # padding_im_ir_flip = np.zeros(
            #     (im_c, max_shape[1], max_shape[2]), dtype=np.float32)
            # padding_im_ir_aug_flip = np.zeros(
            #     (im_c, max_shape[1], max_shape[2]), dtype=np.float32)
            # padding_im_ir_flip[:, :im_h, :im_w] = ir_im_flip
            # padding_im_ir_aug_flip[:, :im_h, :im_w] = ir_im_aug_flip

            data['vis_image'] = padding_im_vis
            data['ir_image'] = padding_im_ir
            data['vis_image_aug'] = padding_im_vis_aug
            data['ir_image_aug'] = padding_im_ir_aug
            # data['ir_image_flip'] = padding_im_ir_flip
            # data['ir_image_aug_flip'] = padding_im_ir_aug_flip
            if 'semantic' in data and data['semantic'] is not None:
                semantic = data['semantic']
                sem_c, sem_h, sem_w = semantic.shape[:]
                padding_sem = np.zeros(
...
@register_op
class Multi_BatchRandomResize_Paired(BaseOperator):
    """
    Resize image to target size randomly. random target_size and interpolation method
    Args:
        target_size (int, list, tuple): image target size, if random size is True, must be list or tuple
    ...
    def __init__(self,
                 ...
                 random_size=True,
                 random_interp=False):
        super(Multi_BatchRandomResize_Paired, self).__init__()
        self.keep_ratio = keep_ratio
        self.interps = [
            cv2.INTER_NEAREST,
            cv2.INTER_LINEAR,
    ...
    def __call__(self, samples, context=None):
        ...
        resizer = Multi_Resize_Paired(target_size, keep_ratio=self.keep_ratio, interp=interp)
        return resizer(samples, context=context)
...
@register_op
class PadRGT_Paired(BaseOperator):
    """
    Pad 0 to `gt_class`, `gt_bbox`, `gt_score`...
    The num_max_boxes is the largest for batch.
    Args:
    ...
    def __init__(self, return_gt_mask=True):
        super(PadRGT_Paired, self).__init__()
        self.return_gt_mask = return_gt_mask
    ...
    def __call__(self, samples, context=None):
        num_max_boxes = max([len(s['gt_bbox_vis']) for s in samples])
        for sample in samples:
            if self.return_gt_mask:
                sample['pad_gt_mask'] = np.zeros(
                    (num_max_boxes, 1), dtype=np.float32)
            ...
            num_gt = len(sample['gt_bbox_vis'])
            pad_gt_class = np.zeros((num_max_boxes, 1), dtype=np.int32)
            pad_gt_bbox_vis = np.zeros((num_max_boxes, 4), dtype=np.float32)
            pad_gt_bbox_ir = np.zeros((num_max_boxes, 4), dtype=np.float32)
            if num_gt > 0:
                pad_gt_class[:num_gt] = sample['gt_class']
                pad_gt_bbox_vis[:num_gt] = sample['gt_bbox_vis']
                pad_gt_bbox_ir[:num_gt] = sample['gt_bbox_ir']
            sample['gt_class'] = pad_gt_class
            sample['gt_bbox_vis'] = pad_gt_bbox_vis
            sample['gt_bbox_ir'] = pad_gt_bbox_ir
            # pad_gt_mask
            if 'pad_gt_mask' in sample:
                sample['pad_gt_mask'][:num_gt] = 1
            # gt_score
            names = ['gt_score', 'is_crowd', 'difficult', 'gt_poly_vis','gt_poly_ir', 'gt_rbox_vis', 'gt_rbox_ir']
            dims = [1, 1, 1, 8,8,5, 5]
            dtypes = [np.float32, np.int32, np.int32, np.float32, np.float32,np.float32, np.float32]
...
@register_op
class PadRGT_Paired_DAOD(BaseOperator):
    """
    Pad 0 to `gt_class`, `gt_bbox`, `gt_score`...
    The num_max_boxes is the largest for batch.
    Args:
    ...
    def __init__(self, return_gt_mask=True):
        super(PadRGT_Paired_DAOD, self).__init__()
        self.return_gt_mask = return_gt_mask

...
class Poly2RBox(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        return sample
@register_op
class BBox2RBox(BaseOperator):
    """ BBox to Rotated Box, using new OpenCV definition since 4.5.1
    Args:
    """
    def __init__(self):
        """ Transform the muiti_image data to numpy format following the rgb format
        """
        super(BBox2RBox, self).__init__()
    def apply(self, sample, context=None):    
    # 输入: boxes shape [B, N, 4]  (xmin, ymin, xmax, ymax)
        if 'gt_bbox_ir' in sample:
            boxes=sample['gt_bbox_ir']
            xmin, ymin, xmax, ymax = boxes[:, 0], boxes[:, 1], boxes[:, 2], boxes[:, 3]
            x_center = (xmin + xmax) / 2
            y_center = (ymin + ymax) / 2
            width = xmax - xmin
            height = ymax - ymin
            angle = np.zeros_like(x_center)  # 角度都是0

            # 最后拼接回到shape [B, N, 5]
            rbox = np.stack([x_center, y_center, width, height, angle], axis=-1)
            sample['gt_rbox_ir']=rbox
        if 'gt_bbox_vis' in sample:
            boxes=sample['gt_bbox_vis']
            xmin, ymin, xmax, ymax = boxes[:, 0], boxes[:, 1], boxes[:, 2], boxes[:, 3]
            x_center = (xmin + xmax) / 2
            y_center = (ymin + ymax) / 2
            width = xmax - xmin
...
class Multi_RandomFlip_Paired(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            PairedTransformHelper.flip_horizontal(sample)
...
class Rbox2Poly(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        sample['gt_bbox'] = np.stack([xmin, ymin, xmax, ymax], axis=1)
        return sample

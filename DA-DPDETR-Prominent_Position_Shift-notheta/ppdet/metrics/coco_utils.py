...
def get_infer_results(outs, catid, bias=0):
    ...
    if 'bbox_vis' in outs:
        if len(outs['bbox_vis']) > 0 and len(outs['bbox_vis'][0]) > 6:
            infer_res['bbox_vis'] = get_det_poly_res(
                outs['bbox_vis'], outs['bbox_num'], im_id, catid, bias=bias)
        else:
            infer_res['bbox_vis'] = get_det_res(
                outs['bbox_vis'], outs['bbox_num'], im_id, catid, bias=bias)

    if 'bbox_ir' in outs:
        if len(outs['bbox_ir']) > 0 and len(outs['bbox_ir'][0]) > 6:
            infer_res['bbox_ir'] = get_det_poly_res(
                outs['bbox_ir'], outs['bbox_num'], im_id, catid, bias=bias)
        else:
            infer_res['bbox_ir'] = get_det_res(
                outs['bbox_ir'], outs['bbox_num'], im_id, catid, bias=bias)

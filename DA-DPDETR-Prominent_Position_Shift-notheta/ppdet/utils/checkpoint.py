...
def _strip_postfix(path):
    ...
    assert ext in ['', '.pdparams', '.pdopt', '.pdmodel', '.pdema'], \
            "Unknown postfix {} from weights".format(ext)
    return path, ext


def load_weight(model, weight, optimizer=None, ema=None, exchange=False):
    if is_url(weight):
        weight = get_weights_path(weight)

    path = _strip_postfix(weight)[0]
    ...
    if ema is not None and os.path.exists(path + '.pdema'):
        if exchange:
            ...
            # Extract ema_step from .pdema (which is now in param_state_dict)
            if 'ema_step' in param_state_dict:
                ema_step = param_state_dict.pop('ema_step')
            logger.info('Loading model weights from {}'.format(path + '.pdema'))
        else:
            ema_state_dict = paddle.load(path + '.pdema')
            # Extract ema_step from .pdema (which is in ema_state_dict)
            if 'ema_step' in ema_state_dict:
                ema_step = ema_state_dict.pop('ema_step')
            logger.info('Loading ema_model weights from {}'.format(path +
                                                                   '.pdema'))
    ...
    if optimizer is not None and os.path.exists(path + '.pdopt'):
        ...
        if ema_state_dict is not None:
            step_to_load = ema_step if ema_step is not None else 0
            ema.resume(ema_state_dict, step_to_load)
...
def multi_match_state_dict(model_state_dict, weight_state_dict, mode='default'):
    ...
    def multi_match(a, b):
        ...
        if b.startswith('neck.'):
            b = b[5:]
            # if 'input_proj' in b:
            #     b = b[:11] + str(int(b[11])+1) + b[12:]

        if b.startswith('transformer.decoder.layers.') and ('attention_weights' in b):
            if a.startswith('transformer.decoder.layers.') and ('attention_weights' in a):
                if '_ir' in a:
                    a = a.replace('_ir', '')
                elif '_vis' in a:
                    a = a.replace('_vis', '')
        ...
        if b.startswith('transformer.decoder.layers.') and ('sampling_offsets' in b):
            if a.startswith('transformer.decoder.layers.') and ('sampling_offsets' in a):
                if '_ir' in a:
                    a = a.replace('_ir', '')
                elif '_vis' in a:
                    a = a.replace('_vis', '')
        ...
        if b.startswith('transformer.dec_bbox_head.'):
            if a.startswith('transformer.dec_bbox_head_vis') or a.startswith('transformer.dec_bbox_head_ir'):
                if '_ir' in a:
                    a = a.replace('_ir', '')
                elif '_vis' in a:
                    a = a.replace('_vis', '')
...
def multi_match_state_dict_DAOD(model_state_dict, weight_state_dict, mode='default'):
    """
    Match between the model state dict and pretrained weight state dict.
    Return the matched state dict.
    ...
    def multi_match(a, b):
        #if b.startswith('backbone.res5'):
        if b.startswith('transformer.enc_'):
            if 'transformer_single.enc_' in a:
                a = a.replace('_single', '')
                if '_ir.' in a:
                    a = a.replace('_ir','')
                elif '_vis.' in a:
                    ...
                    a = a.replace('_fvisir', '')
            elif 'transformer_DPDETR.enc_' in a:
                a = a.replace('_DPDETR', '')
                if '_ir.' in a:
                    a = a.replace('_ir','')
                elif '_vis.' in a:
        ...
        if b.startswith('transformer.input_proj'):
            #b = b.split('.')[2] +'.'+b.split('.')[3] +'.'+b.split('.')[4]
            if a.startswith('transformer_single.input_proj'):
                iid = b.split('.')[2]
                b =  b.split('.')[3] + '.' + b.split('.')[4]
                return (a.endswith('.' + b) and a.startswith('transformer_single.input_proj') and ((iid in a) or (str(int(iid)+3) in a) or (str(int(iid)+6) in a)))
            elif a.startswith('transformer_DPDETR.input_proj'):
                iid = b.split('.')[2]
                b = b.split('.')[3] + '.' + b.split('.')[4]
                return (a.endswith('.' + b) and a.startswith('transformer_DPDETR.input_proj') and (
                            (iid in a) or (str(int(iid) + 3) in a) or (str(int(iid) + 6) in a)))

        # if b.startswith('transformer.dec_'):
        #     if a.startswith('transformer_single.dec_'):
        #         a = a.replace('_single','')
        #     elif a.startswith('transformer_DPDETR.dec_'):
        #         a = a.replace('_DPDETR', '')
        #         if '_ir' in a:
        #             a = a.replace('_ir','')
        #         if '_vis' in a:
        ...
        # if b.startswith('transformer.decoder.layers'):
        #     if a.startswith('transformer_single.decoder.layers'):
        #         a = a.replace('_single', '')
        #     elif a.startswith('transformer_DPDETR.decoder.layers'):
        #         a = a.replace('_DPDETR', '')
        #         if '_visp' in a:
        #             a = a.replace('_visp', '')
        #         if '_irp' in a:
        ...
        if b.startswith('transformer.decoder.layers'):
            iid = b.split('.')[3]
            if 'transformer_single.decoder.layers' in a and ('self_attn' in a or 'norm1' in a):
                a = a.replace('_single', '')
                iid_a = a.split('.')[3]
                if 'self_attn' in b:
                    bb = '.'.join(b.split('.')[4:])
                    return (iid == iid_a and a.endswith('.' + bb))
            if 'transformer_DPDETR.decoder.layers' in a and ('self_attn' in a or 'norm1' in a):
                a = a.replace('_DPDETR', '')
                iid_a = a.split('.')[3]
                if 'self_attn' in b:
                    bb = '.'.join(b.split('.')[4:])
        ...
        if b.startswith('transformer.query_pos_head'):
            if a.startswith('transformer_single.query_pos_head'):
                a = a.replace('_single', '')
            elif a.startswith('transformer_DPDETR.query_pos_head'):
                a = a.replace('_DPDETR', '')
        if b.startswith('transformer.denoising_class_embed'):
            if a.startswith('transformer_single.denoising_class_embed'):
                a = a.replace('_single', '')
            elif a.startswith('transformer_DPDETR.denoising_class_embed'):
                a = a.replace('_DPDETR', '')
        ...
        #
        # if b.startswith('transformer.decoder.layers.') and ('attention_weights' in b):
        #     if a.startswith('transformer_single.decoder.layers.') and ('attention_weights' in a):
        #     #if a.startswith('transformer_single.decoder.layers.'):
        #         if '_single' in a:
        #             a = a.replace('_single', '')
        #         elif '_DPDETR' in a:
        #             a = a.replace('_DPDETR', '')
        #     elif a.startswith('transformer_DPDETR.decoder.layers.') and ('attention_weights' in a):
        #     #elif a.startswith('transformer_DPDETR.decoder.layers.'):
        #         if '_single' in a:
        #             a = a.replace('_single', '')
        #         elif '_DPDETR' in a:
        ...
        # if b.startswith('transformer.decoder.layers.') and ('sampling_offsets' in b):
        # #if b.startswith('transformer.decoder.layers.'):
        #     if a.startswith('transformer_single.decoder.layers.') and ('sampling_offsets' in a):
        #     #if a.startswith('transformer_single.decoder.layers.'):
        #         if '_single' in a:
        #             a = a.replace('_single', '')
        #         elif '_DPDETR' in a:
        #             a = a.replace('_DPDETR', '')
        #     elif a.startswith('transformer_DPDETR.decoder.layers.') and ('sampling_offsets' in a):
        #     #elif a.startswith('transformer_DPDETR.decoder.layers.'):
        #         if '_single' in a:
        #             a = a.replace('_single', '')
        #         elif '_DPDETR' in a:
        ...
        #
        # if b.startswith('transformer.dec_bbox_head.'):
        #     if a.startswith('transformer_single.dec_bbox_head'):
        #         if '_single' in a:
        #             a = a.replace('_single', '')
        #         elif '_DPDETR' in a:
        #             a = a.replace('_DPDETR', '')
        #     elif a.startswith('transformer_DPDETR.dec_bbox_head_vis') or a.startswith('transformer_DPDETR.dec_bbox_head_ir'):
        #         if '_single' in a:
        #             a = a.replace('_single', '')
        #         elif '_DPDETR' in a:
        #             a = a.replace('_DPDETR', '')
        #
        #     if b.startswith('transformer.dec_angle_head.'):
        #         if a.startswith('transformer_single.dec_angle_head'):
        #             if '_single' in a:
        #                 a = a.replace('_single', '')
        #             elif '_DPDETR' in a:
        #                 a = a.replace('_DPDETR', '')
        #         elif a.startswith('transformer_DPDETR.dec_angle_head_vis') or a.startswith(
        #                 'transformer_DPDETR.dec_angle_head_ir'):
        #             if '_single' in a:
        #                 a = a.replace('_single', '')
        #             elif '_DPDETR' in a:
        #                 a = a.replace('_DPDETR', '')
        #
        if b.startswith('transformer.dec_score_head.'):
            if a.startswith('transformer_single.dec_score_head'):
                if '_single' in a:
                    a = a.replace('_single', '')
                elif '_DPDETR' in a:
                    a = a.replace('_DPDETR', '')
            elif a.startswith('transformer_DPDETR.dec_score_head'):
                if '_single' in a:
                    a = a.replace('_single', '')
                elif '_DPDETR' in a:
                    a = a.replace('_DPDETR', '')
...
def load_pretrain_weight(model, pretrain_weight, ARSL_eval=False, mode ='default'):
    ...
    path, ext = _strip_postfix(pretrain_weight)
    ...
    if not ARSL_eval:
        ...
        for k in list(param_state_dict.keys()):
            if k.startswith('ema_step') or k == 'ema_step':
                param_state_dict.pop(k)
        for k, v in param_state_dict.items():
            new_k = k
            if k.startswith('backbone_ir.'):
                new_k = k.replace('backbone_ir.', 'backbone_stream1.')
            elif k.startswith('backbone_vis.'):
                new_k = k.replace('backbone_vis.', 'backbone_stream2.')
            # elif k.startswith('transformer.'):
            #     new_k = k.replace('transformer.', 'transformer_DPDETR.')
            elif k.startswith('neck_ir.'):
                new_k = k.replace('neck_ir.', 'neck_stream1.')
            elif k.startswith('neck_vis.'):
                new_k = k.replace('neck_vis.', 'neck_stream2.')
            
            if new_k != k:
                modify_keys.append((k, new_k))
        original_stdout = sys.stdout
        with open('pretrain_weights_structure.txt', 'w') as f:
            sys.stdout = f
            #print(pre_weights)#会输出{'参数名': 参数值(tensor/numpy)}
            for key, value in sorted(param_state_dict.items()): #.items()用来解包字典
                if isinstance(value, paddle.Tensor): 
                    print(f'Key: {key}  Shape: {value.shape}')
                else:
                    print(f'Key: {key}  Shape: {value.shape} is not a Tensor')
        sys.stdout = original_stdout  
        # 批量修改
        for old_k, new_k in modify_keys:
            param_state_dict[new_k] = param_state_dict.pop(old_k)
        if mode == 'default':
            ...
            param_state_dict = multi_match_state_dict_DAOD(model_dict, param_state_dict, mode)
        elif mode == 'multi_2':

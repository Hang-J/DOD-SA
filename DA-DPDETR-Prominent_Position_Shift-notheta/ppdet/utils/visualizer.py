...
def visualize_results_paired(image,
                      bbox_res,
                      mask_res,
                      segm_res,
                      keypoint_res,
    ...
    """
    if bbox_res is not None:
        image = draw_bbox_paired(image, im_id, catid2name, bbox_res, threshold)
    if mask_res is not None:
        image = draw_mask(image, im_id, mask_res, threshold)
    if segm_res is not None:
        image = draw_segm(image, im_id, catid2name, segm_res, threshold)
...
def draw_bbox_paired(image, im_id, catid2name, bboxes, threshold):
    """
    Draw bbox on image
    """
    draw = ImageDraw.Draw(image)
    ...
    for dt in np.array(bboxes):
        ...
        # draw label
        text = "{}|{} {:.2f}".format(count ,catid2name[catid], score)
        count+=1
        #tw, th = draw.textsize(text)

...
from ppdet.engine import Trainer, init_parallel_env, Trainer_DAOD_no_flip_supression
...
def run(FLAGS, cfg):
    ...
    # build trainer
    trainer = Trainer_DAOD_no_flip_supression(cfg, mode='eval')
    #load weights
    #state_dict = paddle.load(cfg.weights)
    # 2. 遍历打印
    # print(f"预训练权重如下：")
    # for k, v in state_dict.items():
    #     if k.startswith('backbone_ir.'):
    #         k=k.replace('backbone_ir.', 'backbone_stream1.')
    #     elif k.startswith('backbone_vis.'):
    #         k= k.replace('backbone_vis.', 'backbone_stream2.')
    #     elif k.startswith('transformer.'):
    #         k= k.replace('transformer.', 'transformer_DPDETR.')
    #     elif k.startswith('neck_ir.'):
    #         k=k.replace('neck_ir.', 'neck_stream1.')
    #     elif k.startswith('neck_vis.'):
    #         k=k.replace('neck_vis.', 'neck_stream2.')
    #     if hasattr(v, 'shape'):
    #         print(f"{k}: {v.shape}")
    #     else:
    #         print(f"{k}: Type {type(v)} (No shape)")

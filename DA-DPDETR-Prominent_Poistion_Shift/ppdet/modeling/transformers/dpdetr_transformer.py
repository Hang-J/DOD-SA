...
from .deformable_transformer import MSDeformableAttention,MSDeformableAttention_key_aware,MSDeformableAttention_Missing,MSDeformableAttention_Missing_3samplinglayer,MSDeformableAttention_split_attention, MSDeformableAttention_Position_split
...
from .utils import (_get_clones, get_sine_pos_embed,
                    get_contrastive_denoising_training_group,get_contrastive_denoising_training_group_paired,get_contrastive_denoising_training_group_rotated,get_contrastive_denoising_training_group_rotated_paired,  inverse_sigmoid)
import numpy as np
import cv2
__all__ = ['RTDETRTransformer_Rotate', 'Multi_RTDETRTransformer_Rotate',
           'Multi_RTDETRTransformer_Rotate_Paired_Decouple_atten',
           'Multi_RTDETRTransformer_Rotate_Paired']
...
class PPMSDeformableAttention_Rotate(MSDeformableAttention):
    def forward(self,
        ...
        # mask_vis = mask_vis[:,:,:2]
        # mask_vis = mask_vis.unsqueeze(2).unsqueeze(3).unsqueeze(4)
        # mask_vis = paddle.tile(mask_vis, [1, 1, self.num_heads, self.num_levels//2, self.num_points, 1])
        ...
        if reference_points.shape[-1] == 2:
            ...
            # angle = (
            #         angle /
            #         half_pi_bin).clip(0, angle_max - 0.01)

            # angle = (angle + math.pi / 4 - angle * 1e-10) % \
            #         (math.pi / 2) - math.pi / 4 + angle * 1e-10
            ...
            #
            # sampling_locations = reference_points[:, :, None, :, None, :2] + temp
            ...
            # sampling_locations = reference_points[:, :, None, :, None, :2] + paddle.fluid.layers.matmul(
            #     sampling_offsets / self.num_points * reference_points[:, :, None, :, None, 2:] * 0.5,
            #     rotate_matrix)
        ...
        #     #vis_imgs.append(cv2.imread(gt_meta['vis_im_file'][xx]))
        #     #ir_imgs.append(cv2.imread(gt_meta['ir_im_file'][xx]))
        #     ir_imgs.append(cv2.imread(gt_meta['im_file'][xx]))
        # h, w, _ = ir_imgs[0].shape
        # real_hw = [w, h]
        # real_hw = np.array(real_hw)
        # real_location_point = np.array(sampling_locations) * real_hw
        # real_weights = np.array(attention_weights)
        ...
        #                             elif zz == 2:
        #                                 CColor = red
        #                             cv2.circle(ir_imgs[xx], (round(real_location_point[xx][ii][mm][zz][pp][0]),
        #                                                       round(real_location_point[xx][ii][mm][zz][pp][1])),
        #                                        radius - 2, CColor, -1)
        ...
        #     # heatmap_image2 = cv2.applyColorMap(ir_imgs[ii],cv2.COLORMAP_JET)
        #     # cv2.imwrite(
        #     #     '/data1/guojunjie1/python_project/MS-PaddleDetection-develop-240117/output/rtdert_r_drone_vechal(no_pretrain)/sampling_locations/' + 'layer' + str(
        #     #         gt_meta['layer'] + 1) + '/' +
        #     #     gt_meta['vis_im_file'][ii].split('/')[-1].split('.')[0] + '_vis.png', vis_imgs[ii])
        #     # cv2.imwrite(
        #     #     '/data1/guojunjie1/python_project/MS-PaddleDetection-develop-240117/output/rtdert_r_drone_vechal(no_pretrain)/sampling_locations/' + 'layer' + str(
        ...
        #     #     gt_meta['ir_im_file'][ii].split('/')[-1].split('.')[0] + '_ir.png', ir_imgs[ii])
        #     cv2.imwrite(
        #         '/data1/guojunjie1/python_project/MS-PaddleDetection-develop-240117/output/rtdetr_r_drone-vechal(no_pretrain)/sampling_locations/' + 'layer' + str(
        #             gt_meta['layer'] + 1) + '/' +
        #         gt_meta['im_file'][ii].split('/')[-1].split('.')[0] + '_ir.png', ir_imgs[ii])
...
class PPMSDeformableAttention_Rotate_Position_split(MSDeformableAttention_Position_split):
    def forward(self,
                gt_meta,
                query,
                query_vis_sampling,
        ...
        """
        if flag == 'cls':
            lo = 'sampling_location_cls_new'
        elif flag == 'visp':
            lo = 'sampling_location_visp_new'
        elif flag == 'irp':
            lo = 'sampling_location_irp_new'
        ...
        if reference_points_vis.shape[-1] == 2:
            offset_normalizer = paddle.to_tensor(value_spatial_shapes)
            offset_normalizer = offset_normalizer.flip([1]).reshape(
                [1, 1, 1, self.num_levels, 1, 2])
            sampling_locations = reference_points.reshape([
        ...
        elif reference_points_vis.shape[-1] == 4:
            # 前两个是xy 后两个是wh
            # 初始化时offset是在 -n_points ~ n_points 范围之间 这里除以self.n_points是相当于把offset归一化到 0~1
            # 然后再乘以宽高的一半 再加上参考点的中心坐标 这就相当于使得最后的采样点坐标总是位于proposal box内
            # 相当于对采样范围进行了约束 减少了搜索空间
        ...
        # ## sample location of different semantic level visulize decouple attn
        #
        # ##plot location point
        # bs = len(gt_meta['im_id'])
        # vis_imgs = []
        ...
        # low_blue = (230, 216, 173)
        # low_green = (152, 251, 152)
        # low_purple = (186,85eights=/data1/guojunjie1/python_project/MS-PaddleDetection-develop-240117/ResNet50_vd_ssld_v2_pretrained.pdparams --eval,211)
        # low_red = (128,128,240)
        # blue = (255,0,0)
        # green = (0,255,0)
        # purple = (128,0,128)
        ...
        #                 for zz in range(3):
        #                     for pp in range(4):
        #                         if real_weights[xx][ii][mm][zz][pp] > 0.2:
        #                             if zz == 0:
        #                                 CColor = blue
        #                             elif zz == 1:
        #                                 CColor = green
        ...
        #                         #                radius - 3, color_b, -1)
        #
        #                         if real_weights[xx][ii][mm][zz + 3][pp] > 0.2:
        #                             if zz == 0:
        #                                 CColor = blue
        #                             elif zz == 1:
        #                                 CColor = green
        ...
        #     # heatmap_image2 = cv2.applyColorMap(ir_imgs[ii],cv2.COLORMAP_JET)
        #     cv2.imwrite(
        #         '/data1/guojunjie1/python_project/MS-PaddleDetection-develop-240117/output/damsdet_r_paired_v1_drone_vechal/'
        #         +lo+'/' + 'layer' + str(
        #             gt_meta['layer'] + 1) + '/' +
        #         gt_meta['vis_im_file'][ii].split('/')[-1].split('.')[0] + '_vis.png', vis_imgs[ii])
        #     cv2.imwrite(
...
class TransformerDecoder_Rotate_Position_split_Paired_Decouple_atten(nn.Layer):
    def __init__(self, hidden_dim, decoder_layer, num_layers, angle_max, angle_proj, half_pi_bin,eval_idx=-1):
        super(TransformerDecoder_Rotate_Position_split_Paired_Decouple_atten, self).__init__()
        self.layers = _get_clones(decoder_layer, num_layers)
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers
        self.eval_idx = eval_idx if eval_idx >= 0 else num_layers + eval_idx
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            # # fix embedding
            # ref_points_detach_vis_embed = paddle.concat([ref_points_detach_vis,angle_vis], axis=-1)
            # ref_points_detach_ir_embed = paddle.concat([ref_points_detach_ir, angle_ir], axis=-1)
            # query_pos_embed_vis = query_pos_head(ref_points_detach_vis_embed)
            # query_pos_embed_ir = query_pos_head(ref_points_detach_ir_embed)
...
class TransformerDecoder_Rotate_Position_split_Paired(nn.Layer):
    def __init__(self, hidden_dim, decoder_layer, num_layers, angle_max, angle_proj, half_pi_bin,eval_idx=-1):
        super(TransformerDecoder_Rotate_Position_split_Paired, self).__init__()
        self.layers = _get_clones(decoder_layer, num_layers)
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers
        self.eval_idx = eval_idx if eval_idx >= 0 else num_layers + eval_idx
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            output = layer(gt_meta, output, ref_points_input_vis,ref_points_input_ir, angle_vis,angle_ir, memory,
                           memory_spatial_shapes, memory_level_start_index,self.angle_max, self.half_pi_bin,
                           attn_mask, memory_mask, query_pos_embed_vis,query_pos_embed_ir,topk_ind_mask,topk_score, mask_vis)

            gt_meta['layer'] = gt_meta['layer'] + 1

            inter_ref_bbox_vis = F.sigmoid(bbox_head_vis[i](output) + inverse_sigmoid(
                ref_points_detach_vis))
            angle_cls_vis = angle_head_vis[i](output)
            angle_vis = F.softmax(angle_cls_vis.reshape([b, l, 1, self.angle_max + 1
                                              ])).matmul(self.angle_proj)

            inter_ref_bbox_ir = F.sigmoid(bbox_head_ir[i](output) + inverse_sigmoid(
                ref_points_detach_ir))
            angle_cls_ir = angle_head_ir[i](output)
            angle_ir = F.softmax(angle_cls_ir.reshape([b, l, 1, self.angle_max + 1
                                                         ])).matmul(self.angle_proj)
            ...
            if self.training:
                ...
                if i == 0:
                    ...
                    dec_out_bboxes_vis.append(
                        F.sigmoid(bbox_head_vis[i](output) + inverse_sigmoid(
                            ref_points_vis)))
                    dec_out_bboxes_ir.append(
                        F.sigmoid(bbox_head_ir[i](output) + inverse_sigmoid(
                            ref_points_ir)))
            elif i == self.eval_idx:
                dec_out_logits.append(score_head[i](output))
                dec_out_bboxes_vis.append(inter_ref_bbox_vis)
...
class TransformerDecoderLayer_Rotate_Position_split_Decouple_atten(nn.Layer):
    def __init__(self,
                 d_model=256,
                 n_head=8,
                 dim_feedforward=1024,
                 ...
                 key_aware=False,
                 split_attention=False):
        super(TransformerDecoderLayer_Rotate_Position_split_Decouple_atten, self).__init__()
        ...
        self.cross_attn_cls = PPMSDeformableAttention_Rotate_Position_split(d_model, n_head, n_levels,
                                                  n_points, 1.0)
        self.cross_attn_visp = PPMSDeformableAttention_Rotate_Position_split(d_model, n_head, n_levels,
                                                                            n_points, 1.0)
        self.cross_attn_irp = PPMSDeformableAttention_Rotate_Position_split(d_model, n_head, n_levels,
                                                                            n_points, 1.0)
        self.dropout2 = nn.Dropout(dropout)
    ...
    def forward(self,
        ...
        if self.training and dn_meta:
            ...
            for i in range(dn_meta['dn_num_group']):
                ...
                # k_dn = paddle.concat([k_dn, k_cls_dn], axis=1)
                # k_dn = paddle.concat([k_dn, k_visp_dn], axis=1)
                # k_dn = paddle.concat([k_dn, k_irp_dn], axis=1)
            tgt_dn = tgt_dn[:, 1:,:,:]
            q_dn = q_dn[:, 1:, :, :]
...
class TransformerDecoderLayer_Rotate_Position_split(nn.Layer):
    def __init__(self,
                 d_model=256,
                 n_head=8,
                 dim_feedforward=1024,
                 ...
                 key_aware=False,
                 split_attention=False):
        super(TransformerDecoderLayer_Rotate_Position_split, self).__init__()
        ...
        self.cross_attn = PPMSDeformableAttention_Rotate_Position_split(d_model, n_head, n_levels,
                                                  n_points, 1.0)
    ...
    def forward(self,
        ...
        # cross attention
        tgt2 = self.cross_attn(gt_meta,
            self.with_pos_embed(tgt, query_pos_embed),self.with_pos_embed(tgt, query_pos_embed_vis),
                                       self.with_pos_embed(tgt, query_pos_embed_ir),
                                       reference_points_vis,reference_points_ir, angle_vis,angle_ir, memory,
            memory_spatial_shapes, memory_level_start_index,angle_max,half_pi_bin, memory_mask,topk_ind_mask,topk_score, mask_vis=mask_vis, flag='cls')
...
@register
class Multi_RTDETRTransformer_Rotate_Paired_Decouple_atten(nn.Layer):
    __shared__ = ['num_classes', 'hidden_dim', 'eval_size']
    ...
    def __init__(self,
                 ...
                 proj_all = False,
                 split_attention = False):
        super(Multi_RTDETRTransformer_Rotate_Paired_Decouple_atten, self).__init__()
        assert position_embed_type in ['sine', 'learned'], \
            f'ValueError: position_embed_type not supported {position_embed_type}!'
        assert len(backbone_feat_channels) <= num_levels
        assert len(feat_strides) == len(backbone_feat_channels)
        ...
        # Transformer module
        decoder_layer = TransformerDecoderLayer_Rotate_Position_split_Decouple_atten(
            hidden_dim, nhead, dim_feedforward, dropout, activation, num_visir_levels,
            num_decoder_points,self.angle_max, self.angle_proj,key_aware=key_aware,split_attention=split_attention)
        self.decoder = TransformerDecoder_Rotate_Position_split_Paired_Decouple_atten(hidden_dim, decoder_layer,
                                          num_decoder_layers,self.angle_max, self.angle_proj ,self.half_pi_bin, eval_idx)
        ...
        # # with angle
        # self.query_pos_head = MLP(5, 2 * hidden_dim, hidden_dim, num_layers=2)
    ...
    def forward(self, feats, vis_feats, ir_feats, pad_mask=None, gt_meta=None,topk_ind_mask=None,topk_score = None):
        ...
        # if self.num_visir_levels == 6:
        visir_feats = vis_feats + ir_feats
        #visir_feats = ir_feats + vis_feats
        # elif self.num_visir_levels == 5:
        #     visir_feats = [vis_feats[0], vis_feats[1], ir_feats[0], ir_feats[1], vis_feats[2]+ir_feats[2]]
        ...
        if self.training:
            denoising_class, denoising_bbox_unact_vis,denoising_bbox_unact_ir,denosing_angle_vis,denosing_angle_ir, attn_mask, dn_meta = \
                get_contrastive_denoising_training_group_rotated_paired(gt_meta,
                                            self.num_classes,
                                            self.num_queries,
                                            self.denoising_class_embed.weight,
                                            self.num_denoising,
...
@register
class Multi_RTDETRTransformer_Rotate_Paired(nn.Layer):
    __shared__ = ['num_classes', 'hidden_dim', 'eval_size']
    ...
    def __init__(self,
                 ...
                 proj_all = False,
                 split_attention = False):
        super(Multi_RTDETRTransformer_Rotate_Paired, self).__init__()
        assert position_embed_type in ['sine', 'learned'], \
            f'ValueError: position_embed_type not supported {position_embed_type}!'
        assert len(backbone_feat_channels) <= num_levels
        assert len(feat_strides) == len(backbone_feat_channels)
        ...
        # Transformer module
        decoder_layer = TransformerDecoderLayer_Rotate_Position_split(
            hidden_dim, nhead, dim_feedforward, dropout, activation, num_visir_levels,
            num_decoder_points,self.angle_max, self.angle_proj,key_aware=key_aware,split_attention=split_attention)
        self.decoder = TransformerDecoder_Rotate_Position_split_Paired(hidden_dim, decoder_layer,
                                          num_decoder_layers,self.angle_max, self.angle_proj ,self.half_pi_bin, eval_idx)

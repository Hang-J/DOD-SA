...
__all__ = [
    ...
    'DETRPostProcess_Rotate_Paired','DETRPostProcess_Rotate'
]
...
class DETRPostProcess_Rotate(object):
    ...
    def __call__(self, head_out, im_shape, scale_factor, pad_shape, flag=0):
        ...
        if flag == 0:
            bbox_pred = self._box2corners(bbox_pred)
        ...
        bbox_num = paddle.to_tensor(
            self.num_top_queries, dtype='int32').tile([bbox_pred.shape[0]])
        if flag == 0:
            bbox_pred = bbox_pred.reshape([-1, 10])
        elif flag == 1:
...
@register
class DETRPostProcess_Rotate_Paired(object):
    __shared__ = ['num_classes', 'use_focal_loss', 'with_mask']
    __inject__ = []
    ...
    def __init__(self,
                 ...
                 use_avg_mask_score=False,
                 bbox_decode_type='origin'):
        super(DETRPostProcess_Rotate_Paired, self).__init__()
        assert bbox_decode_type in ['origin', 'pad']
    ...
    def __call__(self, head_out, im_shape, scale_factor, pad_shape, flag=0):
        ...
        if flag == 0:
            ...
            bbox_pred_vis = bbox_pred_vis.reshape([-1, 7])
            bbox_pred_ir = bbox_pred_ir.reshape([-1, 7])
        return bbox_pred_vis, bbox_pred_ir, bbox_num, mask_pred

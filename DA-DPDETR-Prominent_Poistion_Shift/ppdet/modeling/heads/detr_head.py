...
__all__ = ['DETRHead', 'DeformableDETRHead', 'DINOHead', 'MaskDINOHead','DINOHead_Rotate', 'DINOHead_Rotate_DAOD',
           'DINOHead_Rotate_Paired_DAOD']
...
class DINOHead_Rotate(nn.Layer):
    ...
    def __init__(self, loss='DINOLoss_Rotate',group=False,groupx4=False,groupx5=False,groupx3_v3=False,groupx3_v4=False,groupx3_v5=False,
                 groupx3_v6=False):
    ...
    def forward(self, out_transformer, body_feats, inputs=None):
        ...
        if self.training:
            ...
            #b,_, height, width = inputs['image'].shape
            im_shape = [inputs['im_shape'][0,1], inputs['im_shape'][0,0]]
            im_shape = paddle.to_tensor(im_shape).squeeze(1)
...
@register
class DINOHead_Rotate_DAOD(nn.Layer):
    __inject__ = ['loss']

    def __init__(self, loss='DINOLoss_Rotate'):
        super(DINOHead_Rotate_DAOD, self).__init__()
        self.loss = loss


    def forward(self, out_transformer, body_feats, inputs=None, flag=None):
        if flag == 'DAOD':
            (out_bboxes,out_logits,out_angles_cls,gt_rbox,gt_class,im_shape,dn_out_bboxes,
             dn_out_logits,dn_out_angles_cls,dn_meta) = out_transformer
            im_shape = [im_shape[0, 1], im_shape[0, 0]]
            im_shape = paddle.to_tensor(im_shape).squeeze(1)
            return self.loss(
                out_bboxes,
                out_logits,
        ...
        if self.training:
            assert inputs is not None
            assert 'gt_rbox_ir' in inputs and 'gt_class' in inputs
            ...
            if inputs['flag'] == 3: #3 means do sup_cat train
                return self.loss(
                    out_bboxes,
                    out_logits,
                    out_angles_cls,
                    inputs['gt_rbox_ir_cat'],
                    inputs['gt_class_cat'],
                    im_shape,
                    dn_out_bboxes=dn_out_bboxes,
                    dn_out_logits=dn_out_logits,
                    dn_out_angle_cls=dn_out_angles_cls,
                    dn_meta=dn_meta)
            elif inputs['flag'] == 7: #7 means do sup train
                return self.loss(
                    out_bboxes,
                    out_logits,
                    out_angles_cls,
                ...
                return (out_bboxes, out_logits, out_angles_cls,dn_out_bboxes,dn_out_logits,dn_out_angles_cls,dn_meta)
            else:
                return self.loss(
                    out_bboxes,
                    out_logits,
...
@register
class DINOHead_Rotate_Paired(nn.Layer):
    __inject__ = ['loss']

    def __init__(self, loss='DINOLoss_Rotate_Paired',group=False,groupx4=False,groupx5=False,groupx3_v3=False,groupx3_v4=False,groupx3_v5=False,
                 groupx3_v6=False):
        super(DINOHead_Rotate_Paired, self).__init__()
        self.loss = loss
        self.group = group
        self.groupx4 = groupx4
        self.groupx5 = groupx5
    ...
    def forward(self, out_transformer, body_feats, inputs=None):
        if self.groupx5:
            ...
            (dec_out_bboxes_vis,dec_out_bboxes_ir, dec_out_logits, dec_out_angles_cls_vis,dec_out_angles_cls_ir,
             dec_out_angles_vis,dec_out_angles_ir, enc_topk_bboxes_vis,enc_topk_bboxes_ir, enc_topk_logits, enc_topk_angles_cls_vis,enc_topk_angles_cls_ir,
             angle_max,angle_proj,
             dn_meta) = out_transformer

        b, l = enc_topk_angles_cls_vis.shape[:2]
        enc_topk_angles_vis = F.softmax(enc_topk_angles_cls_vis.reshape([b, l, 1, angle_max + 1
                                                 ])).matmul(angle_proj)
        enc_topk_angles_ir = F.softmax(enc_topk_angles_cls_ir.reshape([b, l, 1, angle_max + 1
                                                                         ])).matmul(angle_proj)
        dec_out_bboxes_vis = paddle.concat([dec_out_bboxes_vis,dec_out_angles_vis], axis=-1)
        enc_out_bboxes_vis = paddle.concat([enc_topk_bboxes_vis, enc_topk_angles_vis], axis=-1)
        dec_out_bboxes_ir = paddle.concat([dec_out_bboxes_ir, dec_out_angles_ir], axis=-1)
        enc_out_bboxes_ir = paddle.concat([enc_topk_bboxes_ir, enc_topk_angles_ir], axis=-1)
        ...
        if self.training:
            ...
            if dn_meta is not None:
                if isinstance(dn_meta, list):
                    ...
                    if self.group:
                        ...
                        dn_meta['dn_num_split'] = [dn_meta['dn_num_split'][0], dn_meta['dn_num_split'][1] * 5]
                    dn_out_bboxes_vis, dec_out_bboxes_vis = paddle.split(
                        dec_out_bboxes_vis, dn_meta['dn_num_split'], axis=2)
                    dn_out_bboxes_ir, dec_out_bboxes_ir = paddle.split(
                        dec_out_bboxes_ir, dn_meta['dn_num_split'], axis=2)
                    dn_out_logits, dec_out_logits = paddle.split(
                        dec_out_logits, dn_meta['dn_num_split'], axis=2)
                    dn_out_angles_cls_vis, dec_out_angles_cls_vis = paddle.split(
                        dec_out_angles_cls_vis, dn_meta['dn_num_split'], axis=2)
                    dn_out_angles_cls_ir, dec_out_angles_cls_ir = paddle.split(
                        dec_out_angles_cls_ir, dn_meta['dn_num_split'], axis=2)


            else:
                dn_out_bboxes_vis,dn_out_bboxes_ir, dn_out_logits, dn_out_angles_cls_vis,dn_out_angles_cls_ir, dn_out_angles_vis,dn_out_angles_ir = None, None, None, None, None, None, None


            out_bboxes_vis = paddle.concat(
                [enc_out_bboxes_vis.unsqueeze(0), dec_out_bboxes_vis])
            out_bboxes_ir = paddle.concat(
                [enc_out_bboxes_ir.unsqueeze(0), dec_out_bboxes_ir])
            out_logits = paddle.concat(
                [enc_topk_logits.unsqueeze(0), dec_out_logits])
            out_angles_cls_vis = paddle.concat(
                [enc_topk_angles_cls_vis.unsqueeze(0), dec_out_angles_cls_vis]
            )
            out_angles_cls_ir = paddle.concat(
                [enc_topk_angles_cls_ir.unsqueeze(0), dec_out_angles_cls_ir]
            )
            #b,_, height, width = inputs['image'].shape
            im_shape = [inputs['im_shape'][0,1], inputs['im_shape'][0,0]]
            im_shape = paddle.to_tensor(im_shape).squeeze(1)
            ...
            if self.groupx5:
                ...
                return self.loss(
                    ...
                    inputs['gt_class'],
                    im_shape,
                    dn_out_bboxes_vis=dn_out_bboxes_vis,
                    dn_out_bboxes_ir=dn_out_bboxes_ir,
                    dn_out_logits=dn_out_logits,
                    dn_out_angle_cls_vis=dn_out_angles_cls_vis,
                    dn_out_angle_cls_ir=dn_out_angles_cls_ir,
                    dn_meta=dn_meta)
        else:
            return (dec_out_bboxes_ir[-1], dec_out_logits[-1], None)

            # #paired infer
            # return (dec_out_bboxes_vis[-1],dec_out_bboxes_ir[-1], dec_out_logits[-1], None)

@register
class DINOHead_Rotate_Paired_DAOD(nn.Layer):
    __inject__ = ['loss']

    def __init__(self, loss='DINOLoss_Rotate_Paired'):
        super(DINOHead_Rotate_Paired_DAOD, self).__init__()
        self.loss = loss
    ...
    def forward(self, out_transformer, body_feats, inputs=None):
        ...
        if self.training:
            ...
            # if inputs['flag'] == 8:
            #     # return (out_bboxes_vis, out_bboxes_ir, out_logits,
            #     #         out_angles_cls_vis, out_angles_cls_ir)
            #     return self.loss(
            #         out_bboxes_vis,
            #         out_bboxes_ir,
            #         out_logits,
            #         out_angles_cls_vis,
            #         out_angles_cls_ir,
            #         inputs['paired_teacher_rbox_vis'],
            #         inputs['paired_gt_rbox_ir'],
            #         inputs['paired_gt_class'],
            #         im_shape,
            #         dn_out_bboxes_vis=dn_out_bboxes_vis,
            #         dn_out_bboxes_ir=dn_out_bboxes_ir,
            #         dn_out_logits=dn_out_logits,
            ...
            #         dn_out_angle_cls_ir=dn_out_angles_cls_ir,
            #         dn_meta=dn_meta)
            if inputs['flag'] == 10 or inputs['flag'] == 8:
                # return (out_bboxes_vis, out_bboxes_ir, out_logits,
                #         out_angles_cls_vis, out_angles_cls_ir)
                paired_gt_rbox_ir=[]
                paired_teacher_rbox_vis=[]
                for t1, t2 in zip(inputs['paired_gt_rbox_ir_match'], inputs['paired_gt_rbox_ir_unmatch']):
                    combined_tensor = paddle.concat((t1, t2), axis=0)
                    paired_gt_rbox_ir.append(combined_tensor)
                for t1, t2 in zip(inputs['paired_teacher_bbox_match'], inputs['paired_teacher_bbox_unmatch']):
                    combined_tensor = paddle.concat((t1, t2), axis=0)
                ...
                return self.loss(
                    ...
                    dn_out_angle_cls_ir=dn_out_angles_cls_ir,
                    dn_meta=dn_meta,
                    paired_gt_rbox_match_shape=[tensors.shape[0] for tensors in inputs['paired_gt_rbox_ir_match']],
                    paired_gt_rbox_unmatch_shape=[tensors.shape[0] for tensors in inputs['paired_gt_rbox_ir_unmatch']])  
        else:
            if inputs['flag'] == 9 or inputs['flag'] == 'infer_paired':
...
@register
class DINOHead_Paired(nn.Layer):
    __inject__ = ['loss']

    def __init__(self, loss='DINOLoss_Paired',group=False,groupx4=False,groupx5=False,groupx3_v3=False,groupx3_v4=False,groupx3_v5=False,
                 groupx3_v6=False):
        super(DINOHead_Paired, self).__init__()
        self.loss = loss
        self.group = group
        self.groupx4 = groupx4
        self.groupx5 = groupx5
    ...
    def forward(self, out_transformer, body_feats, inputs=None):
        if self.groupx5:
            ...
            (dec_out_bboxes_vis, dec_out_bboxes_ir, dec_out_logits, enc_topk_bboxes_vis, enc_topk_bboxes_ir, enc_topk_logits,
             dn_meta, mask_vis) = out_transformer
        if self.training:
            assert inputs is not None
            assert 'gt_bbox_vis' in inputs and 'gt_class' in inputs and 'gt_bbox_ir' in inputs

            if dn_meta is not None:
                if isinstance(dn_meta, list):
                    dual_groups = len(dn_meta) - 1
                    dec_out_bboxes_vis = paddle.split(
                        dec_out_bboxes_vis, dual_groups + 1, axis=2)
                    dec_out_bboxes_ir = paddle.split(
                        dec_out_bboxes_ir, dual_groups + 1, axis=2)
                    dec_out_logits = paddle.split(
                        dec_out_logits, dual_groups + 1, axis=2)
                    enc_topk_bboxes_vis = paddle.split(
                        enc_topk_bboxes_vis, dual_groups + 1, axis=1)
                    enc_topk_bboxes_ir = paddle.split(
                        enc_topk_bboxes_ir, dual_groups + 1, axis=1)
                    enc_topk_logits = paddle.split(
                        enc_topk_logits, dual_groups + 1, axis=1)
                ...
                dn_out_bboxes_vis, dn_out_bboxes_ir, dn_out_logits = None, None, None

            out_bboxes_vis = paddle.concat(
                [enc_topk_bboxes_vis.unsqueeze(0), dec_out_bboxes_vis])
            out_bboxes_ir = paddle.concat(
                [enc_topk_bboxes_ir.unsqueeze(0), dec_out_bboxes_ir])
            out_logits = paddle.concat(
                [enc_topk_logits.unsqueeze(0), dec_out_logits])
            ...
            temp_out_bboxes_vis = dec_out_bboxes_vis[-1]
            temp_out_bboxes_ir = dec_out_bboxes_ir[-1]
            dec_out_bboxes_final = paddle.where(mask_vis, temp_out_bboxes_vis, temp_out_bboxes_ir)
            #return (dec_out_bboxes_ir[-1], dec_out_logits[-1], None)
            return (dec_out_bboxes_final, dec_out_logits[-1], None)

...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
                        if curr_iter >= stage3_iter: #STAGE 3
                            ...
                            paired_gt_rbox_ir_match, paired_gt_rbox_ir_unmatch, paired_gt_class, \
                                ...
                                = self.paired_label_processing_uncopyloss(
                                    data,
                                    teacher_preds['bbox'],
                                    teacher_preds['class'],
                                    teacher_preds['score']
...
#######################640*544 --> 840*712#####################
        ...
        for mm in range(len(gt_rbox_ir)):
            ...
            for i in range(len(coordinates_ir)):
                ...
                ir_search_scale=(coordinates_ir[i][0],coordinates_ir[i][1],coordinates_ir[i][2]*1.00,coordinates_ir[i][3]*1.00,coordinates_ir[i][4])
    ...
    def paired_label_processing_uncopyloss(self, data, teacher_bbox, teacher_class, teacher_score,iou_threshold=0.7):
        gt_rbox_ir=copy.deepcopy(data['gt_rbox_ir'])
        gt_class=copy.deepcopy(data['gt_class'])
        im_id=copy.deepcopy(data['im_id'])
        def points_in_rotated_box(points, box):
        ...
        for mm in range(len(gt_rbox_ir)):
            ...
            for i in range(len(coordinates_ir)):
                center_points_vis=coordinates_vis[:,0:2]
                ir_search_scale=(coordinates_ir[i][0],coordinates_ir[i][1],coordinates_ir[i][2]*1.0,coordinates_ir[i][3]*1.0,coordinates_ir[i][4])
                mask = points_in_rotated_box(center_points_vis,ir_search_scale)
                index_in_search=np.where(mask)[0]
                max_iou=0.0
                best_k=-1

...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def paired_spatial_search_nms(self, boxes_vis, boxes_ir, score_thr, iou_thr=0.5):
        ...
        # --- 2. 预计算几何参数 ---
        params_vis = np.array([self.get_rect_params(b) for b in boxes_vis_high])
        params_ir = np.array([self.get_rect_params(b) for b in boxes_ir_high])

        # --- 3. 排序 (仅对 High 部分排序) ---
        pair_scores = np.maximum(boxes_vis_high[:, 1], boxes_ir_high[:, 1])
        sorted_indices = np.argsort(pair_scores)[::-1]
        
        boxes_vis_high = boxes_vis_high[sorted_indices]
        boxes_ir_high = boxes_ir_high[sorted_indices]
        params_vis = params_vis[sorted_indices]
        params_ir = params_ir[sorted_indices]
        
        num_boxes = len(boxes_vis_high)
        ...
        for i in range(num_boxes):
            ...
            # --- A. 空间搜索 ---
            # 搜索范围扩大 1.5 倍 (保留你的逻辑)
            search_box = (cx_i, cy_i, 1.5 * w_i, 1.5 * h_i, angle_i)
        ...
        # 获取 NMS 后的高分结果
        final_vis_high = boxes_vis_high[keep_indices]
        final_ir_high = boxes_ir_high[keep_indices]

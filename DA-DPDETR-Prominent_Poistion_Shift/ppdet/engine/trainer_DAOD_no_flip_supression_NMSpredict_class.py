...
class Trainer_DAOD_no_flip_supression(object):
    def __init__(self, cfg, mode='train'):
        ...
        if 'model' not in self.cfg:
            ...
            #conduct flops
            #paddle.flops(self.model, input_size=[1,3,640,544],print_detail=True)
        else:
...
#######################640*544 --> 840*712#####################
    ...
    def predict(self,
        ...
        return results
    # -----------------------------------------------------------------------------------
    # 新增逻辑：处理 10维 (Class, Score, 8 Coords) 数据的空间搜索 + 形状 NMS
    # -----------------------------------------------------------------------------------
    ...
    def multi_predict_paired(self,
        ...
        # 遍历所有 batch 的结果进行处理
        for res in results:
            if 'bbox_num' not in res or 'bbox_vis' not in res or 'bbox_ir' not in res:
                continue
                
            origin_bbox_nums = res['bbox_num']
            boxes_vis_all = res['bbox_vis']
            boxes_ir_all = res['bbox_ir']
            ...
            if len(new_vis_list) > 0:
                res['bbox_vis'] = np.concatenate(new_vis_list, axis=0)
                res['bbox_ir'] = np.concatenate(new_ir_list, axis=0)
            else:
                res['bbox_vis'] = np.zeros((0, boxes_vis_all.shape[1]), dtype=boxes_vis_all.dtype)
                res['bbox_ir'] = np.zeros((0, boxes_ir_all.shape[1]), dtype=boxes_ir_all.dtype)
            
            # 更新 bbox_num
            res['bbox_num'] = np.array(new_counts_list, dtype='int32')

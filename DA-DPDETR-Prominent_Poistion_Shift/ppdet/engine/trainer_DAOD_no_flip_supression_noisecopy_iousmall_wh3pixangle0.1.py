...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def paired_label_processing(self, gt_rbox_ir, gt_class, teacher_bbox, teacher_class,iou_threshold=0.5):
        ...
        for mm in range(len(gt_rbox_ir)):
            ...
            for i in range(len(coordinates_ir)):
                ...
                for k in range(len(coordinates_vis)):
                    ...
                    if iou > iou_threshold:  # IoU 阈值
                        w_ir, h_ir, angle_ir = coordinates_ir[i][2], coordinates_ir[i][3], coordinates_ir[i][4]
                        w_vis, h_vis, angle_vis = coordinates_vis[k][2], coordinates_vis[k][3], coordinates_vis[k][4]

                        # 长宽和角度差异限制
                        w_diff = abs(w_ir - w_vis)
                        h_diff = abs(h_ir - h_vis)
                        angle_diff = abs(angle_ir - angle_vis)

                        if w_diff <= 3 and h_diff <= 3 and angle_diff < 0.1:  # <--- 修改处
                            if iou > max_iou:
                                max_iou = iou
                                best_k = k
                if best_k!=-1:  # 匹配上了
            ...
            for box, cls in zip(coordinates_ir, categories_ir):
                noisy_box = box.copy()  # 拷贝原IR框
                noisy_box[0] += np.random.uniform(-10, 10)  # x偏移
                noisy_box[1] += np.random.uniform(-10, 10)  # y偏移
                coordinates_vis_unmatch.append(noisy_box)  # <-- 修改：添加加噪的框作为VIS框

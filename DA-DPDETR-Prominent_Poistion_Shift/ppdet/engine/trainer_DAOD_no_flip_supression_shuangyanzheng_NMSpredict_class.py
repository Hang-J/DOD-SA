...
from ppdet.modeling.rbox_utils import poly2rbox_le135_np
                            ...
                            if curr_iter > st_iter:
                                ...
                                with paddle.no_grad():
                                    ...
                                    for xx in range(len(teacher_preds['bbox'])) :
                                        teacher_preds['bbox'][xx] = teacher_preds['bbox'][xx][teacher_preds['bbox'][xx][:,1] > self.psudo_labels_threhold]
                                ...
                                data['teacher_gt_class']=teacher_preds['class']
                                data['teacher_gt_rbox_vis']=teacher_preds['bbox']
                                data['flag'] = 4
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
                        if curr_iter >= stage2_iter and curr_iter < stage3_iter: #STAGE 2
                            ...
                            with paddle.no_grad():
                                ...
                                for xx in range(len(teacher_preds['bbox'])):
                                    teacher_preds['bbox'][xx] = teacher_preds['bbox'][xx][
                                        teacher_preds['bbox'][xx][:, 1] > self.psudo_labels_threhold]
                                    teacher_preds['bbox_num'][xx] = len(teacher_preds['bbox'][xx])
                            ...
                            loss_dict_unsup = self.model.get_ssod_loss_single(
                                (student_preds[0], student_preds[1], student_preds[2],
                                 teacher_preds['bbox'], teacher_preds['class'], data['im_shape'],
                                 student_preds[3],student_preds[4],student_preds[5],student_preds[6]))
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
                        if curr_iter >= stage3_iter: #STAGE 3
                            ...
                            # paired label processing
                            # paired_gt_rbox_vis, paired_gt_class, paired_teacher_rbox_ir = self.paired_label_processing(
                            #     data['gt_rbox_vis'],
                            #     data['gt_class'],
                            #     teacher_preds['bbox'],
                            #     teacher_preds['class'])
            ...
            if validate and is_snapshot:
                ...
                with paddle.no_grad():
                    ...
                    if curr_iter <= stage2_iter:
                        if curr_iter >= st_iter:
                            self._eval_with_loader(self._eval_loader, eval_flag, my_eval_flag='eval_stage1_2')
                        else:
                            self._eval_with_loader(self._eval_loader, eval_flag, my_eval_flag='eval_stage1')
                        # self._eval_with_loader2(self._eval_loader, eval_flag, my_eval_flag='eval_stage1')
                        ...
                        # self._eval_with_loader2(self._eval_loader, eval_flag, my_eval_flag='eval_stage2')
                    elif curr_iter > stage3_iter:
                        self._eval_with_loader(self._eval_loader, eval_flag, my_eval_flag='infer_paired')
                        # self._eval_with_loader2(self._eval_loader, eval_flag, my_eval_flag='eval_stage3')
    ...
    def paired_label_processing(self, gt_rbox_ir, gt_class, teacher_bbox, teacher_class):
        ...
        curr_iter = len(self.loader) * self.epoch + self.step
        ...
        for mm in range(len(gt_rbox_ir)):
            ...
            for i in range(len(coordinates_ir)):
                ...
                # 遍历ir框，看是否有匹配上的
                for k in range(len(coordinates_vis)):
                    iou = compute_iou(coordinates_ir[i],coordinates_vis[k])
                    if iou >= 0.6:  # 匹配上了
                        coordinates_ir_match.append(coordinates_ir[i])
                        coordinates_vis_match.append(coordinates_vis[k])
                        categories_ir_new.append(categories_ir[i])
                        categories_vis_new.append(categories_vis[k])
                        record_pop_idx_ir.append(i)  # 记录匹配上的idx
                        record_pop_idx_vis.append(k)
                        break

            # 在原coordinates中弹出，剩余的说明各自label标注的两个模态上
            ...
            for box, cls in zip(coordinates_ir, categories_ir):
                ...
                categories_vis_new.append(cls)
                coordinates_ir_unmatch.append(box)
                categories_ir_new.append(cls)
                # 再复制红外的
            # for box, cls in zip(coordinates_vis, categories_vis):
            #     coordinates_vis_new.append(box)
            #     categories_vis_new.append(cls)
            #     coordinates_ir_new.append(box)
            #     categories_ir_new.append(cls)
            ...
            new_gt_class.append(paddle.to_tensor(categories_ir_new))  
        return new_gt_rbox_ir_match, new_gt_rbox_ir_unmatch, new_gt_class, new_teacher_bbox_match, new_teacher_bbox_unmatch
    def get_rect_params(self,box_10d):
        """
        从 10维向量中提取旋转矩形参数 (cx, cy, w, h, theta_rad)
        box_10d: [cls, score, x1, y1, x2, y2, x3, y3, x4, y4]
        """
        rbox_fn = poly2rbox_le135_np
        cx, cy, w, h, theta_rad = rbox_fn(box_10d[2:])
        # cx, cy, w, h, theta_rad=rbox_fn(box_10d[2:].reshape(4,2))
        # # 提取 8个坐标点
        # coords = box_10d[2:].reshape(4, 2).astype(np.float32)
        
        # # 使用 minAreaRect 获取最小外接矩形
        # # 返回值: ((cx, cy), (w, h), angle_deg)
        # ((cx, cy), (w, h), angle_deg) = cv2.minAreaRect(coords)
        
        # # 转为弧度 (minAreaRect 返回的是角度，范围通常是 [-90, 0) 或其他，视版本而定)
        # # points_in_rotated_box 需要的是弧度
        # theta_rad = angle_deg * np.pi / 180.0
        
        return cx, cy, w, h, theta_rad
    ...
    def calculate_shape_aligned_iou(self, w1, h1, w2, h2):
        """
        计算中心对齐、角度一致后的形状 IoU (只比较宽高重叠)
        """
        inter_w = np.minimum(w1, w2)
        inter_h = np.minimum(h1, h2)
        inter_area = np.maximum(0, inter_w * inter_h)
        union_area = (w1 * h1) + (w2 * h2) - inter_area
        return inter_area / (union_area + 1e-6)

    def paired_spatial_search_nms(self, boxes_vis, boxes_ir, score_thr, iou_thr=0.5):
        if boxes_vis is None or len(boxes_vis) == 0:
            return boxes_vis, boxes_ir

        # --- 1. 分离高分框和低分框 ---
        scores_vis = boxes_vis[:, 1]
        scores_ir = boxes_ir[:, 1]
        
        # 筛选掩码：任意一个模态分数达标，即视为高分框，参与 NMS
        high_mask = (scores_vis >= score_thr) | (scores_ir >= score_thr)
        
        # 分离出需要做 NMS 的部分 (High)
        boxes_vis_high = boxes_vis[high_mask]
        boxes_ir_high = boxes_ir[high_mask]
        
        # 分离出直接保留的部分 (Low)
        boxes_vis_low = boxes_vis[~high_mask]
        boxes_ir_low = boxes_ir[~high_mask]

        # 如果没有高分框，直接返回低分框（或者原数据）
        if len(boxes_vis_high) == 0:
            return boxes_vis, boxes_ir

        # ================= 对 High 部分进行 NMS 处理（按类别） =================
        kept_vis_high = []
        kept_ir_high = []

        for cls_id in np.unique(boxes_vis_high[:, 0]):
            cls_mask = boxes_vis_high[:, 0] == cls_id
            cls_vis_high = boxes_vis_high[cls_mask]
            cls_ir_high = boxes_ir_high[cls_mask]

            if len(cls_vis_high) == 0:
                continue

            params_vis = np.array([self.get_rect_params(b) for b in cls_vis_high])
            params_ir = np.array([self.get_rect_params(b) for b in cls_ir_high])

            # --- 3. 排序 (仅对当前类别的 High 部分排序) ---
            pair_scores = np.maximum(cls_vis_high[:, 1], cls_ir_high[:, 1])
            sorted_indices = np.argsort(pair_scores)[::-1]
            
            cls_vis_high = cls_vis_high[sorted_indices]
            cls_ir_high = cls_ir_high[sorted_indices]
            params_vis = params_vis[sorted_indices]
            params_ir = params_ir[sorted_indices]
            
            num_boxes = len(cls_vis_high)
            ...
            for i in range(num_boxes):
                ...
                # 使用 IR 框作为空间搜索的基准 (保留你的逻辑)
                cx_i, cy_i = params_ir[i, 0], params_ir[i, 1]
                w_i, h_i = params_ir[i, 2], params_ir[i, 3]
                angle_i = params_ir[i, 4]
                
                candidate_indices = np.arange(i + 1, num_boxes)
                valid_mask = ~suppressed[candidate_indices]
                if not np.any(valid_mask):
                    continue
                valid_candidate_indices = candidate_indices[valid_mask]
                
                # 提取候选框中心点 (使用 IR 的中心点进行匹配查询)
                candidate_centers = params_ir[valid_candidate_indices, 0:2]
                
                # --- A. 空间搜索 ---
                search_box = (cx_i, cy_i, 1.2 * w_i, 1.2 * h_i, angle_i)
                in_box_mask = self.points_in_rotated_box(candidate_centers, search_box)
                
                indices_to_check = valid_candidate_indices[in_box_mask]
                
                # --- B. 形状 IoU 检查 ---
                for j in indices_to_check:
                    iou_vis = self.calculate_shape_aligned_iou(
                        params_vis[i, 2], params_vis[i, 3], 
                        params_vis[j, 2], params_vis[j, 3]
                    )
                    
                    iou_ir = self.calculate_shape_aligned_iou(
                        params_ir[i, 2], params_ir[i, 3],
                        params_ir[j, 2], params_ir[j, 3]
                    )
                    
                    # 严格抑制条件
                    if iou_vis > iou_thr and iou_ir > iou_thr:
                        suppressed[j] = True

            keep_indices = np.where(~suppressed)[0]
            if len(keep_indices) > 0:
                kept_vis_high.append(cls_vis_high[keep_indices])
                kept_ir_high.append(cls_ir_high[keep_indices])

        if len(kept_vis_high) > 0:
            final_vis_high = np.concatenate(kept_vis_high, axis=0)
            final_ir_high = np.concatenate(kept_ir_high, axis=0)
        else:
            final_vis_high = np.zeros((0, boxes_vis_high.shape[1]), dtype=boxes_vis_high.dtype)
            final_ir_high = np.zeros((0, boxes_ir_high.shape[1]), dtype=boxes_ir_high.dtype)

        # ================= 结果合并 =================
        
        # 将保留的高分框与原始低分框拼接
        if len(boxes_vis_low) > 0:
            if len(final_vis_high) > 0:
                final_vis = np.concatenate([final_vis_high, boxes_vis_low], axis=0)
                final_ir = np.concatenate([final_ir_high, boxes_ir_low], axis=0)
            else:
                final_vis = boxes_vis_low
                final_ir = boxes_ir_low
        else:
            final_vis = final_vis_high
            final_ir = final_ir_high

        return final_vis, final_ir
    ...
    def _eval_with_loader(self, loader, eval_flag=0, my_eval_flag = 'eval_stage3'):
        ...
        for step_id, data in enumerate(loader):
            self.status['step_id'] = step_id
            self._compose_callback.on_step_begin(self.status)
            if my_eval_flag=='eval_stage1_2':
                data['flag'] = 'eval_stage1'
            else:
            ...
            if self.use_amp:
                ...
                outs = eval_model(data)
    # -----------------------------------------------------------------------------------
            if 'bbox_num' not in outs or 'bbox_vis' not in outs or 'bbox_ir' not in outs:
                continue
                
            origin_bbox_nums = outs['bbox_num']
            boxes_vis_all = outs['bbox_vis']
            boxes_ir_all = outs['bbox_ir']
            
            if boxes_vis_all.shape[0] == 0:
                continue

            # 按图切分 batch 数据
            split_points = np.cumsum(origin_bbox_nums)[:-1]
            vis_list = np.split(boxes_vis_all, split_points)
            ir_list = np.split(boxes_ir_all, split_points)
            ...
            # 逐张图进行处理
            for img_vis_boxes, img_ir_boxes in zip(vis_list, ir_list):
                processed_vis, processed_ir = self.paired_spatial_search_nms(
                    img_vis_boxes, 
                    img_ir_boxes, 
                    score_thr=0.2, 
                    iou_thr=0.8
                )
                new_vis_list.append(processed_vis)
                new_ir_list.append(processed_ir)
                new_counts_list.append(len(processed_vis))
            
            # 重新拼接并写回 results
            if len(new_vis_list) > 0:
                outs['bbox_vis'] = np.concatenate(new_vis_list, axis=0)
                outs['bbox_ir'] = np.concatenate(new_ir_list, axis=0)
            else:
                outs['bbox_vis'] = np.zeros((0, boxes_vis_all.shape[1]), dtype=boxes_vis_all.dtype)
                outs['bbox_ir'] = np.zeros((0, boxes_ir_all.shape[1]), dtype=boxes_ir_all.dtype)
            
            # 更新 bbox_num
            outs['bbox_num'] = np.array(new_counts_list, dtype='int32')
                
    # -----------------------------------------------------------------------------------

            data['gt_poly']=data['gt_poly_ir']
            data['gt_bbox']=data['gt_bbox_ir']
            if my_eval_flag=='infer_paired':
                outs['bbox']=outs['bbox_ir']
            for metric in self._metrics:
    ...
    # -----------------------------------------------------------------------------------
            data['gt_poly']=data['gt_poly_vis']       
            data['gt_bbox']=data['gt_bbox_vis']
            if my_eval_flag=='infer_paired':
                outs['bbox']=outs['bbox_vis']
            elif my_eval_flag=='eval_stage1_2':
                pass
            for metric in self._metrics:
                metric.update(data, outs)
            ...
            if not hasattr(self, '_eval_loader'):
                ...
                if self.cfg.metric == "Pose3DEval":
                    ...
                    self._eval_loader = create('EvalReader2')(
                        ...
                        self.cfg.worker_num,
                        batch_sampler=self._eval_batch_sampler)            
            self._eval_with_loader(self._eval_loader, 0, my_eval_flag='infer_paired')

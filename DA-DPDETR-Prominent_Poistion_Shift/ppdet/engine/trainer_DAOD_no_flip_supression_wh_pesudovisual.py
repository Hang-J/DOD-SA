...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
                        if curr_iter >= stage2_iter and curr_iter < stage3_iter: #STAGE 2
                            ...
                            self.paired_label_visual2(data['im_id'],data['ir_image'],data['vis_image'],\
                                {'match_bbox': paired_gt_rbox_ir_match,'match_class':class_ir_match,'unmatch_bbox': paired_gt_rbox_ir_unmatch,'unmatch_class':class_ir_unmatch,'match_iou':match_iou,'unmatch_iou':unmatch_iou},\
                                {'match_bbox': paired_teacher_bbox_match,'match_class':match_teacher_categories,'match_score':match_teacher_score,'unmatch_bbox': res_teacher_coordinates,'unmatch_class':res_teacher_categories,'unmatch_score':res_teacher_score})
                            # self.paired_label_visual2_notext(data['im_id'],data['ir_image'],data['vis_image'],\
                            #     {'match_bbox': paired_gt_rbox_ir_match,'match_class':class_ir_match,'unmatch_bbox': paired_gt_rbox_ir_unmatch,'unmatch_class':class_ir_unmatch,'match_iou':match_iou,'unmatch_iou':unmatch_iou},\
                            #     {'match_bbox': paired_teacher_bbox_match,'match_class':match_teacher_categories,'match_score':match_teacher_score,'unmatch_bbox': res_teacher_coordinates,'unmatch_class':res_teacher_categories,'unmatch_score':res_teacher_score})
...
##################################sup- 真实标签监督   unsup- 伪标签监督   multi- DP-MSDecoder####################
    ...
    def paired_label_visual(self, im_id, ir_image, vis_image, bboxes_ir, bboxes_vis):
        ...
        for id in im_id:
            ...
            imid2savepath[id[0]]=[f'{folder}/{id_str_ir}']
            imid2savepath[id[0]]+=[f'{folder}/{id_str_vis}']
        for i, im_id in enumerate(imid2savepath):
            vis_image_savepath = imid2savepath[int(im_id)][1]
            ir_image_savepath = imid2savepath[int(im_id)][0] 
            ...
            vis_image.save(vis_image_savepath, quality=99)
            ir_image.save(ir_image_savepath, quality=99)  
    def paired_label_visual2(self, im_id, ir_image, vis_image, bboxes_ir, bboxes_vis):
        def box2corners(pred_bboxes):
            x, y, w, h, angle = pred_bboxes
            cos_a_half = math.cos(angle) * 0.5
            sin_a_half = math.sin(angle) * 0.5
        ...
        def draw_bbox_paired(image, bboxes,i):
            ...
            for key, value in bboxes.items():
                for k,value1 in enumerate(bboxes[key]):
                    bboxes[key][k]=value1.numpy()
            for j,_ in enumerate(bboxes['match_bbox'][i]):
                if bboxes.get('match_score') is not None:
                    catid, bbox, score = bboxes['match_class'][i][j], bboxes['match_bbox'][i][j], bboxes['match_score'][i][j]
                else:
                    catid, bbox, iou= bboxes['match_class'][i][j], bboxes['match_bbox'][i][j], bboxes['match_iou'][i][j]
                # if score < threshold:
                #     continue
                catid=catid[0]
                x1, y1, x2, y2, x3, y3, x4, y4 = box2corners(bbox)#*840/640
                ...
                if bboxes.get('match_score') is not None:
                    ...
                    text = "{}|{}|{:.2f}".format(count,catid2name[catid],iou)
                count+=1
                bbox = draw.textbbox((0,0), text)
                tw = bbox[2] - bbox[0]
                th = bbox[3] - bbox[1]
                draw.rectangle(
                    [(xmin + 1, ymin - th), (xmin + tw + 1, ymin)], fill=(255, 0, 0))
                draw.text((xmin + 1, ymin - th), text, fill=(255, 255, 255))
            for j,_ in enumerate(bboxes['unmatch_bbox'][i]):
                if bboxes.get('unmatch_score') is not None:
                    catid, bbox, score = bboxes['unmatch_class'][i][j], bboxes['unmatch_bbox'][i][j], bboxes['unmatch_score'][i][j]
                else:
                    catid, bbox, iou= bboxes['unmatch_class'][i][j], bboxes['unmatch_bbox'][i][j], bboxes['unmatch_iou'][i][j]
                # if score < threshold:
                #     continue
                catid=catid[0]
                if catid not in catid2color:
    ...
    def paired_label_visual2_notext(self, im_id, ir_image, vis_image, bboxes_ir, bboxes_vis):
        def box2corners(pred_bboxes):
            x, y, w, h, angle = pred_bboxes
            cos_a_half = math.cos(angle) * 0.5
            sin_a_half = math.sin(angle) * 0.5
        ...
        def tensor_to_pil_images(tensor):
            ...
            return images       
        folder = f'paired_label_visual_notext/{self.epoch}'
        # 创建文件夹（如果不存在，且支持递归创建多级目录）
        os.makedirs(folder, exist_ok=True)
        ir_images=tensor_to_pil_images(ir_image)
        vis_images=tensor_to_pil_images(vis_image)
        ...
        for i, im_id in enumerate(imid2savepath):
            ...
            vis_image.save(vis_image_savepath, quality=99)
            ir_image.save(ir_image_savepath, quality=99)
    def paired_label_processing(self, gt_rbox_ir, gt_class, teacher_bbox, teacher_class, teacher_score,iou_threshold=0.4):
        ...
        return new_gt_rbox_ir_match, new_gt_rbox_ir_unmatch, new_gt_class_ir, new_teacher_bbox_match, new_teacher_bbox_unmatch,new_teacher_categories_match,new_teacher_score_match,res_teacher_coordinates,res_teacher_categories,res_teacher_score,new_record_ir_match_iou,new_record_ir_unmatch_iou

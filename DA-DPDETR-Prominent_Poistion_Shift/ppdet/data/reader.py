    ...
    def __call__(self, data):
        ...
        for f in self.transforms_cls:
            try:
                data = f(data)
                # if ii == 0 :
                #     cv2.imwrite('/data/hdd/guojunjie/pp-output-groupx3-missing-v3-m3fdmissing/original_pic/test.png',data[0]['vis_image'])
                # ii = ii+1
            except Exception as e:
                stack_info = traceback.format_exc()
        ...
        if self.collate_batch:
            batch_data = default_collate_fn(data)#将列表中的Batch个字典合成起来，多出一个Batch维度
        else:
            batch_data = {}
...
@register
class EvalReader2(BaseDataLoader):
    __shared__ = ['num_classes']
    ...
    def __init__(self,
                 ...
                 num_classes=80,
                 **kwargs):
        super(EvalReader2, self).__init__(sample_transforms, batch_transforms,
                                         batch_size, shuffle, drop_last,
                                         num_classes, **kwargs)

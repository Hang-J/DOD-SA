...
class Multi_RRotate_Paired(BaseOperator):
    ...
    def __init__(self, scale=1.0, angle=0., fill_value=0., auto_bound=True,rotate_prob=1.0):
    ...
    def get_rotated_matrix(self, angle, scale, h, w):
        ...
        if self.auto_bound:
            ...
            # ✅ 缩放后图像内容的实际尺寸
            self.ratio=ratio
            self.scaled_w = int(w * ratio)
            self.scaled_h = int(h * ratio)
    ...
    def apply(self, sample, context=None):
        ...
        ir_image = sample['ir_image']
        h, w = vis_image.shape[:2]
        matrix2, h2, w2 = self.get_rotated_matrix(0, self.scale, h, w)
        matrix, h, w = self.get_rotated_matrix(self.angle, self.scale, h, w)
        sample['vis_image'] = self.apply_image(vis_image, matrix, h, w)
        sample['ir_image'] = self.apply_image(ir_image, matrix, h, w)
        ...
        if self.rotate_prob==1.0:
            sample['ratio1']=np.array([self.ratio], dtype=np.float32)
            sample['ratio2']=np.array([1.0], dtype=np.float32)
            sample['matrix1']=np.array([matrix], dtype=np.float32)
            sample['matrix2']=np.array([matrix2], dtype=np.float32)         
            #cv2.imwrite(f"/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/rbox_Drone_Vehicle/train/transform_trainimg/{(sample['im_id'][0]+1):05d}_rotated_1_{self.angle}_{self.scaled_w}_{self.scaled_h}.jpg",sample['ir_image'])
        if self.rotate_prob==0.5:
            sample['ratio2']=np.array([self.ratio], dtype=np.float32)
            sample['matrix2']=np.array([matrix], dtype=np.float32)  
            #cv2.imwrite(f"/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/rbox_Drone_Vehicle/train/transform_trainimg/{(sample['im_id'][0]+1):05d}_rotated_2_{self.angle}_{self.scaled_w}_{self.scaled_h}.jpg",sample['ir_image'])
...
class Multi_RandomRRotate_Paired(BaseOperator):
    ...
    def apply(self, sample, context=None):
        if np.random.rand() > self.rotate_prob:
            sample['angle2'] = np.array([0], dtype=np.float32)
            return sample

        angle = self.get_angle(self.angle, self.angle_mode)
        scale = self.get_scale(self.scale, self.scale_mode)
        if self.rotate_prob==1.0:
            sample['angle1'] = np.array([angle], dtype=np.float32)
        if self.rotate_prob==0.5:
            sample['angle2'] = np.array([angle], dtype=np.float32)
        rotator = Multi_RRotate_Paired(scale, angle, self.fill_value, self.auto_bound,self.rotate_prob)
        return rotator(sample)
    ...
    def apply(self, sample, context=None):
        ...
        Output:
            sample: the image, bounding box and segmentation part
                    in sample are flipped.
        """
        sample['flipped'] = np.array([0], dtype=np.float32)
        #cv2.imwrite(f"/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/rbox_Drone_Vehicle/train/transform_trainimg/{(sample['im_id'][0]+1):05d}_unfliped.jpg",sample['ir_image'])        
        if np.random.uniform(0, 1) < self.prob:
            im_vis = sample['vis_image']
            im_ir = sample['ir_image']
            height, width = im_vis.shape[:2]
            ...
            sample['flipped'] = np.array([1], dtype=np.float32)
            sample['vis_image'] = im_vis
            sample['ir_image'] = im_ir
            #cv2.imwrite(f"/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/rbox_Drone_Vehicle/train/transform_trainimg/{(sample['im_id'][0]+1):05d}_fliped.jpg",sample['ir_image'])
        return sample
@register_op
class Eval_Shift_VIS(BaseOperator):#HWC
    def __init__(self, decision, pix1, pix2):
        super(Eval_Shift_VIS, self).__init__()
    ...
    def apply(self, sample, context=None):
        ...
        if self.decision == 0: #向上移动
            # apply img
            im = self.apply_image_top2down(im,self.pix1)
            # apply bbox
            gt_bbox = self.apply_pts_top2down(gt_bbox,self.pix1)
            gt_poly = self.apply_pts_top2down(gt_poly,self.pix1)
        elif self.decision == 1: #向下移动
            # apply img
            im = self.apply_image_down2top(im, self.pix1)
            # apply bbox
            gt_bbox = self.apply_pts_down2top(gt_bbox, self.pix1)
            gt_poly = self.apply_pts_down2top(gt_poly, self.pix1)
        elif self.decision == 2: #向左移动
            # apply img
            im = self.apply_image_left2right(im, self.pix1)
            # apply bbox
            gt_bbox = self.apply_pts_left2right(gt_bbox, self.pix1)
            gt_poly = self.apply_pts_left2right(gt_poly, self.pix1)
        elif self.decision == 3: #向右移动
            # apply img
            im = self.apply_image_right2left(im, self.pix1)
            # apply bbox
            gt_bbox = self.apply_pts_right2left(gt_bbox, self.pix1)
            gt_poly = self.apply_pts_right2left(gt_poly, self.pix1)
        elif self.decision == 4:  #向上pix1  向左pix2移动
            # apply img
            im = self.apply_image_top2down(im, self.pix1)
            # apply bbox
            gt_bbox = self.apply_pts_top2down(gt_bbox, self.pix1)
            ...
            # apply img
            im = self.apply_image_left2right(im, self.pix2)
            # apply bbox
            gt_bbox = self.apply_pts_left2right(gt_bbox, self.pix2)
            gt_poly = self.apply_pts_left2right(gt_poly, self.pix2)
        elif self.decision == 5:  #向上pix1  向右pix2移动
            ...
            # apply img
            im = self.apply_image_right2left(im, self.pix2)
            # apply bbox
            gt_bbox = self.apply_pts_right2left(gt_bbox, self.pix2)
            gt_poly = self.apply_pts_right2left(gt_poly, self.pix2)
        elif self.decision == 6:  #向下pix1  向左pix2移动
            ...
            gt_bbox = self.apply_pts_left2right(gt_bbox, self.pix2)
            gt_poly = self.apply_pts_left2right(gt_poly, self.pix2)
        elif self.decision == 7:  #向下pix1  向右pix2移动
        ...
        #cv2.imwrite(f"/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/rbox_Drone_Vehicle/val/shifted_valimg/{(sample['im_id'][0]+1):05d}.jpg",image)#f"{num:05d}.jpg"
        #save_denormalized_image(image,f"/data1/jinhang/DA-DPDETR-Prominent_Position_Shift/DA-DPDETR/dataset/rbox_Drone_Vehicle/val/shifted_valimg/{(sample['im_id'][0]+1):05d}.jpg")
        if flag == 'vis':
            sample['vis_image'] = im
        ...
        return sample
def save_denormalized_image(im, save_path, mean=None, std=None, is_scale=True):
    """
    将归一化后的图像反归一化并保存成图片文件

    Args:
        im (np.ndarray): 输入图像，形状 (C, H, W)，float32。
        save_path (str): 保存路径，比如 'output.png'
        mean (list or tuple): 每个通道的均值，比如 [0.485, 0.456, 0.406]
        std (list or tuple): 每个通道的标准差，比如 [0.229, 0.224, 0.225]
        is_scale (bool): 是否在归一化前做了1/255的缩放，默认True
    """
    if mean is None:
        mean = [0.485, 0.456, 0.406]  # 可以改成你的默认值
    if std is None:
        std = [0.229, 0.224, 0.225]   # 可以改成你的默认值
    ...
    # 反归一化
    im *= np.array(std)[np.newaxis, np.newaxis, :]
    im += np.array(mean)[np.newaxis, np.newaxis, :]
    if is_scale:
        im *= 255.0
    ...
    img_resized = img.resize((840, 712))
    img_resized.save(save_path)

@register_op
class Multi_Shift_Paired(BaseOperator):
    def __init__(self, prob=0.4, max_shift = 13):
        """
        Args:
            prob (float): the probability of flipping image
        """

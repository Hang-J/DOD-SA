...
class Multi_PadBatch(BaseOperator):
    ...
    def __call__(self, samples, context=None):
        ...
        max_shape = np.array(
            [data['vis_image'].shape for data in inner_samples]).max(axis=0)
        if coarsest_stride > 0:
            max_shape[1] = int(
                np.ceil(max_shape[1] / coarsest_stride) * coarsest_stride)
            max_shape[2] = int(
        ...
        for data in inner_samples:
            ...
            ir_im = data['ir_image']
            im_c, im_h, im_w = vis_im.shape[:]
            padding_im_vis = np.zeros(
                (im_c, max_shape[1], max_shape[2]), dtype=np.float32)
            padding_im_ir = np.zeros(
                (im_c, max_shape[1], max_shape[2]), dtype=np.float32)
            padding_im_vis[:, :im_h, :im_w] = vis_im
            padding_im_ir[:, :im_h, :im_w] = ir_im
    ...
    def __call__(self, samples, context=None):
        ...
        for data in inner_samples:
            ...
            padding_im_ir_aug = np.zeros(
                (im_c, max_shape[1], max_shape[2]), dtype=np.float32)
            padding_im_vis_aug[:, :im_h, :im_w] = vis_im_aug
            padding_im_ir_aug[:, :im_h, :im_w] = ir_im_aug

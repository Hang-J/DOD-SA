...
@register_op
class Multi_Permute(BaseOperator):#维度转换
    def __init__(self):
        """
        Change the channel to be (C, H, W)
        """
...
@register_op
class Multi_RandomFlip_Paired(BaseOperator):
    def __init__(self, prob=0.5):
        """
        Args:
            prob (float): the probability of flipping image
        """
        super(Multi_RandomFlip_Paired, self).__init__()
        self.prob = prob
        if not (isinstance(self.prob, float)):
            raise TypeError("{}: input type is invalid.".format(self))
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            if 'gt_bbox_vis' in sample and len(sample['gt_bbox_vis']) > 0:
                sample['gt_bbox_vis'] = self.apply_bbox(sample['gt_bbox_vis'], width)
                sample['gt_bbox_ir'] = self.apply_bbox(sample['gt_bbox_ir'], width)
            if 'gt_poly' in sample and len(sample['gt_poly']) > 0:
                sample['gt_poly'] = self.apply_segm(sample['gt_poly'], height,
                                                    width)
            if 'gt_keypoint' in sample and len(sample['gt_keypoint']) > 0:
...
class RandomGrayscale_DAOD(BaseOperator):
    ...
    def apply(self, sample, context=None):
        if np.random.uniform(0, 1) < self.prob:
            from paddle.vision.transforms import Grayscale
            transform = Grayscale(num_output_channels=3)
            sample['vis_image_aug'] = transform(sample['vis_image_aug'])
            sample['ir_image_aug'] = transform(sample['ir_image_aug'])
        return sample

#只要触碰边界，无论高度和遮挡标签符合哪个子集的要求，该 GT 都会变成 Ignore. 
...
class KAISTParams(Params):
    ...
    def setDetParams(self):
        ...
        self.HtRng = [[55, 1e5 ** 2], [1, 1e5 ** 2], [1, 45], [45, 115], [115, 1e5 ** 2], [1, 1e5 ** 2], [1, 1e5 ** 2], [1, 1e5 ** 2]]
        self.OccRng = [[0, 1], [0, 1, 2], [0], [0], [0], [0], [1], [2]]
        self.SetupLbl = ['Reasonable', 'All', 'Far', 'Medium', 'Near', 'None', 'Partial', 'Heavy']
...
def evaluate(test_annotation_file: str, user_submission_file: str):
    ...
    all_eval_result = dict()
    metric_list = list()
    # for ind in range(len(kaistEval.params.SetupLbl)):
    for ind in range(8):
        eval_result = {
        ...
        setName = kaistEval.params.SetupLbl[ind]

        eval_result['all'].params.imgIds = imgIds
        eval_result['all'].evaluate(ind)
        eval_result['all'].accumulate()
        MR_all = eval_result['all'].summarize(ind)

        eval_result['day'].params.imgIds = imgIds[:1455]
        eval_result['day'].evaluate(ind)
        eval_result['day'].accumulate()
        MR_day = eval_result['day'].summarize(ind)

        eval_result['night'].params.imgIds = imgIds[1455:]
        eval_result['night'].evaluate(ind)
        eval_result['night'].accumulate()
        MR_night = eval_result['night'].summarize(ind)
        ...
        msg = f'\n########## Method: {method} : {setName} ##########\n' \
            ...
            + '######################################\n\n'
        print(msg)
        all_eval_result[setName] = eval_result
        metric_list.append(MR_all)
        metric_list.append(MR_day)
        metric_list.append(MR_night)

    return all_eval_result, metric_list
...
def draw_all(eval_results, filename='figure.jpg'):
    ...
    colors = [plt.colormaps['Paired'](ii)[:3] for ii in range(len(eval_results))]
    ...
    if not pdf:
        ...
        axes[1].set_title('Day', fontsize=30, weight='bold')

        KAISTPedEval.draw_figure(axes[2], eval_results_night, methods, colors)
        axes[2].set_title('Night', fontsize=30, weight='bold')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rstFiles', type=str, nargs='+', default=['state_of_arts/method_bbox.json'],
                        help='Please put the path of the result file. Only support json, txt format.')
    args = parser.parse_args()

    phase = "Multispectral"
    results = [evaluate(args.annFile, rstFile) for rstFile in args.rstFiles]

    results = [_[0] for _ in results]
    # Sort results by MR_all

    set_Names = [ 'All','Far', 'Medium', 'Near', 'None', 'Partial', 'Heavy']
    for ind, set_name in enumerate(set_Names):
        ...
        results_img_path = os.path.join('result_' + set_name + '.pdf')
